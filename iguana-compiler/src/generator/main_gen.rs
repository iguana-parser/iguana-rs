...
pub fn generate(grammar: &Grammar) -> TokenStream {
    ...
    quote! {
        ...
        struct Cli {
            ...
            #[arg(short = 'n', long = "nonterminal", value_name = "NAME", help_heading = "Parsing")]
            ...
            #[arg(long, value_enum, default_value_t, requires = "trace", help_heading = "Tracing")]
            format: TraceFormat,
            ...
            #[arg(long, value_name = "MS", default_value_t = 5.0, help_heading = "Corpus testing")]
            slow_ms: f64,

            /// Soft tolerance (percent) on the aggregate parse time for --corpus-test
            ///
            /// A larger drift is reported but does not fail
            #[arg(long, value_name = "PCT", default_value_t = 30.0, help_heading = "Corpus testing")]
            perf_tolerance: f64,
        ...
        fn main() -> Result<(), io::Error> {
            ...
            if args.check_sexpr || args.regenerate_sexpr {
                ...
                if args.write_parse_tree.is_some()
                    ...
                    || args.trace.is_some()
                {
                    eprintln!("Warning: output flags (--write-parse-tree, --write-sppf, --write-gss, --write-gss-nodes, --write-result, --write-stats, --vis, --trace) are ignored in golden mode.");
                }
            ...
            if let Some(filter) = &args.corpus_test {
                ...
                if args.start_nonterminal.is_some() {
                    eprintln!("Note: -n/--nonterminal is ignored with --corpus-test; the start nonterminal comes from repos.txt.");
                }
