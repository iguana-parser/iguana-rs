...
pub struct GenConfig {
    ...
    pub wasm: bool,
    /// When true, scaffold the namespaced license texts for Rust code generated
    /// by Iguana. Repository-owned fixtures can disable the copies because the
    /// repository license files already cover them.
    pub license_files: bool,
...
const GENERATED_LICENSE_MIT: &str = "IGUANA-GENERATED-LICENSE-MIT";
const GENERATED_LICENSE_APACHE: &str = "IGUANA-GENERATED-LICENSE-APACHE";
...
/// Scaffold Cargo.toml, generated-code license texts, and, for a CLI parser,
/// src/main.rs into `output_dir`.
///
...
/// lib sources. The namespaced license texts cover only Rust code generated
/// by Iguana and are written only when absent. For wasm bundles,
/// `generate_wasm` writes the pair at the bundle root instead. Cargo.toml is
/// configuration the user owns (dependencies, license, versions), so an
/// existing one is preserved; `force` overwrites it (useful when the
/// Cargo.toml template has changed and you want the new shape).
pub fn generate_scaffold(
    ...
    if config.license_files && !config.wasm {
        write_if_absent(LICENSE_MIT, &output_dir.join(GENERATED_LICENSE_MIT))?;
        write_if_absent(LICENSE_APACHE, &output_dir.join(GENERATED_LICENSE_APACHE))?;
    }
...
/// Write the wasm bundle into `output_dir`: the generated-code license texts
/// at the bundle root, the wrapper crate under `wasm/`, and `manifest.json`.
/// The parser lib sources and the root crate's Cargo.toml come from
/// `generate_sources` and `generate_scaffold`.
///
/// The license texts are emitted only when `config.license_files` is true and
/// are never overwritten. One pair at the bundle root covers both crates. The
/// wrapper `lib.rs` and `manifest.json` are derived from the grammar and always
/// overwritten. The wrapper `Cargo.toml` is scaffolding, so it is written only
/// when absent (or when `force` is set), preserving a hand-edited dependency
/// such as a local path to the runtime.
...
fn write_if_absent(content: &str, path: &Path) -> io::Result<()> {
    if !path.exists() {
...
mod tests {
    ...
    #[test]
    fn force_does_not_overwrite_license_or_notice_files() {
        let nonce = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system clock should be after the Unix epoch")
            .as_nanos();
        let dir = env::temp_dir().join(format!(
            "iguana-generated-license-{}-{nonce}",
            process::id()
        ));
        fs::create_dir(&dir).expect("temporary directory should be created");
        let generated_license = dir.join(GENERATED_LICENSE_MIT);
        let project_license = dir.join("LICENSE-MIT");
        let project_notice = dir.join("NOTICE");

        fs::write(&generated_license, "custom generated-code terms")
            .expect("generated license should be written");
        ...
        assert_eq!(
            fs::read_to_string(&generated_license).expect("generated license should be readable"),
            "custom generated-code terms"
        );
        assert_eq!(
    ...
    #[test]
    fn wasm_license_files_are_optional_and_only_at_bundle_root() {
        let nonce = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system clock should be after the Unix epoch")
            .as_nanos();
        ...
        for license_files in [true, false] {
            let dir = env::temp_dir().join(format!(
                "iguana-wasm-license-{}-{nonce}-{license_files}",
                process::id()
            ));
            ...
            generate_wasm(&grammar, &dir, config, None, false)
                .expect("wasm scaffolding should succeed");

            assert_eq!(dir.join(GENERATED_LICENSE_MIT).exists(), license_files);
            assert_eq!(dir.join(GENERATED_LICENSE_APACHE).exists(), license_files);
            assert!(!dir.join("wasm").join(GENERATED_LICENSE_MIT).exists());
            assert!(!dir.join("wasm").join(GENERATED_LICENSE_APACHE).exists());
            fs::remove_dir_all(dir).expect("temporary directory should be removed");
        }
    }
