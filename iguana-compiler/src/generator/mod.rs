...
/// The license note for generated output. Generated parsers reproduce
/// fragments of the generator's GPL-licensed source, and this note is the one
/// license statement a user of a generated parser sees, so it states the
/// effect first: the output is theirs to license.
const LICENSE_NOTE_RUST: &str = "\
...
fn write_rust_file(content: impl AsRef<str>, path: &Path) -> io::Result<()> {
    ...
    file.write_all(LICENSE_NOTE_RUST.as_bytes())?;
    file.write_all(b"\n")?;
