...
- `npm run check` — type-check every npm workspace (`web-ui`, `web-viewer`, `design-viewer`, `terrarium`, `tree-widget`) with `svelte-check`, or `tsc` for the tree widget. Fails on type errors, reports warnings without failing. Run it after any frontend change; CI runs the same command.
