// @generated by iguana. Do not edit manually.

use crate::{
    grammar::*,
    parse_tree::{self, IggyParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::IggyScanner,
};
#[cfg(feature = "instrument")]
use iguana_runtime::grammar::Grammar;
#[allow(unused_imports)]
use iguana_runtime::input::Span;
#[cfg(feature = "debug-trace")]
use iguana_runtime::trace::TraceEvent;
use iguana_runtime::{
    arena::{Arena, ArenaVec},
    descriptor::Descriptor,
    env::{Env, EnvId},
    gss::GSSNode,
    ids::{BindingId, GssNodeId, NonterminalId, SlotId, TerminalId},
    input::Input,
    parser::{
        DESCRIPTORS_CAPACITY_DIVISOR, DESCRIPTORS_CAPACITY_FLOOR, ENVS_CAPACITY_MULTIPLIER,
        GLLFailure, GLLFailureKind, GSS_CAPACITY_MULTIPLIER, Parser, SPPF_CAPACITY_MULTIPLIER,
        init_logger,
    },
    record,
    result::{ParseError, ParseSuccess},
    scanner::{Scanner, TerminalSet},
    sppf::{IntermediateNode, NonterminalNode, SPPFNode, SPPFNodeId, TerminalNode},
    utils::{inline_map::InlineMap, inline_vec::InlineVec},
};
use rustc_hash::FxHashMap;
use std::cell::OnceCell;
const BINDING_P: BindingId = BindingId(0);
const BINDING_E: BindingId = BindingId(1);
const BINDING_L_PR: BindingId = BindingId(2);
const BINDING_L_LABEL: BindingId = BindingId(3);
const BINDING_R_PR: BindingId = BindingId(4);
const BINDING_R_LABEL: BindingId = BindingId(5);
impl<'i, 'arena> Parser<'i, 'arena> for IggyParser<'i, 'arena> {
    type Grammar = IggyGrammar;
    type ConcreteParser<'input, 'parser_arena> = IggyParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        IggyParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = IggyParseTreeBuilder::new(tree_arena);
        create_parse_tree(root, self, &builder)
    }
    // The GLL main loop. Each descriptor starts at its slot. A slot that reaches the next slot
    // of its alternative sets `next`, so a descriptor runs through its alternative as far as it
    // can before the loop takes the next descriptor. The GSS node and the environment are fixed
    // for the whole alternative.
    fn execute(&mut self) {
        while let Some(descriptor) = self.next_descriptor() {
            let mut input_index = descriptor.input_index;
            let mut result = descriptor.sppf_node_id();
            let gss_node_id = descriptor.gss_node_id;
            let env = descriptor.env_id();
            let mut next = Some(descriptor.slot_id);
            while let Some(slot_id) = next {
                next = None;
                record!(
                    self,
                    ProcessingDescriptor,
                    input_index,
                    slot_id,
                    result,
                    gss_node_id
                );
                match slot_id {
                    // Grammar : . "grammar" Layout name:Identifier Layout Star_0
                    SlotId(0) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(11),
                            input_index,
                            SlotId(0),
                            Some(gss_node_id),
                        ) {
                            // Grammar : "grammar" . Layout name:Identifier Layout Star_0
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(1));
                        }
                    }
                    // Grammar : "grammar" . Layout name:Identifier Layout Star_0
                    SlotId(1) => {
                        if let Some(right_child) =
                            self.parse_layout_ll1(input_index, Some((SlotId(1), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(2), env)
                            {
                                // Grammar : "grammar" Layout . name:Identifier Layout Star_0
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(2));
                            }
                        }
                    }
                    // Grammar : "grammar" Layout . name:Identifier Layout Star_0
                    SlotId(2) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(2),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(3), env)
                            {
                                // Grammar : "grammar" Layout name:Identifier . Layout Star_0
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(3));
                            }
                        }
                    }
                    // Grammar : "grammar" Layout name:Identifier . Layout Star_0
                    SlotId(3) => {
                        if let Some(right_child) =
                            self.parse_layout_ll1(input_index, Some((SlotId(3), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(4), env)
                            {
                                // Grammar : "grammar" Layout name:Identifier Layout . Star_0
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(4));
                            }
                        }
                    }
                    // Grammar : "grammar" Layout name:Identifier Layout . Star_0
                    SlotId(4) => {
                        self.create(NonterminalId(19), result, gss_node_id, SlotId(5), env);
                    }
                    // Grammar : "grammar" Layout name:Identifier Layout Star_0.
                    SlotId(5) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(0),
                            SlotId(5),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(5), nonterminal_node_id, None);
                    }
                    // Rule : . SyntaxRule
                    SlotId(6) => {
                        self.create(NonterminalId(2), result, gss_node_id, SlotId(7), env);
                    }
                    // Rule : SyntaxRule.
                    SlotId(7) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(1),
                            SlotId(7),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(7), nonterminal_node_id, None);
                    }
                    // Rule : . RegexRule
                    SlotId(8) => {
                        self.create(NonterminalId(4), result, gss_node_id, SlotId(9), env);
                    }
                    // Rule : RegexRule.
                    SlotId(9) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(1),
                            SlotId(9),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(9), nonterminal_node_id, None);
                    }
                    // SyntaxRule : . Opt_1 Layout head:Identifier Layout "=" Layout Star_1
                    SlotId(10) => {
                        if let Some(right_child) =
                            self.parse_opt_1_ll1(input_index, Some((SlotId(10), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // SyntaxRule : Opt_1 . Layout head:Identifier Layout "=" Layout Star_1
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(11));
                        }
                    }
                    // SyntaxRule : Opt_1 . Layout head:Identifier Layout "=" Layout Star_1
                    SlotId(11) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(11), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(12), env)
                            {
                                // SyntaxRule : Opt_1 Layout . head:Identifier Layout "=" Layout Star_1
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(12));
                            }
                        }
                    }
                    // SyntaxRule : Opt_1 Layout . head:Identifier Layout "=" Layout Star_1
                    SlotId(12) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(12),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(13), env)
                            {
                                // SyntaxRule : Opt_1 Layout head:Identifier . Layout "=" Layout Star_1
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(13));
                            }
                        }
                    }
                    // SyntaxRule : Opt_1 Layout head:Identifier . Layout "=" Layout Star_1
                    SlotId(13) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(13), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(14), env)
                            {
                                // SyntaxRule : Opt_1 Layout head:Identifier Layout . "=" Layout Star_1
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(14));
                            }
                        }
                    }
                    // SyntaxRule : Opt_1 Layout head:Identifier Layout . "=" Layout Star_1
                    SlotId(14) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(12),
                            input_index,
                            SlotId(14),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(15), env)
                            {
                                // SyntaxRule : Opt_1 Layout head:Identifier Layout "=" . Layout Star_1
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(15));
                            }
                        }
                    }
                    // SyntaxRule : Opt_1 Layout head:Identifier Layout "=" . Layout Star_1
                    SlotId(15) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(15), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(16), env)
                            {
                                // SyntaxRule : Opt_1 Layout head:Identifier Layout "=" Layout . Star_1
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(16));
                            }
                        }
                    }
                    // SyntaxRule : Opt_1 Layout head:Identifier Layout "=" Layout . Star_1
                    SlotId(16) => {
                        self.create(NonterminalId(23), result, gss_node_id, SlotId(17), env);
                    }
                    // SyntaxRule : Opt_1 Layout head:Identifier Layout "=" Layout Star_1.
                    SlotId(17) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(2),
                            SlotId(17),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(17), nonterminal_node_id, None);
                    }
                    // Annotation : . "@NoLayout"
                    SlotId(18) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(14),
                            input_index,
                            SlotId(18),
                            Some(gss_node_id),
                        ) {
                            // Annotation : "@NoLayout".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(19));
                        }
                    }
                    // Annotation : "@NoLayout".
                    SlotId(19) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(3),
                            SlotId(19),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(19), nonterminal_node_id, None);
                    }
                    // Annotation : . "@Layout"
                    SlotId(20) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(15),
                            input_index,
                            SlotId(20),
                            Some(gss_node_id),
                        ) {
                            // Annotation : "@Layout".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(21));
                        }
                    }
                    // Annotation : "@Layout".
                    SlotId(21) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(3),
                            SlotId(21),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(21), nonterminal_node_id, None);
                    }
                    // RegexRule : . layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(22) => {
                        if let Some(right_child) =
                            self.parse_opt_3_ll1(input_index, Some((SlotId(22), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // RegexRule : layout:Opt_3 . Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                            // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(23));
                        }
                    }
                    // RegexRule : layout:Opt_3 . Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(23) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(23), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(24), env)
                            {
                                // RegexRule : layout:Opt_3 Layout . id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(24));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout . id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(24) => {
                        if let Some(right_child) =
                            self.parse_opt_4_ll1(input_index, Some((SlotId(24), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(25), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 . Layout "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(25));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 . Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(25) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(25), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(26), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout . "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(26));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout . "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(26) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(17),
                            input_index,
                            SlotId(26),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(27), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" . Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(27));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" . Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(27) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(27), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(28), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout . Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(28));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout . Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(28) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(28),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(29), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier . Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(29));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier . Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(29) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(29), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(30), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout .
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(30));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout .
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(30) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(12),
                            input_index,
                            SlotId(30),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(31), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" . Layout Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(31));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" . Layout Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(31) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(31), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(32), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" Layout . Opt_5 Layout body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(32));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout . Opt_5 Layout body:Plus_2 Layout Star_2
                    SlotId(32) => {
                        self.create(NonterminalId(26), result, gss_node_id, SlotId(33), env);
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 . Layout body:Plus_2 Layout Star_2
                    SlotId(33) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(33), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(34), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout . body:Plus_2 Layout Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(34));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout . body:Plus_2 Layout Star_2
                    SlotId(34) => {
                        self.create(NonterminalId(28), result, gss_node_id, SlotId(35), env);
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 . Layout Star_2
                    SlotId(35) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(35), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(36), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout . Star_2
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(36));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout . Star_2
                    SlotId(36) => {
                        if let Some(right_child) = self
                            .parse_star_2_ll1(input_index, Some((SlotId(36), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(37), env)
                            {
                                // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                                // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(37));
                            }
                        }
                    }
                    // RegexRule : layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
                    // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2.
                    SlotId(37) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(4),
                            SlotId(37),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(37), nonterminal_node_id, None);
                    }
                    // RegexPreCondition : . Identifier Layout "!<<"
                    SlotId(38) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(38),
                            Some(gss_node_id),
                        ) {
                            // RegexPreCondition : Identifier . Layout "!<<"
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(39));
                        }
                    }
                    // RegexPreCondition : Identifier . Layout "!<<"
                    SlotId(39) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(39), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(40), env)
                            {
                                // RegexPreCondition : Identifier Layout . "!<<"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(40));
                            }
                        }
                    }
                    // RegexPreCondition : Identifier Layout . "!<<"
                    SlotId(40) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(19),
                            input_index,
                            SlotId(40),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(41), env)
                            {
                                // RegexPreCondition : Identifier Layout "!<<".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(41));
                            }
                        }
                    }
                    // RegexPreCondition : Identifier Layout "!<<".
                    SlotId(41) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(5),
                            SlotId(41),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(41), nonterminal_node_id, None);
                    }
                    // RegexPostCondition : . "\\" Layout Identifier
                    SlotId(42) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(20),
                            input_index,
                            SlotId(42),
                            Some(gss_node_id),
                        ) {
                            // RegexPostCondition : "\\" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(43));
                        }
                    }
                    // RegexPostCondition : "\\" . Layout Identifier
                    SlotId(43) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(43), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(44), env)
                            {
                                // RegexPostCondition : "\\" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(44));
                            }
                        }
                    }
                    // RegexPostCondition : "\\" Layout . Identifier
                    SlotId(44) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(44),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(45), env)
                            {
                                // RegexPostCondition : "\\" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(45));
                            }
                        }
                    }
                    // RegexPostCondition : "\\" Layout Identifier.
                    SlotId(45) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(6),
                            SlotId(45),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(45), nonterminal_node_id, None);
                    }
                    // RegexPostCondition : . "!>>" Layout Identifier
                    SlotId(46) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(21),
                            input_index,
                            SlotId(46),
                            Some(gss_node_id),
                        ) {
                            // RegexPostCondition : "!>>" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(47));
                        }
                    }
                    // RegexPostCondition : "!>>" . Layout Identifier
                    SlotId(47) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(47), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(48), env)
                            {
                                // RegexPostCondition : "!>>" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(48));
                            }
                        }
                    }
                    // RegexPostCondition : "!>>" Layout . Identifier
                    SlotId(48) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(48),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(49), env)
                            {
                                // RegexPostCondition : "!>>" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(49));
                            }
                        }
                    }
                    // RegexPostCondition : "!>>" Layout Identifier.
                    SlotId(49) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(6),
                            SlotId(49),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(49), nonterminal_node_id, None);
                    }
                    // PriorityLevel : . Opt_7 Layout Star_3
                    SlotId(50) => {
                        self.create(NonterminalId(32), result, gss_node_id, SlotId(51), env);
                    }
                    // PriorityLevel : Opt_7 . Layout Star_3
                    SlotId(51) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(51), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(52), env)
                            {
                                // PriorityLevel : Opt_7 Layout . Star_3
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(52));
                            }
                        }
                    }
                    // PriorityLevel : Opt_7 Layout . Star_3
                    SlotId(52) => {
                        self.create(NonterminalId(35), result, gss_node_id, SlotId(53), env);
                    }
                    // PriorityLevel : Opt_7 Layout Star_3.
                    SlotId(53) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(7),
                            SlotId(53),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(53), nonterminal_node_id, None);
                    }
                    // Associativity : . "left"
                    SlotId(54) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(22),
                            input_index,
                            SlotId(54),
                            Some(gss_node_id),
                        ) {
                            // Associativity : "left".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(55));
                        }
                    }
                    // Associativity : "left".
                    SlotId(55) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(8),
                            SlotId(55),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(55), nonterminal_node_id, None);
                    }
                    // Associativity : . "right"
                    SlotId(56) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(23),
                            input_index,
                            SlotId(56),
                            Some(gss_node_id),
                        ) {
                            // Associativity : "right".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(57));
                        }
                    }
                    // Associativity : "right".
                    SlotId(57) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(8),
                            SlotId(57),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(57), nonterminal_node_id, None);
                    }
                    // Associativity : . "none"
                    SlotId(58) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(24),
                            input_index,
                            SlotId(58),
                            Some(gss_node_id),
                        ) {
                            // Associativity : "none".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(59));
                        }
                    }
                    // Associativity : "none".
                    SlotId(59) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(8),
                            SlotId(59),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(59), nonterminal_node_id, None);
                    }
                    // Alternative : . Plus_6 Layout Opt_9
                    SlotId(60) => {
                        self.create(NonterminalId(36), result, gss_node_id, SlotId(61), env);
                    }
                    // Alternative : Plus_6 . Layout Opt_9
                    SlotId(61) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(61), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(62), env)
                            {
                                // Alternative : Plus_6 Layout . Opt_9
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(62));
                            }
                        }
                    }
                    // Alternative : Plus_6 Layout . Opt_9
                    SlotId(62) => {
                        if let Some(right_child) =
                            self.parse_opt_9_ll1(input_index, Some((SlotId(62), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(63), env)
                            {
                                // Alternative : Plus_6 Layout Opt_9.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(63));
                            }
                        }
                    }
                    // Alternative : Plus_6 Layout Opt_9.
                    SlotId(63) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(9),
                            SlotId(63),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(63), nonterminal_node_id, None);
                    }
                    // Alternative : . "(" Layout ")" Layout Opt_9
                    SlotId(64) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(25),
                            input_index,
                            SlotId(64),
                            Some(gss_node_id),
                        ) {
                            // Alternative : "(" . Layout ")" Layout Opt_9
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(65));
                        }
                    }
                    // Alternative : "(" . Layout ")" Layout Opt_9
                    SlotId(65) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(65), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(66), env)
                            {
                                // Alternative : "(" Layout . ")" Layout Opt_9
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(66));
                            }
                        }
                    }
                    // Alternative : "(" Layout . ")" Layout Opt_9
                    SlotId(66) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(26),
                            input_index,
                            SlotId(66),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(67), env)
                            {
                                // Alternative : "(" Layout ")" . Layout Opt_9
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(67));
                            }
                        }
                    }
                    // Alternative : "(" Layout ")" . Layout Opt_9
                    SlotId(67) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(67), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(68), env)
                            {
                                // Alternative : "(" Layout ")" Layout . Opt_9
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(68));
                            }
                        }
                    }
                    // Alternative : "(" Layout ")" Layout . Opt_9
                    SlotId(68) => {
                        if let Some(right_child) =
                            self.parse_opt_9_ll1(input_index, Some((SlotId(68), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(69), env)
                            {
                                // Alternative : "(" Layout ")" Layout Opt_9.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(69));
                            }
                        }
                    }
                    // Alternative : "(" Layout ")" Layout Opt_9.
                    SlotId(69) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(9),
                            SlotId(69),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(69), nonterminal_node_id, None);
                    }
                    // Symbol(p: i32, e: i32) : . [1 & e == 0] Identifier return (0, 0)
                    SlotId(333) => {
                        if (1) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(334));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1 & e == 0] . Identifier return (0, 0)
                    SlotId(334) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(334),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [1 & e == 0] Identifier . return (0, 0)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(335));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1 & e == 0] Identifier . return (0, 0)
                    SlotId(335) => {
                        next = Some(SlotId(336));
                    }
                    // Symbol(p: i32, e: i32) : [1 & e == 0] Identifier return (0, 0).
                    SlotId(336) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 0;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(336),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(336),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [2 & e == 0] "(" Layout seqs:Plus_7 Layout ")" return (0, 1)
                    SlotId(337) => {
                        if (2) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(338));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] . "(" Layout seqs:Plus_7 Layout ")" return (0, 1)
                    SlotId(338) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(25),
                            input_index,
                            SlotId(338),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [2 & e == 0] "(" . Layout seqs:Plus_7 Layout ")" return (0, 1)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(339));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" . Layout seqs:Plus_7 Layout ")" return (0, 1)
                    SlotId(339) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(339), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(340), env)
                            {
                                // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout . seqs:Plus_7 Layout ")" return (0, 1)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(340));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout . seqs:Plus_7 Layout ")" return (0, 1)
                    SlotId(340) => {
                        self.create(NonterminalId(38), result, gss_node_id, SlotId(341), env);
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 . Layout ")" return (0, 1)
                    SlotId(341) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(341), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(342), env)
                            {
                                // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 Layout . ")" return (0, 1)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(342));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 Layout . ")" return (0, 1)
                    SlotId(342) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(26),
                            input_index,
                            SlotId(342),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(343), env)
                            {
                                // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 Layout ")" . return (0, 1)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(343));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 Layout ")" . return (0, 1)
                    SlotId(343) => {
                        next = Some(SlotId(344));
                    }
                    // Symbol(p: i32, e: i32) : [2 & e == 0] "(" Layout seqs:Plus_7 Layout ")" return (0, 1).
                    SlotId(344) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 1;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(344),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(344),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [4 & e == 0] String return (0, 2)
                    SlotId(345) => {
                        if (4) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(346));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [4 & e == 0] . String return (0, 2)
                    SlotId(346) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(3),
                            input_index,
                            SlotId(346),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [4 & e == 0] String . return (0, 2)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(347));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [4 & e == 0] String . return (0, 2)
                    SlotId(347) => {
                        next = Some(SlotId(348));
                    }
                    // Symbol(p: i32, e: i32) : [4 & e == 0] String return (0, 2).
                    SlotId(348) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 2;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(348),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(348),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(349) => {
                        if (8) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(350));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] . "{" Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(350) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(27),
                            input_index,
                            SlotId(350),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [8 & e == 0] "{" . Layout symbol:Symbol(0, 0) Layout
                            // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(351));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" . Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(351) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(351), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(352), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout . symbol:Symbol(0, 0) Layout
                                // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(352));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout . symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(352) => {
                        self.create_symbol(result, gss_node_id, SlotId(353), env, 0, 0);
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) . Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(353) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(353), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(354), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout .
                                // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(354));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout .
                    // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                    SlotId(354) => {
                        self.create_symbol(result, gss_node_id, SlotId(355), env, 0, 0);
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) . Layout "}" Layout "*" return (0, 3)
                    SlotId(355) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(355), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(356), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout . "}" Layout "*" return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(356));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout . "}" Layout "*" return (0, 3)
                    SlotId(356) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(28),
                            input_index,
                            SlotId(356),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(357), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" . Layout "*" return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(357));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" . Layout "*" return (0, 3)
                    SlotId(357) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(357), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(358), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" Layout . "*" return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(358));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout . "*" return (0, 3)
                    SlotId(358) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(29),
                            input_index,
                            SlotId(358),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(359), env)
                            {
                                // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" Layout "*" . return (0, 3)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(359));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout "*" . return (0, 3)
                    SlotId(359) => {
                        next = Some(SlotId(360));
                    }
                    // Symbol(p: i32, e: i32) : [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout "*" return (0, 3).
                    SlotId(360) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 3;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(360),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(360),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(361) => {
                        if (16) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(362));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] . "{" Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(362) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(27),
                            input_index,
                            SlotId(362),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [16 & e == 0] "{" . Layout symbol:Symbol(0, 0) Layout
                            // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(363));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" . Layout symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(363) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(363), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(364), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout . symbol:Symbol(0, 0) Layout
                                // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(364));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout . symbol:Symbol(0, 0) Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(364) => {
                        self.create_symbol(result, gss_node_id, SlotId(365), env, 0, 0);
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) . Layout
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(365) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(365), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(366), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout .
                                // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(366));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout .
                    // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                    SlotId(366) => {
                        self.create_symbol(result, gss_node_id, SlotId(367), env, 0, 0);
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) . Layout "}" Layout "+" return (0, 4)
                    SlotId(367) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(367), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(368), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout . "}" Layout "+" return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(368));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout . "}" Layout "+" return (0, 4)
                    SlotId(368) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(28),
                            input_index,
                            SlotId(368),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(369), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" . Layout "+" return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(369));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" . Layout "+" return (0, 4)
                    SlotId(369) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(369), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(370), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" Layout . "+" return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(370));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout . "+" return (0, 4)
                    SlotId(370) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(30),
                            input_index,
                            SlotId(370),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(371), env)
                            {
                                // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                                // 0) Layout "}" Layout "+" . return (0, 4)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(371));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout "+" . return (0, 4)
                    SlotId(371) => {
                        next = Some(SlotId(372));
                    }
                    // Symbol(p: i32, e: i32) : [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout sep:Symbol(0,
                    // 0) Layout "}" Layout "+" return (0, 4).
                    SlotId(372) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 4;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(372),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(372),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "*" return (0, 5)
                    SlotId(373) => {
                        if (32) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(374));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] . [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "*" return (0, 5)
                    SlotId(374) => {
                        if 3 >= self.lookup(BINDING_P, env.unwrap()) {
                            next = Some(SlotId(375));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] . (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "*" return (0, 5)
                    SlotId(375) => {
                        self.create_symbol(
                            result,
                            gss_node_id,
                            SlotId(376),
                            env,
                            self.lookup(BINDING_P, env.unwrap()),
                            0,
                        );
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) . [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "*" return (0, 5)
                    SlotId(376) => {
                        if (self.lookup(BINDING_L_PR, env.unwrap()) == 0)
                            || (self.lookup(BINDING_L_PR, env.unwrap()) >= 3)
                        {
                            next = Some(SlotId(377));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] . Layout "*" return (0, 5)
                    SlotId(377) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(377), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(378), env)
                            {
                                // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout . "*" return (0, 5)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(378));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout . "*" return (0, 5)
                    SlotId(378) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(29),
                            input_index,
                            SlotId(378),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(379), env)
                            {
                                // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout "*" . return (0, 5)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(379));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "*" . return (0, 5)
                    SlotId(379) => {
                        next = Some(SlotId(380));
                    }
                    // Symbol(p: i32, e: i32) : [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "*" return (0, 5).
                    SlotId(380) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 5;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(380),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(380),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "+" return (0, 6)
                    SlotId(381) => {
                        if (64) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(382));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] . [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "+" return (0, 6)
                    SlotId(382) => {
                        if 3 >= self.lookup(BINDING_P, env.unwrap()) {
                            next = Some(SlotId(383));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] . (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "+" return (0, 6)
                    SlotId(383) => {
                        self.create_symbol(
                            result,
                            gss_node_id,
                            SlotId(384),
                            env,
                            self.lookup(BINDING_P, env.unwrap()),
                            0,
                        );
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) . [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "+" return (0, 6)
                    SlotId(384) => {
                        if (self.lookup(BINDING_L_PR, env.unwrap()) == 0)
                            || (self.lookup(BINDING_L_PR, env.unwrap()) >= 3)
                        {
                            next = Some(SlotId(385));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] . Layout "+" return (0, 6)
                    SlotId(385) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(385), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(386), env)
                            {
                                // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout . "+" return (0, 6)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(386));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout . "+" return (0, 6)
                    SlotId(386) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(30),
                            input_index,
                            SlotId(386),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(387), env)
                            {
                                // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout "+" . return (0, 6)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(387));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "+" . return (0, 6)
                    SlotId(387) => {
                        next = Some(SlotId(388));
                    }
                    // Symbol(p: i32, e: i32) : [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "+" return (0, 6).
                    SlotId(388) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 6;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(388),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(388),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "?" return (0, 7)
                    SlotId(389) => {
                        if (128) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(390));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] . [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "?" return (0, 7)
                    SlotId(390) => {
                        if 3 >= self.lookup(BINDING_P, env.unwrap()) {
                            next = Some(SlotId(391));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] . (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "?" return (0, 7)
                    SlotId(391) => {
                        self.create_symbol(
                            result,
                            gss_node_id,
                            SlotId(392),
                            env,
                            self.lookup(BINDING_P, env.unwrap()),
                            0,
                        );
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) . [(l_pr ==
                    // 0) || (l_pr >= 3)] Layout "?" return (0, 7)
                    SlotId(392) => {
                        if (self.lookup(BINDING_L_PR, env.unwrap()) == 0)
                            || (self.lookup(BINDING_L_PR, env.unwrap()) >= 3)
                        {
                            next = Some(SlotId(393));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] . Layout "?" return (0, 7)
                    SlotId(393) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(393), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(394), env)
                            {
                                // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout . "?" return (0, 7)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(394));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout . "?" return (0, 7)
                    SlotId(394) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(31),
                            input_index,
                            SlotId(394),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(395), env)
                            {
                                // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] Layout "?" . return (0, 7)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(395));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "?" . return (0, 7)
                    SlotId(395) => {
                        next = Some(SlotId(396));
                    }
                    // Symbol(p: i32, e: i32) : [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] Layout "?" return (0, 7).
                    SlotId(396) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 7;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(396),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(396),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(397) => {
                        if (256) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(398));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] . [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(398) => {
                        if 3 >= self.lookup(BINDING_P, env.unwrap()) {
                            next = Some(SlotId(399));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] . (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                    // 0) || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(399) => {
                        self.create_symbol(
                            result,
                            gss_node_id,
                            SlotId(400),
                            env,
                            self.lookup(BINDING_P, env.unwrap()),
                            0,
                        );
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) . [(l_pr ==
                    // 0) || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(400) => {
                        if (self.lookup(BINDING_L_PR, env.unwrap()) == 0)
                            || (self.lookup(BINDING_L_PR, env.unwrap()) >= 3)
                        {
                            next = Some(SlotId(401));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] . [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(401) => {
                        if (self.lookup(BINDING_L_LABEL, env.unwrap()) == 31)
                            || (((256) >> (self.lookup(BINDING_L_LABEL, env.unwrap()))) & (1) == 0)
                        {
                            next = Some(SlotId(402));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] . Layout
                    // conditions:Plus_8 return (0, 8)
                    SlotId(402) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(402), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(403), env)
                            {
                                // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                                // || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout .
                                // conditions:Plus_8 return (0, 8)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(403));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout .
                    // conditions:Plus_8 return (0, 8)
                    SlotId(403) => {
                        self.create(NonterminalId(39), result, gss_node_id, SlotId(404), env);
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 . return (0, 8)
                    SlotId(404) => {
                        next = Some(SlotId(405));
                    }
                    // Symbol(p: i32, e: i32) : [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr == 0)
                    // || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                    // conditions:Plus_8 return (0, 8).
                    SlotId(405) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 8;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(405),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(405),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [512 & e == 0] conditions:Plus_9 Layout (r_pr,
                    // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                    SlotId(406) => {
                        if (512) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(407));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [512 & e == 0] . conditions:Plus_9 Layout (r_pr,
                    // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                    SlotId(407) => {
                        self.create(NonterminalId(40), result, gss_node_id, SlotId(408), env);
                    }
                    // Symbol(p: i32, e: i32) : [512 & e == 0] conditions:Plus_9 . Layout (r_pr,
                    // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                    SlotId(408) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(408), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(409), env)
                            {
                                // Symbol(p: i32, e: i32) : [512 & e == 0] conditions:Plus_9 Layout . (r_pr,
                                // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(409));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [512 & e == 0] conditions:Plus_9 Layout . (r_pr,
                    // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                    SlotId(409) => {
                        self.create_symbol(result, gss_node_id, SlotId(410), env, 2, 512);
                    }
                    // Symbol(p: i32, e: i32) : [512 & e == 0] conditions:Plus_9 Layout (r_pr, r_label)=Symbol(2,
                    // 512) . return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                    SlotId(410) => {
                        next = Some(SlotId(411));
                    }
                    // Symbol(p: i32, e: i32) : [512 & e == 0] conditions:Plus_9 Layout (r_pr, r_label)=Symbol(2,
                    // 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9).
                    SlotId(411) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = ((if self.lookup(BINDING_R_PR, env.unwrap()) == 0 {
                            2
                        } else {
                            std::cmp::min(self.lookup(BINDING_R_PR, env.unwrap()), 2)
                        }) << (16))
                            | (9);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(411),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(411),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // Symbol(p: i32, e: i32) : . [1024 & e == 0] label:Identifier Layout ":" Layout Symbol(1, 0)
                    // return (1, 10)
                    SlotId(412) => {
                        if (1024) & (self.lookup(BINDING_E, env.unwrap())) == 0 {
                            next = Some(SlotId(413));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] . label:Identifier Layout ":" Layout Symbol(1, 0)
                    // return (1, 10)
                    SlotId(413) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(413),
                            Some(gss_node_id),
                        ) {
                            // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier . Layout ":" Layout Symbol(1, 0)
                            // return (1, 10)
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(414));
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier . Layout ":" Layout Symbol(1, 0)
                    // return (1, 10)
                    SlotId(414) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(414), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(415), env)
                            {
                                // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout . ":" Layout Symbol(1, 0)
                                // return (1, 10)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(415));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout . ":" Layout Symbol(1, 0)
                    // return (1, 10)
                    SlotId(415) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(32),
                            input_index,
                            SlotId(415),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(416), env)
                            {
                                // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" . Layout Symbol(1, 0)
                                // return (1, 10)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(416));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" . Layout Symbol(1, 0)
                    // return (1, 10)
                    SlotId(416) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(416), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(417), env)
                            {
                                // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" Layout . Symbol(1, 0)
                                // return (1, 10)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(417));
                            }
                        }
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" Layout . Symbol(1, 0)
                    // return (1, 10)
                    SlotId(417) => {
                        self.create_symbol(result, gss_node_id, SlotId(418), env, 1, 0);
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" Layout Symbol(1, 0) .
                    // return (1, 10)
                    SlotId(418) => {
                        next = Some(SlotId(419));
                    }
                    // Symbol(p: i32, e: i32) : [1024 & e == 0] label:Identifier Layout ":" Layout Symbol(1, 0)
                    // return (1, 10).
                    SlotId(419) => {
                        let Some(result) = result else {
                            unreachable!("result cannot be None here.")
                        };
                        let node = self.sppf_node(result);
                        let return_value = 65546;
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(64),
                            SlotId(419),
                            node.left_extent(),
                            node.right_extent(),
                            result,
                            gss_node_id,
                            Some(return_value),
                        );
                        self.pop(
                            gss_node_id,
                            SlotId(419),
                            nonterminal_node_id,
                            Some(return_value),
                        );
                    }
                    // PreCondition : . Identifier Layout "!<<"
                    SlotId(70) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(70),
                            Some(gss_node_id),
                        ) {
                            // PreCondition : Identifier . Layout "!<<"
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(71));
                        }
                    }
                    // PreCondition : Identifier . Layout "!<<"
                    SlotId(71) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(71), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(72), env)
                            {
                                // PreCondition : Identifier Layout . "!<<"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(72));
                            }
                        }
                    }
                    // PreCondition : Identifier Layout . "!<<"
                    SlotId(72) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(19),
                            input_index,
                            SlotId(72),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(73), env)
                            {
                                // PreCondition : Identifier Layout "!<<".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(73));
                            }
                        }
                    }
                    // PreCondition : Identifier Layout "!<<".
                    SlotId(73) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(10),
                            SlotId(73),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(73), nonterminal_node_id, None);
                    }
                    // PostCondition : . "\\" Layout Identifier
                    SlotId(74) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(20),
                            input_index,
                            SlotId(74),
                            Some(gss_node_id),
                        ) {
                            // PostCondition : "\\" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(75));
                        }
                    }
                    // PostCondition : "\\" . Layout Identifier
                    SlotId(75) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(75), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(76), env)
                            {
                                // PostCondition : "\\" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(76));
                            }
                        }
                    }
                    // PostCondition : "\\" Layout . Identifier
                    SlotId(76) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(76),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(77), env)
                            {
                                // PostCondition : "\\" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(77));
                            }
                        }
                    }
                    // PostCondition : "\\" Layout Identifier.
                    SlotId(77) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(11),
                            SlotId(77),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(77), nonterminal_node_id, None);
                    }
                    // PostCondition : . "!>>" Layout Identifier
                    SlotId(78) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(21),
                            input_index,
                            SlotId(78),
                            Some(gss_node_id),
                        ) {
                            // PostCondition : "!>>" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(79));
                        }
                    }
                    // PostCondition : "!>>" . Layout Identifier
                    SlotId(79) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(79), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(80), env)
                            {
                                // PostCondition : "!>>" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(80));
                            }
                        }
                    }
                    // PostCondition : "!>>" Layout . Identifier
                    SlotId(80) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(80),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(81), env)
                            {
                                // PostCondition : "!>>" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(81));
                            }
                        }
                    }
                    // PostCondition : "!>>" Layout Identifier.
                    SlotId(81) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(11),
                            SlotId(81),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(81), nonterminal_node_id, None);
                    }
                    // PostCondition : . "!>>>" Layout Identifier
                    SlotId(82) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(33),
                            input_index,
                            SlotId(82),
                            Some(gss_node_id),
                        ) {
                            // PostCondition : "!>>>" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(83));
                        }
                    }
                    // PostCondition : "!>>>" . Layout Identifier
                    SlotId(83) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(83), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(84), env)
                            {
                                // PostCondition : "!>>>" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(84));
                            }
                        }
                    }
                    // PostCondition : "!>>>" Layout . Identifier
                    SlotId(84) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(84),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(85), env)
                            {
                                // PostCondition : "!>>>" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(85));
                            }
                        }
                    }
                    // PostCondition : "!>>>" Layout Identifier.
                    SlotId(85) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(11),
                            SlotId(85),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(85), nonterminal_node_id, None);
                    }
                    // PostCondition : . "!" Layout Identifier
                    SlotId(86) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(34),
                            input_index,
                            SlotId(86),
                            Some(gss_node_id),
                        ) {
                            // PostCondition : "!" . Layout Identifier
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(87));
                        }
                    }
                    // PostCondition : "!" . Layout Identifier
                    SlotId(87) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(87), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(88), env)
                            {
                                // PostCondition : "!" Layout . Identifier
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(88));
                            }
                        }
                    }
                    // PostCondition : "!" Layout . Identifier
                    SlotId(88) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(88),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(89), env)
                            {
                                // PostCondition : "!" Layout Identifier.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(89));
                            }
                        }
                    }
                    // PostCondition : "!" Layout Identifier.
                    SlotId(89) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(11),
                            SlotId(89),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(89), nonterminal_node_id, None);
                    }
                    // Regex : . Regex Layout "+"
                    SlotId(90) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(91), env);
                    }
                    // Regex : Regex . Layout "+"
                    SlotId(91) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(91), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(92), env)
                            {
                                // Regex : Regex Layout . "+"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(92));
                            }
                        }
                    }
                    // Regex : Regex Layout . "+"
                    SlotId(92) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(30),
                            input_index,
                            SlotId(92),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(93), env)
                            {
                                // Regex : Regex Layout "+".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(93));
                            }
                        }
                    }
                    // Regex : Regex Layout "+".
                    SlotId(93) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(93),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(93), nonterminal_node_id, None);
                    }
                    // Regex : . Regex Layout "*"
                    SlotId(94) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(95), env);
                    }
                    // Regex : Regex . Layout "*"
                    SlotId(95) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(95), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(96), env)
                            {
                                // Regex : Regex Layout . "*"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(96));
                            }
                        }
                    }
                    // Regex : Regex Layout . "*"
                    SlotId(96) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(29),
                            input_index,
                            SlotId(96),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(97), env)
                            {
                                // Regex : Regex Layout "*".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(97));
                            }
                        }
                    }
                    // Regex : Regex Layout "*".
                    SlotId(97) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(97),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(97), nonterminal_node_id, None);
                    }
                    // Regex : . Regex Layout "?"
                    SlotId(98) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(99), env);
                    }
                    // Regex : Regex . Layout "?"
                    SlotId(99) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(99), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(100), env)
                            {
                                // Regex : Regex Layout . "?"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(100));
                            }
                        }
                    }
                    // Regex : Regex Layout . "?"
                    SlotId(100) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(31),
                            input_index,
                            SlotId(100),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(101), env)
                            {
                                // Regex : Regex Layout "?".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(101));
                            }
                        }
                    }
                    // Regex : Regex Layout "?".
                    SlotId(101) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(101),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(101), nonterminal_node_id, None);
                    }
                    // Regex : . "(" Layout seqs:Plus_2 Layout ")"
                    SlotId(102) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(25),
                            input_index,
                            SlotId(102),
                            Some(gss_node_id),
                        ) {
                            // Regex : "(" . Layout seqs:Plus_2 Layout ")"
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(103));
                        }
                    }
                    // Regex : "(" . Layout seqs:Plus_2 Layout ")"
                    SlotId(103) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(103), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(104), env)
                            {
                                // Regex : "(" Layout . seqs:Plus_2 Layout ")"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(104));
                            }
                        }
                    }
                    // Regex : "(" Layout . seqs:Plus_2 Layout ")"
                    SlotId(104) => {
                        self.create(NonterminalId(28), result, gss_node_id, SlotId(105), env);
                    }
                    // Regex : "(" Layout seqs:Plus_2 . Layout ")"
                    SlotId(105) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(105), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(106), env)
                            {
                                // Regex : "(" Layout seqs:Plus_2 Layout . ")"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(106));
                            }
                        }
                    }
                    // Regex : "(" Layout seqs:Plus_2 Layout . ")"
                    SlotId(106) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(26),
                            input_index,
                            SlotId(106),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(107), env)
                            {
                                // Regex : "(" Layout seqs:Plus_2 Layout ")".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(107));
                            }
                        }
                    }
                    // Regex : "(" Layout seqs:Plus_2 Layout ")".
                    SlotId(107) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(107),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(107), nonterminal_node_id, None);
                    }
                    // Regex : . CharClass
                    SlotId(108) => {
                        self.create(NonterminalId(13), result, gss_node_id, SlotId(109), env);
                    }
                    // Regex : CharClass.
                    SlotId(109) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(109),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(109), nonterminal_node_id, None);
                    }
                    // Regex : . Char
                    SlotId(110) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(4),
                            input_index,
                            SlotId(110),
                            Some(gss_node_id),
                        ) {
                            // Regex : Char.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(111));
                        }
                    }
                    // Regex : Char.
                    SlotId(111) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(111),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(111), nonterminal_node_id, None);
                    }
                    // Regex : . String
                    SlotId(112) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(3),
                            input_index,
                            SlotId(112),
                            Some(gss_node_id),
                        ) {
                            // Regex : String.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(113));
                        }
                    }
                    // Regex : String.
                    SlotId(113) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(113),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(113), nonterminal_node_id, None);
                    }
                    // Regex : . Identifier
                    SlotId(114) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(2),
                            input_index,
                            SlotId(114),
                            Some(gss_node_id),
                        ) {
                            // Regex : Identifier.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(115));
                        }
                    }
                    // Regex : Identifier.
                    SlotId(115) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(12),
                            SlotId(115),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(115), nonterminal_node_id, None);
                    }
                    // CharClass : . neg:Opt_10 Layout "[" Layout Plus_10 Layout "]"
                    SlotId(116) => {
                        if let Some(right_child) = self
                            .parse_opt_10_ll1(input_index, Some((SlotId(116), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // CharClass : neg:Opt_10 . Layout "[" Layout Plus_10 Layout "]"
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(117));
                        }
                    }
                    // CharClass : neg:Opt_10 . Layout "[" Layout Plus_10 Layout "]"
                    SlotId(117) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(117), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(118), env)
                            {
                                // CharClass : neg:Opt_10 Layout . "[" Layout Plus_10 Layout "]"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(118));
                            }
                        }
                    }
                    // CharClass : neg:Opt_10 Layout . "[" Layout Plus_10 Layout "]"
                    SlotId(118) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(35),
                            input_index,
                            SlotId(118),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(119), env)
                            {
                                // CharClass : neg:Opt_10 Layout "[" . Layout Plus_10 Layout "]"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(119));
                            }
                        }
                    }
                    // CharClass : neg:Opt_10 Layout "[" . Layout Plus_10 Layout "]"
                    SlotId(119) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(119), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(120), env)
                            {
                                // CharClass : neg:Opt_10 Layout "[" Layout . Plus_10 Layout "]"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(120));
                            }
                        }
                    }
                    // CharClass : neg:Opt_10 Layout "[" Layout . Plus_10 Layout "]"
                    SlotId(120) => {
                        self.create(NonterminalId(42), result, gss_node_id, SlotId(121), env);
                    }
                    // CharClass : neg:Opt_10 Layout "[" Layout Plus_10 . Layout "]"
                    SlotId(121) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(121), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(122), env)
                            {
                                // CharClass : neg:Opt_10 Layout "[" Layout Plus_10 Layout . "]"
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(122));
                            }
                        }
                    }
                    // CharClass : neg:Opt_10 Layout "[" Layout Plus_10 Layout . "]"
                    SlotId(122) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(36),
                            input_index,
                            SlotId(122),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(123), env)
                            {
                                // CharClass : neg:Opt_10 Layout "[" Layout Plus_10 Layout "]".
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(123));
                            }
                        }
                    }
                    // CharClass : neg:Opt_10 Layout "[" Layout Plus_10 Layout "]".
                    SlotId(123) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(13),
                            SlotId(123),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(123), nonterminal_node_id, None);
                    }
                    // RangeElement : . Range
                    SlotId(124) => {
                        if let Some(right_child) = self
                            .parse_range_ll1(input_index, Some((SlotId(124), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // RangeElement : Range.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(125));
                        }
                    }
                    // RangeElement : Range.
                    SlotId(125) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(14),
                            SlotId(125),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(125), nonterminal_node_id, None);
                    }
                    // RangeElement : . RangeChar
                    SlotId(126) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(6),
                            input_index,
                            SlotId(126),
                            Some(gss_node_id),
                        ) {
                            // RangeElement : RangeChar.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(127));
                        }
                    }
                    // RangeElement : RangeChar.
                    SlotId(127) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(14),
                            SlotId(127),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(127), nonterminal_node_id, None);
                    }
                    // Range : . start:RangeChar Layout "-" Layout end:RangeChar
                    SlotId(128) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(6),
                            input_index,
                            SlotId(128),
                            Some(gss_node_id),
                        ) {
                            // Range : start:RangeChar . Layout "-" Layout end:RangeChar
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(129));
                        }
                    }
                    // Range : start:RangeChar . Layout "-" Layout end:RangeChar
                    SlotId(129) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(129), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(130), env)
                            {
                                // Range : start:RangeChar Layout . "-" Layout end:RangeChar
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(130));
                            }
                        }
                    }
                    // Range : start:RangeChar Layout . "-" Layout end:RangeChar
                    SlotId(130) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(37),
                            input_index,
                            SlotId(130),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(131), env)
                            {
                                // Range : start:RangeChar Layout "-" . Layout end:RangeChar
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(131));
                            }
                        }
                    }
                    // Range : start:RangeChar Layout "-" . Layout end:RangeChar
                    SlotId(131) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(131), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(132), env)
                            {
                                // Range : start:RangeChar Layout "-" Layout . end:RangeChar
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(132));
                            }
                        }
                    }
                    // Range : start:RangeChar Layout "-" Layout . end:RangeChar
                    SlotId(132) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(6),
                            input_index,
                            SlotId(132),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(133), env)
                            {
                                // Range : start:RangeChar Layout "-" Layout end:RangeChar.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(133));
                            }
                        }
                    }
                    // Range : start:RangeChar Layout "-" Layout end:RangeChar.
                    SlotId(133) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(15),
                            SlotId(133),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(133), nonterminal_node_id, None);
                    }
                    // Layout : . Star_4 !>> WS !>> LineComment
                    SlotId(134) => {
                        if let Some(right_child) = self
                            .parse_star_4_ll1(input_index, Some((SlotId(134), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(failure) = self.post_conditions(SlotId(135), input_index, j)
                            {
                                self.add_failure(j, SlotId(135), Some(gss_node_id), failure);
                            } else {
                                // Layout : Star_4 !>> WS !>> LineComment.
                                input_index = j;
                                result = Some(right_child);
                                next = Some(SlotId(135));
                            }
                        }
                    }
                    // Layout : Star_4 !>> WS !>> LineComment.
                    SlotId(135) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(16),
                            SlotId(135),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(135), nonterminal_node_id, None);
                    }
                    // Plus_0 : . Plus_0 Layout Rule
                    SlotId(136) => {
                        self.create(NonterminalId(17), result, gss_node_id, SlotId(137), env);
                    }
                    // Plus_0 : Plus_0 . Layout Rule
                    SlotId(137) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(137), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(138), env)
                            {
                                // Plus_0 : Plus_0 Layout . Rule
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(138));
                            }
                        }
                    }
                    // Plus_0 : Plus_0 Layout . Rule
                    SlotId(138) => {
                        self.create(NonterminalId(1), result, gss_node_id, SlotId(139), env);
                    }
                    // Plus_0 : Plus_0 Layout Rule.
                    SlotId(139) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(17),
                            SlotId(139),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(139), nonterminal_node_id, None);
                    }
                    // Plus_0 : . Rule
                    SlotId(140) => {
                        self.create(NonterminalId(1), result, gss_node_id, SlotId(141), env);
                    }
                    // Plus_0 : Rule.
                    SlotId(141) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(17),
                            SlotId(141),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(141), nonterminal_node_id, None);
                    }
                    // Opt_0 : . Plus_0
                    SlotId(142) => {
                        self.create(NonterminalId(17), result, gss_node_id, SlotId(143), env);
                    }
                    // Opt_0 : Plus_0.
                    SlotId(143) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(18),
                            SlotId(143),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(143), nonterminal_node_id, None);
                    }
                    // Opt_0 : .
                    SlotId(144) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(18),
                            SlotId(144),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(144), nonterminal_node_id, None);
                    }
                    // Star_0 : . Opt_0
                    SlotId(145) => {
                        self.create(NonterminalId(18), result, gss_node_id, SlotId(146), env);
                    }
                    // Star_0 : Opt_0.
                    SlotId(146) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(19),
                            SlotId(146),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(146), nonterminal_node_id, None);
                    }
                    // Opt_1 : . Annotation
                    SlotId(147) => {
                        if let Some(right_child) = self.parse_annotation_ll1(
                            input_index,
                            Some((SlotId(147), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_1 : Annotation.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(148));
                        }
                    }
                    // Opt_1 : Annotation.
                    SlotId(148) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(20),
                            SlotId(148),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(148), nonterminal_node_id, None);
                    }
                    // Opt_1 : .
                    SlotId(149) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(20),
                            SlotId(149),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(149), nonterminal_node_id, None);
                    }
                    // Plus_1 : . Plus_1 Layout ">" Layout PriorityLevel
                    SlotId(150) => {
                        self.create(NonterminalId(21), result, gss_node_id, SlotId(151), env);
                    }
                    // Plus_1 : Plus_1 . Layout ">" Layout PriorityLevel
                    SlotId(151) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(151), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(152), env)
                            {
                                // Plus_1 : Plus_1 Layout . ">" Layout PriorityLevel
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(152));
                            }
                        }
                    }
                    // Plus_1 : Plus_1 Layout . ">" Layout PriorityLevel
                    SlotId(152) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(13),
                            input_index,
                            SlotId(152),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(153), env)
                            {
                                // Plus_1 : Plus_1 Layout ">" . Layout PriorityLevel
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(153));
                            }
                        }
                    }
                    // Plus_1 : Plus_1 Layout ">" . Layout PriorityLevel
                    SlotId(153) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(153), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(154), env)
                            {
                                // Plus_1 : Plus_1 Layout ">" Layout . PriorityLevel
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(154));
                            }
                        }
                    }
                    // Plus_1 : Plus_1 Layout ">" Layout . PriorityLevel
                    SlotId(154) => {
                        self.create(NonterminalId(7), result, gss_node_id, SlotId(155), env);
                    }
                    // Plus_1 : Plus_1 Layout ">" Layout PriorityLevel.
                    SlotId(155) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(21),
                            SlotId(155),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(155), nonterminal_node_id, None);
                    }
                    // Plus_1 : . PriorityLevel
                    SlotId(156) => {
                        self.create(NonterminalId(7), result, gss_node_id, SlotId(157), env);
                    }
                    // Plus_1 : PriorityLevel.
                    SlotId(157) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(21),
                            SlotId(157),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(157), nonterminal_node_id, None);
                    }
                    // Opt_2 : . Plus_1
                    SlotId(158) => {
                        self.create(NonterminalId(21), result, gss_node_id, SlotId(159), env);
                    }
                    // Opt_2 : Plus_1.
                    SlotId(159) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(22),
                            SlotId(159),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(159), nonterminal_node_id, None);
                    }
                    // Opt_2 : .
                    SlotId(160) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(22),
                            SlotId(160),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(160), nonterminal_node_id, None);
                    }
                    // Star_1 : . Opt_2
                    SlotId(161) => {
                        self.create(NonterminalId(22), result, gss_node_id, SlotId(162), env);
                    }
                    // Star_1 : Opt_2.
                    SlotId(162) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(23),
                            SlotId(162),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(162), nonterminal_node_id, None);
                    }
                    // Opt_3 : . "@Layout"
                    SlotId(163) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(15),
                            input_index,
                            SlotId(163),
                            Some(gss_node_id),
                        ) {
                            // Opt_3 : "@Layout".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(164));
                        }
                    }
                    // Opt_3 : "@Layout".
                    SlotId(164) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(24),
                            SlotId(164),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(164), nonterminal_node_id, None);
                    }
                    // Opt_3 : .
                    SlotId(165) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(24),
                            SlotId(165),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(165), nonterminal_node_id, None);
                    }
                    // Opt_4 : . "@Identifier"
                    SlotId(166) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(16),
                            input_index,
                            SlotId(166),
                            Some(gss_node_id),
                        ) {
                            // Opt_4 : "@Identifier".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(167));
                        }
                    }
                    // Opt_4 : "@Identifier".
                    SlotId(167) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(25),
                            SlotId(167),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(167), nonterminal_node_id, None);
                    }
                    // Opt_4 : .
                    SlotId(168) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(25),
                            SlotId(168),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(168), nonterminal_node_id, None);
                    }
                    // Opt_5 : . RegexPreCondition
                    SlotId(169) => {
                        if let Some(right_child) = self.parse_regex_pre_condition_ll1(
                            input_index,
                            Some((SlotId(169), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_5 : RegexPreCondition.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(170));
                        }
                    }
                    // Opt_5 : RegexPreCondition.
                    SlotId(170) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(26),
                            SlotId(170),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(170), nonterminal_node_id, None);
                    }
                    // Opt_5 : .
                    SlotId(171) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(26),
                            SlotId(171),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(171), nonterminal_node_id, None);
                    }
                    // Plus_3 : . Plus_3 Layout Regex
                    SlotId(172) => {
                        self.create(NonterminalId(27), result, gss_node_id, SlotId(173), env);
                    }
                    // Plus_3 : Plus_3 . Layout Regex
                    SlotId(173) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(173), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(174), env)
                            {
                                // Plus_3 : Plus_3 Layout . Regex
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(174));
                            }
                        }
                    }
                    // Plus_3 : Plus_3 Layout . Regex
                    SlotId(174) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(175), env);
                    }
                    // Plus_3 : Plus_3 Layout Regex.
                    SlotId(175) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(27),
                            SlotId(175),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(175), nonterminal_node_id, None);
                    }
                    // Plus_3 : . Regex
                    SlotId(176) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(177), env);
                    }
                    // Plus_3 : Regex.
                    SlotId(177) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(27),
                            SlotId(177),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(177), nonterminal_node_id, None);
                    }
                    // Plus_2 : . Plus_2 Layout "|" Layout Plus_3
                    SlotId(178) => {
                        self.create(NonterminalId(28), result, gss_node_id, SlotId(179), env);
                    }
                    // Plus_2 : Plus_2 . Layout "|" Layout Plus_3
                    SlotId(179) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(179), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(180), env)
                            {
                                // Plus_2 : Plus_2 Layout . "|" Layout Plus_3
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(180));
                            }
                        }
                    }
                    // Plus_2 : Plus_2 Layout . "|" Layout Plus_3
                    SlotId(180) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(18),
                            input_index,
                            SlotId(180),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(181), env)
                            {
                                // Plus_2 : Plus_2 Layout "|" . Layout Plus_3
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(181));
                            }
                        }
                    }
                    // Plus_2 : Plus_2 Layout "|" . Layout Plus_3
                    SlotId(181) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(181), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(182), env)
                            {
                                // Plus_2 : Plus_2 Layout "|" Layout . Plus_3
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(182));
                            }
                        }
                    }
                    // Plus_2 : Plus_2 Layout "|" Layout . Plus_3
                    SlotId(182) => {
                        self.create(NonterminalId(27), result, gss_node_id, SlotId(183), env);
                    }
                    // Plus_2 : Plus_2 Layout "|" Layout Plus_3.
                    SlotId(183) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(28),
                            SlotId(183),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(183), nonterminal_node_id, None);
                    }
                    // Plus_2 : . Plus_3
                    SlotId(184) => {
                        self.create(NonterminalId(27), result, gss_node_id, SlotId(185), env);
                    }
                    // Plus_2 : Plus_3.
                    SlotId(185) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(28),
                            SlotId(185),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(185), nonterminal_node_id, None);
                    }
                    // Plus_4 : . Plus_4 Layout RegexPostCondition
                    SlotId(186) => {
                        if let Some(right_child) = self
                            .parse_plus_4_ll1(input_index, Some((SlotId(186), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_4 : Plus_4 . Layout RegexPostCondition
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(187));
                        }
                    }
                    // Plus_4 : Plus_4 . Layout RegexPostCondition
                    SlotId(187) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(187), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(188), env)
                            {
                                // Plus_4 : Plus_4 Layout . RegexPostCondition
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(188));
                            }
                        }
                    }
                    // Plus_4 : Plus_4 Layout . RegexPostCondition
                    SlotId(188) => {
                        if let Some(right_child) = self.parse_regex_post_condition_ll1(
                            input_index,
                            Some((SlotId(188), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(189), env)
                            {
                                // Plus_4 : Plus_4 Layout RegexPostCondition.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(189));
                            }
                        }
                    }
                    // Plus_4 : Plus_4 Layout RegexPostCondition.
                    SlotId(189) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(29),
                            SlotId(189),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(189), nonterminal_node_id, None);
                    }
                    // Plus_4 : . RegexPostCondition
                    SlotId(190) => {
                        if let Some(right_child) = self.parse_regex_post_condition_ll1(
                            input_index,
                            Some((SlotId(190), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_4 : RegexPostCondition.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(191));
                        }
                    }
                    // Plus_4 : RegexPostCondition.
                    SlotId(191) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(29),
                            SlotId(191),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(191), nonterminal_node_id, None);
                    }
                    // Opt_6 : . Plus_4
                    SlotId(192) => {
                        if let Some(right_child) = self
                            .parse_plus_4_ll1(input_index, Some((SlotId(192), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_6 : Plus_4.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(193));
                        }
                    }
                    // Opt_6 : Plus_4.
                    SlotId(193) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(30),
                            SlotId(193),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(193), nonterminal_node_id, None);
                    }
                    // Opt_6 : .
                    SlotId(194) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(30),
                            SlotId(194),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(194), nonterminal_node_id, None);
                    }
                    // Star_2 : . Opt_6
                    SlotId(195) => {
                        if let Some(right_child) = self
                            .parse_opt_6_ll1(input_index, Some((SlotId(195), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Star_2 : Opt_6.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(196));
                        }
                    }
                    // Star_2 : Opt_6.
                    SlotId(196) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(31),
                            SlotId(196),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(196), nonterminal_node_id, None);
                    }
                    // Opt_7 : . Associativity
                    SlotId(197) => {
                        if let Some(right_child) = self.parse_associativity_ll1(
                            input_index,
                            Some((SlotId(197), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_7 : Associativity.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(198));
                        }
                    }
                    // Opt_7 : Associativity.
                    SlotId(198) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(32),
                            SlotId(198),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(198), nonterminal_node_id, None);
                    }
                    // Opt_7 : .
                    SlotId(199) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(32),
                            SlotId(199),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(199), nonterminal_node_id, None);
                    }
                    // Plus_5 : . Plus_5 Layout "|" Layout Alternative
                    SlotId(200) => {
                        self.create(NonterminalId(33), result, gss_node_id, SlotId(201), env);
                    }
                    // Plus_5 : Plus_5 . Layout "|" Layout Alternative
                    SlotId(201) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(201), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(202), env)
                            {
                                // Plus_5 : Plus_5 Layout . "|" Layout Alternative
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(202));
                            }
                        }
                    }
                    // Plus_5 : Plus_5 Layout . "|" Layout Alternative
                    SlotId(202) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(18),
                            input_index,
                            SlotId(202),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(203), env)
                            {
                                // Plus_5 : Plus_5 Layout "|" . Layout Alternative
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(203));
                            }
                        }
                    }
                    // Plus_5 : Plus_5 Layout "|" . Layout Alternative
                    SlotId(203) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(203), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(204), env)
                            {
                                // Plus_5 : Plus_5 Layout "|" Layout . Alternative
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(204));
                            }
                        }
                    }
                    // Plus_5 : Plus_5 Layout "|" Layout . Alternative
                    SlotId(204) => {
                        self.create(NonterminalId(9), result, gss_node_id, SlotId(205), env);
                    }
                    // Plus_5 : Plus_5 Layout "|" Layout Alternative.
                    SlotId(205) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(33),
                            SlotId(205),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(205), nonterminal_node_id, None);
                    }
                    // Plus_5 : . Alternative
                    SlotId(206) => {
                        self.create(NonterminalId(9), result, gss_node_id, SlotId(207), env);
                    }
                    // Plus_5 : Alternative.
                    SlotId(207) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(33),
                            SlotId(207),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(207), nonterminal_node_id, None);
                    }
                    // Opt_8 : . Plus_5
                    SlotId(208) => {
                        self.create(NonterminalId(33), result, gss_node_id, SlotId(209), env);
                    }
                    // Opt_8 : Plus_5.
                    SlotId(209) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(34),
                            SlotId(209),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(209), nonterminal_node_id, None);
                    }
                    // Opt_8 : .
                    SlotId(210) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(34),
                            SlotId(210),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(210), nonterminal_node_id, None);
                    }
                    // Star_3 : . Opt_8
                    SlotId(211) => {
                        self.create(NonterminalId(34), result, gss_node_id, SlotId(212), env);
                    }
                    // Star_3 : Opt_8.
                    SlotId(212) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(35),
                            SlotId(212),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(212), nonterminal_node_id, None);
                    }
                    // Plus_6 : . Plus_6 Layout Symbol(0, 0)
                    SlotId(213) => {
                        self.create(NonterminalId(36), result, gss_node_id, SlotId(214), env);
                    }
                    // Plus_6 : Plus_6 . Layout Symbol(0, 0)
                    SlotId(214) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(214), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(215), env)
                            {
                                // Plus_6 : Plus_6 Layout . Symbol(0, 0)
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(215));
                            }
                        }
                    }
                    // Plus_6 : Plus_6 Layout . Symbol(0, 0)
                    SlotId(215) => {
                        self.create_symbol(result, gss_node_id, SlotId(216), env, 0, 0);
                    }
                    // Plus_6 : Plus_6 Layout Symbol(0, 0).
                    SlotId(216) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(36),
                            SlotId(216),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(216), nonterminal_node_id, None);
                    }
                    // Plus_6 : . Symbol(0, 0)
                    SlotId(217) => {
                        self.create_symbol(result, gss_node_id, SlotId(218), env, 0, 0);
                    }
                    // Plus_6 : Symbol(0, 0).
                    SlotId(218) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(36),
                            SlotId(218),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(218), nonterminal_node_id, None);
                    }
                    // Opt_9 : . Label
                    SlotId(219) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(7),
                            input_index,
                            SlotId(219),
                            Some(gss_node_id),
                        ) {
                            // Opt_9 : Label.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(220));
                        }
                    }
                    // Opt_9 : Label.
                    SlotId(220) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(37),
                            SlotId(220),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(220), nonterminal_node_id, None);
                    }
                    // Opt_9 : .
                    SlotId(221) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(37),
                            SlotId(221),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(221), nonterminal_node_id, None);
                    }
                    // Plus_7 : . Plus_7 Layout "|" Layout Plus_6
                    SlotId(222) => {
                        self.create(NonterminalId(38), result, gss_node_id, SlotId(223), env);
                    }
                    // Plus_7 : Plus_7 . Layout "|" Layout Plus_6
                    SlotId(223) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(223), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(224), env)
                            {
                                // Plus_7 : Plus_7 Layout . "|" Layout Plus_6
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(224));
                            }
                        }
                    }
                    // Plus_7 : Plus_7 Layout . "|" Layout Plus_6
                    SlotId(224) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(18),
                            input_index,
                            SlotId(224),
                            Some(gss_node_id),
                        ) {
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(225), env)
                            {
                                // Plus_7 : Plus_7 Layout "|" . Layout Plus_6
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(225));
                            }
                        }
                    }
                    // Plus_7 : Plus_7 Layout "|" . Layout Plus_6
                    SlotId(225) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(225), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(226), env)
                            {
                                // Plus_7 : Plus_7 Layout "|" Layout . Plus_6
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(226));
                            }
                        }
                    }
                    // Plus_7 : Plus_7 Layout "|" Layout . Plus_6
                    SlotId(226) => {
                        self.create(NonterminalId(36), result, gss_node_id, SlotId(227), env);
                    }
                    // Plus_7 : Plus_7 Layout "|" Layout Plus_6.
                    SlotId(227) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(38),
                            SlotId(227),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(227), nonterminal_node_id, None);
                    }
                    // Plus_7 : . Plus_6
                    SlotId(228) => {
                        self.create(NonterminalId(36), result, gss_node_id, SlotId(229), env);
                    }
                    // Plus_7 : Plus_6.
                    SlotId(229) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(38),
                            SlotId(229),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(229), nonterminal_node_id, None);
                    }
                    // Plus_8 : . Plus_8 Layout PostCondition
                    SlotId(230) => {
                        self.create(NonterminalId(39), result, gss_node_id, SlotId(231), env);
                    }
                    // Plus_8 : Plus_8 . Layout PostCondition
                    SlotId(231) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(231), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(232), env)
                            {
                                // Plus_8 : Plus_8 Layout . PostCondition
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(232));
                            }
                        }
                    }
                    // Plus_8 : Plus_8 Layout . PostCondition
                    SlotId(232) => {
                        self.create(NonterminalId(11), result, gss_node_id, SlotId(233), env);
                    }
                    // Plus_8 : Plus_8 Layout PostCondition.
                    SlotId(233) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(39),
                            SlotId(233),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(233), nonterminal_node_id, None);
                    }
                    // Plus_8 : . PostCondition
                    SlotId(234) => {
                        self.create(NonterminalId(11), result, gss_node_id, SlotId(235), env);
                    }
                    // Plus_8 : PostCondition.
                    SlotId(235) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(39),
                            SlotId(235),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(235), nonterminal_node_id, None);
                    }
                    // Plus_9 : . Plus_9 Layout PreCondition
                    SlotId(236) => {
                        self.create(NonterminalId(40), result, gss_node_id, SlotId(237), env);
                    }
                    // Plus_9 : Plus_9 . Layout PreCondition
                    SlotId(237) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(237), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(238), env)
                            {
                                // Plus_9 : Plus_9 Layout . PreCondition
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(238));
                            }
                        }
                    }
                    // Plus_9 : Plus_9 Layout . PreCondition
                    SlotId(238) => {
                        if let Some(right_child) = self.parse_pre_condition_ll1(
                            input_index,
                            Some((SlotId(238), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(239), env)
                            {
                                // Plus_9 : Plus_9 Layout PreCondition.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(239));
                            }
                        }
                    }
                    // Plus_9 : Plus_9 Layout PreCondition.
                    SlotId(239) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(40),
                            SlotId(239),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(239), nonterminal_node_id, None);
                    }
                    // Plus_9 : . PreCondition
                    SlotId(240) => {
                        if let Some(right_child) = self.parse_pre_condition_ll1(
                            input_index,
                            Some((SlotId(240), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_9 : PreCondition.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(241));
                        }
                    }
                    // Plus_9 : PreCondition.
                    SlotId(241) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(40),
                            SlotId(241),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(241), nonterminal_node_id, None);
                    }
                    // Opt_10 : . "!"
                    SlotId(242) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(34),
                            input_index,
                            SlotId(242),
                            Some(gss_node_id),
                        ) {
                            // Opt_10 : "!".
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(243));
                        }
                    }
                    // Opt_10 : "!".
                    SlotId(243) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(41),
                            SlotId(243),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(243), nonterminal_node_id, None);
                    }
                    // Opt_10 : .
                    SlotId(244) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(41),
                            SlotId(244),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(244), nonterminal_node_id, None);
                    }
                    // Plus_10 : . Plus_10 Layout RangeElement
                    SlotId(245) => {
                        self.create(NonterminalId(42), result, gss_node_id, SlotId(246), env);
                    }
                    // Plus_10 : Plus_10 . Layout RangeElement
                    SlotId(246) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(246), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(247), env)
                            {
                                // Plus_10 : Plus_10 Layout . RangeElement
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(247));
                            }
                        }
                    }
                    // Plus_10 : Plus_10 Layout . RangeElement
                    SlotId(247) => {
                        self.create(NonterminalId(14), result, gss_node_id, SlotId(248), env);
                    }
                    // Plus_10 : Plus_10 Layout RangeElement.
                    SlotId(248) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(42),
                            SlotId(248),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(248), nonterminal_node_id, None);
                    }
                    // Plus_10 : . RangeElement
                    SlotId(249) => {
                        self.create(NonterminalId(14), result, gss_node_id, SlotId(250), env);
                    }
                    // Plus_10 : RangeElement.
                    SlotId(250) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(42),
                            SlotId(250),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(250), nonterminal_node_id, None);
                    }
                    // Alt_0 : . WS
                    SlotId(251) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(8),
                            input_index,
                            SlotId(251),
                            Some(gss_node_id),
                        ) {
                            // Alt_0 : WS.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(252));
                        }
                    }
                    // Alt_0 : WS.
                    SlotId(252) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(43),
                            SlotId(252),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(252), nonterminal_node_id, None);
                    }
                    // Alt_0 : . LineComment
                    SlotId(253) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            TerminalId(10),
                            input_index,
                            SlotId(253),
                            Some(gss_node_id),
                        ) {
                            // Alt_0 : LineComment.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(254));
                        }
                    }
                    // Alt_0 : LineComment.
                    SlotId(254) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(43),
                            SlotId(254),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(254), nonterminal_node_id, None);
                    }
                    // Plus_11 : . Plus_11 Alt_0
                    SlotId(255) => {
                        if let Some(right_child) = self
                            .parse_plus_11_ll1(input_index, Some((SlotId(255), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_11 : Plus_11 . Alt_0
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(256));
                        }
                    }
                    // Plus_11 : Plus_11 . Alt_0
                    SlotId(256) => {
                        if let Some(right_child) = self
                            .parse_alt_0_ll1(input_index, Some((SlotId(256), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(257), env)
                            {
                                // Plus_11 : Plus_11 Alt_0.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(257));
                            }
                        }
                    }
                    // Plus_11 : Plus_11 Alt_0.
                    SlotId(257) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(44),
                            SlotId(257),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(257), nonterminal_node_id, None);
                    }
                    // Plus_11 : . Alt_0
                    SlotId(258) => {
                        if let Some(right_child) = self
                            .parse_alt_0_ll1(input_index, Some((SlotId(258), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_11 : Alt_0.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(259));
                        }
                    }
                    // Plus_11 : Alt_0.
                    SlotId(259) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(44),
                            SlotId(259),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(259), nonterminal_node_id, None);
                    }
                    // Opt_11 : . Plus_11
                    SlotId(260) => {
                        if let Some(right_child) = self
                            .parse_plus_11_ll1(input_index, Some((SlotId(260), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_11 : Plus_11.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(261));
                        }
                    }
                    // Opt_11 : Plus_11.
                    SlotId(261) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(45),
                            SlotId(261),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(261), nonterminal_node_id, None);
                    }
                    // Opt_11 : .
                    SlotId(262) => {
                        let epsilon_node_id = self.get_or_create_epsilon_node(input_index);
                        let nonterminal_node_id = self.get_or_create_nonterminal_node(
                            NonterminalId(45),
                            SlotId(262),
                            input_index,
                            input_index,
                            epsilon_node_id,
                            gss_node_id,
                            None,
                        );
                        self.pop(gss_node_id, SlotId(262), nonterminal_node_id, None);
                    }
                    // Star_4 : . Opt_11
                    SlotId(263) => {
                        if let Some(right_child) = self
                            .parse_opt_11_ll1(input_index, Some((SlotId(263), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Star_4 : Opt_11.
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(264));
                        }
                    }
                    // Star_4 : Opt_11.
                    SlotId(264) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(46),
                            SlotId(264),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(264), nonterminal_node_id, None);
                    }
                    // StartGrammar : . Layout start:Grammar Layout
                    SlotId(265) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(265), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartGrammar : Layout . start:Grammar Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(266));
                        }
                    }
                    // StartGrammar : Layout . start:Grammar Layout
                    SlotId(266) => {
                        self.create(NonterminalId(0), result, gss_node_id, SlotId(267), env);
                    }
                    // StartGrammar : Layout start:Grammar . Layout
                    SlotId(267) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(267), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(268), env)
                            {
                                // StartGrammar : Layout start:Grammar Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(268));
                            }
                        }
                    }
                    // StartGrammar : Layout start:Grammar Layout.
                    SlotId(268) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(47),
                            SlotId(268),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(268), nonterminal_node_id, None);
                    }
                    // StartRule : . Layout start:Rule Layout
                    SlotId(269) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(269), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRule : Layout . start:Rule Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(270));
                        }
                    }
                    // StartRule : Layout . start:Rule Layout
                    SlotId(270) => {
                        self.create(NonterminalId(1), result, gss_node_id, SlotId(271), env);
                    }
                    // StartRule : Layout start:Rule . Layout
                    SlotId(271) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(271), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(272), env)
                            {
                                // StartRule : Layout start:Rule Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(272));
                            }
                        }
                    }
                    // StartRule : Layout start:Rule Layout.
                    SlotId(272) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(48),
                            SlotId(272),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(272), nonterminal_node_id, None);
                    }
                    // StartSyntaxRule : . Layout start:SyntaxRule Layout
                    SlotId(273) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(273), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartSyntaxRule : Layout . start:SyntaxRule Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(274));
                        }
                    }
                    // StartSyntaxRule : Layout . start:SyntaxRule Layout
                    SlotId(274) => {
                        self.create(NonterminalId(2), result, gss_node_id, SlotId(275), env);
                    }
                    // StartSyntaxRule : Layout start:SyntaxRule . Layout
                    SlotId(275) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(275), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(276), env)
                            {
                                // StartSyntaxRule : Layout start:SyntaxRule Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(276));
                            }
                        }
                    }
                    // StartSyntaxRule : Layout start:SyntaxRule Layout.
                    SlotId(276) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(49),
                            SlotId(276),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(276), nonterminal_node_id, None);
                    }
                    // StartAnnotation : . Layout start:Annotation Layout
                    SlotId(277) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(277), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartAnnotation : Layout . start:Annotation Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(278));
                        }
                    }
                    // StartAnnotation : Layout . start:Annotation Layout
                    SlotId(278) => {
                        if let Some(right_child) = self.parse_annotation_ll1(
                            input_index,
                            Some((SlotId(278), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(279), env)
                            {
                                // StartAnnotation : Layout start:Annotation . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(279));
                            }
                        }
                    }
                    // StartAnnotation : Layout start:Annotation . Layout
                    SlotId(279) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(279), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(280), env)
                            {
                                // StartAnnotation : Layout start:Annotation Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(280));
                            }
                        }
                    }
                    // StartAnnotation : Layout start:Annotation Layout.
                    SlotId(280) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(50),
                            SlotId(280),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(280), nonterminal_node_id, None);
                    }
                    // StartRegexRule : . Layout start:RegexRule Layout
                    SlotId(281) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(281), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRegexRule : Layout . start:RegexRule Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(282));
                        }
                    }
                    // StartRegexRule : Layout . start:RegexRule Layout
                    SlotId(282) => {
                        self.create(NonterminalId(4), result, gss_node_id, SlotId(283), env);
                    }
                    // StartRegexRule : Layout start:RegexRule . Layout
                    SlotId(283) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(283), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(284), env)
                            {
                                // StartRegexRule : Layout start:RegexRule Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(284));
                            }
                        }
                    }
                    // StartRegexRule : Layout start:RegexRule Layout.
                    SlotId(284) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(51),
                            SlotId(284),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(284), nonterminal_node_id, None);
                    }
                    // StartRegexPreCondition : . Layout start:RegexPreCondition Layout
                    SlotId(285) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(285), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRegexPreCondition : Layout . start:RegexPreCondition Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(286));
                        }
                    }
                    // StartRegexPreCondition : Layout . start:RegexPreCondition Layout
                    SlotId(286) => {
                        if let Some(right_child) = self.parse_regex_pre_condition_ll1(
                            input_index,
                            Some((SlotId(286), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(287), env)
                            {
                                // StartRegexPreCondition : Layout start:RegexPreCondition . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(287));
                            }
                        }
                    }
                    // StartRegexPreCondition : Layout start:RegexPreCondition . Layout
                    SlotId(287) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(287), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(288), env)
                            {
                                // StartRegexPreCondition : Layout start:RegexPreCondition Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(288));
                            }
                        }
                    }
                    // StartRegexPreCondition : Layout start:RegexPreCondition Layout.
                    SlotId(288) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(52),
                            SlotId(288),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(288), nonterminal_node_id, None);
                    }
                    // StartRegexPostCondition : . Layout start:RegexPostCondition Layout
                    SlotId(289) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(289), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRegexPostCondition : Layout . start:RegexPostCondition Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(290));
                        }
                    }
                    // StartRegexPostCondition : Layout . start:RegexPostCondition Layout
                    SlotId(290) => {
                        if let Some(right_child) = self.parse_regex_post_condition_ll1(
                            input_index,
                            Some((SlotId(290), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(291), env)
                            {
                                // StartRegexPostCondition : Layout start:RegexPostCondition . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(291));
                            }
                        }
                    }
                    // StartRegexPostCondition : Layout start:RegexPostCondition . Layout
                    SlotId(291) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(291), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(292), env)
                            {
                                // StartRegexPostCondition : Layout start:RegexPostCondition Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(292));
                            }
                        }
                    }
                    // StartRegexPostCondition : Layout start:RegexPostCondition Layout.
                    SlotId(292) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(53),
                            SlotId(292),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(292), nonterminal_node_id, None);
                    }
                    // StartPriorityLevel : . Layout start:PriorityLevel Layout
                    SlotId(293) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(293), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartPriorityLevel : Layout . start:PriorityLevel Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(294));
                        }
                    }
                    // StartPriorityLevel : Layout . start:PriorityLevel Layout
                    SlotId(294) => {
                        self.create(NonterminalId(7), result, gss_node_id, SlotId(295), env);
                    }
                    // StartPriorityLevel : Layout start:PriorityLevel . Layout
                    SlotId(295) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(295), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(296), env)
                            {
                                // StartPriorityLevel : Layout start:PriorityLevel Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(296));
                            }
                        }
                    }
                    // StartPriorityLevel : Layout start:PriorityLevel Layout.
                    SlotId(296) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(54),
                            SlotId(296),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(296), nonterminal_node_id, None);
                    }
                    // StartAssociativity : . Layout start:Associativity Layout
                    SlotId(297) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(297), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartAssociativity : Layout . start:Associativity Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(298));
                        }
                    }
                    // StartAssociativity : Layout . start:Associativity Layout
                    SlotId(298) => {
                        if let Some(right_child) = self.parse_associativity_ll1(
                            input_index,
                            Some((SlotId(298), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(299), env)
                            {
                                // StartAssociativity : Layout start:Associativity . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(299));
                            }
                        }
                    }
                    // StartAssociativity : Layout start:Associativity . Layout
                    SlotId(299) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(299), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(300), env)
                            {
                                // StartAssociativity : Layout start:Associativity Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(300));
                            }
                        }
                    }
                    // StartAssociativity : Layout start:Associativity Layout.
                    SlotId(300) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(55),
                            SlotId(300),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(300), nonterminal_node_id, None);
                    }
                    // StartAlternative : . Layout start:Alternative Layout
                    SlotId(301) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(301), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartAlternative : Layout . start:Alternative Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(302));
                        }
                    }
                    // StartAlternative : Layout . start:Alternative Layout
                    SlotId(302) => {
                        self.create(NonterminalId(9), result, gss_node_id, SlotId(303), env);
                    }
                    // StartAlternative : Layout start:Alternative . Layout
                    SlotId(303) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(303), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(304), env)
                            {
                                // StartAlternative : Layout start:Alternative Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(304));
                            }
                        }
                    }
                    // StartAlternative : Layout start:Alternative Layout.
                    SlotId(304) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(56),
                            SlotId(304),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(304), nonterminal_node_id, None);
                    }
                    // StartSymbol : . Layout start:Symbol(0, 0) Layout
                    SlotId(305) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(305), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartSymbol : Layout . start:Symbol(0, 0) Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(306));
                        }
                    }
                    // StartSymbol : Layout . start:Symbol(0, 0) Layout
                    SlotId(306) => {
                        self.create_symbol(result, gss_node_id, SlotId(307), env, 0, 0);
                    }
                    // StartSymbol : Layout start:Symbol(0, 0) . Layout
                    SlotId(307) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(307), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(308), env)
                            {
                                // StartSymbol : Layout start:Symbol(0, 0) Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(308));
                            }
                        }
                    }
                    // StartSymbol : Layout start:Symbol(0, 0) Layout.
                    SlotId(308) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(57),
                            SlotId(308),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(308), nonterminal_node_id, None);
                    }
                    // StartPreCondition : . Layout start:PreCondition Layout
                    SlotId(309) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(309), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartPreCondition : Layout . start:PreCondition Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(310));
                        }
                    }
                    // StartPreCondition : Layout . start:PreCondition Layout
                    SlotId(310) => {
                        if let Some(right_child) = self.parse_pre_condition_ll1(
                            input_index,
                            Some((SlotId(310), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(311), env)
                            {
                                // StartPreCondition : Layout start:PreCondition . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(311));
                            }
                        }
                    }
                    // StartPreCondition : Layout start:PreCondition . Layout
                    SlotId(311) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(311), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(312), env)
                            {
                                // StartPreCondition : Layout start:PreCondition Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(312));
                            }
                        }
                    }
                    // StartPreCondition : Layout start:PreCondition Layout.
                    SlotId(312) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(58),
                            SlotId(312),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(312), nonterminal_node_id, None);
                    }
                    // StartPostCondition : . Layout start:PostCondition Layout
                    SlotId(313) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(313), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartPostCondition : Layout . start:PostCondition Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(314));
                        }
                    }
                    // StartPostCondition : Layout . start:PostCondition Layout
                    SlotId(314) => {
                        self.create(NonterminalId(11), result, gss_node_id, SlotId(315), env);
                    }
                    // StartPostCondition : Layout start:PostCondition . Layout
                    SlotId(315) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(315), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(316), env)
                            {
                                // StartPostCondition : Layout start:PostCondition Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(316));
                            }
                        }
                    }
                    // StartPostCondition : Layout start:PostCondition Layout.
                    SlotId(316) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(59),
                            SlotId(316),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(316), nonterminal_node_id, None);
                    }
                    // StartRegex : . Layout start:Regex Layout
                    SlotId(317) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(317), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRegex : Layout . start:Regex Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(318));
                        }
                    }
                    // StartRegex : Layout . start:Regex Layout
                    SlotId(318) => {
                        self.create(NonterminalId(12), result, gss_node_id, SlotId(319), env);
                    }
                    // StartRegex : Layout start:Regex . Layout
                    SlotId(319) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(319), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(320), env)
                            {
                                // StartRegex : Layout start:Regex Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(320));
                            }
                        }
                    }
                    // StartRegex : Layout start:Regex Layout.
                    SlotId(320) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(60),
                            SlotId(320),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(320), nonterminal_node_id, None);
                    }
                    // StartCharClass : . Layout start:CharClass Layout
                    SlotId(321) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(321), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartCharClass : Layout . start:CharClass Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(322));
                        }
                    }
                    // StartCharClass : Layout . start:CharClass Layout
                    SlotId(322) => {
                        self.create(NonterminalId(13), result, gss_node_id, SlotId(323), env);
                    }
                    // StartCharClass : Layout start:CharClass . Layout
                    SlotId(323) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(323), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(324), env)
                            {
                                // StartCharClass : Layout start:CharClass Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(324));
                            }
                        }
                    }
                    // StartCharClass : Layout start:CharClass Layout.
                    SlotId(324) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(61),
                            SlotId(324),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(324), nonterminal_node_id, None);
                    }
                    // StartRangeElement : . Layout start:RangeElement Layout
                    SlotId(325) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(325), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRangeElement : Layout . start:RangeElement Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(326));
                        }
                    }
                    // StartRangeElement : Layout . start:RangeElement Layout
                    SlotId(326) => {
                        self.create(NonterminalId(14), result, gss_node_id, SlotId(327), env);
                    }
                    // StartRangeElement : Layout start:RangeElement . Layout
                    SlotId(327) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(327), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(328), env)
                            {
                                // StartRangeElement : Layout start:RangeElement Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(328));
                            }
                        }
                    }
                    // StartRangeElement : Layout start:RangeElement Layout.
                    SlotId(328) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(62),
                            SlotId(328),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(328), nonterminal_node_id, None);
                    }
                    // StartRange : . Layout start:Range Layout
                    SlotId(329) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(329), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartRange : Layout . start:Range Layout
                            input_index = j;
                            result = Some(right_child);
                            next = Some(SlotId(330));
                        }
                    }
                    // StartRange : Layout . start:Range Layout
                    SlotId(330) => {
                        if let Some(right_child) = self
                            .parse_range_ll1(input_index, Some((SlotId(330), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(331), env)
                            {
                                // StartRange : Layout start:Range . Layout
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(331));
                            }
                        }
                    }
                    // StartRange : Layout start:Range . Layout
                    SlotId(331) => {
                        if let Some(right_child) = self
                            .parse_layout_ll1(input_index, Some((SlotId(331), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                                self.create_intermediate_node(result, right_child, SlotId(332), env)
                            {
                                // StartRange : Layout start:Range Layout.
                                input_index = j;
                                result = Some(new_node);
                                next = Some(SlotId(332));
                            }
                        }
                    }
                    // StartRange : Layout start:Range Layout.
                    SlotId(332) => {
                        let nonterminal_node_id = self.create_nonterminal_node(
                            result,
                            NonterminalId(63),
                            SlotId(332),
                            gss_node_id,
                        );
                        self.pop(gss_node_id, SlotId(332), nonterminal_node_id, None);
                    }
                    _ => {
                        panic!("Unknown grammar slot id: {slot_id}");
                    }
                }
            }
        }
    }
    fn add_first_descriptors(
        &mut self,
        nonterminal_id: NonterminalId,
        input_index: u32,
        gss_node_id: GssNodeId,
        env: Option<EnvId>,
    ) {
        match nonterminal_id {
            // Grammar : . "grammar" Layout name:Identifier Layout Star_0
            NonterminalId(0) => {
                self.add_first_descriptor(SlotId(0), input_index, gss_node_id, env);
            }
            // Rule
            NonterminalId(1) => {
                let mut matched = false;
                // Rule : . SyntaxRule
                if self.scanner.match_any(&FIRST_SET_RULE_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(6), input_index, gss_node_id, env);
                }
                // Rule : . RegexRule
                if self.scanner.match_any(&FIRST_SET_RULE_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(8), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(6),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_RULE),
                    );
                }
            }
            // SyntaxRule : . Opt_1 Layout head:Identifier Layout "=" Layout Star_1
            NonterminalId(2) => {
                self.add_first_descriptor(SlotId(10), input_index, gss_node_id, env);
            }
            // Annotation
            NonterminalId(3) => {
                let mut matched = false;
                // Annotation : . "@NoLayout"
                if self
                    .scanner
                    .match_any(&FIRST_SET_ANNOTATION_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(18), input_index, gss_node_id, env);
                }
                // Annotation : . "@Layout"
                if self
                    .scanner
                    .match_any(&FIRST_SET_ANNOTATION_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(20), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(18),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_ANNOTATION),
                    );
                }
            }
            // RegexRule : . layout:Opt_3 Layout id_annot:Opt_4 Layout "@Regex" Layout Identifier Layout
            // "=" Layout Opt_5 Layout body:Plus_2 Layout Star_2
            NonterminalId(4) => {
                self.add_first_descriptor(SlotId(22), input_index, gss_node_id, env);
            }
            // RegexPreCondition : . Identifier Layout "!<<"
            NonterminalId(5) => {
                self.add_first_descriptor(SlotId(38), input_index, gss_node_id, env);
            }
            // RegexPostCondition
            NonterminalId(6) => {
                let mut matched = false;
                // RegexPostCondition : . "\\" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_REGEX_POST_CONDITION_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(42), input_index, gss_node_id, env);
                }
                // RegexPostCondition : . "!>>" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_REGEX_POST_CONDITION_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(46), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(42),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_REGEX_POST_CONDITION),
                    );
                }
            }
            // PriorityLevel : . Opt_7 Layout Star_3
            NonterminalId(7) => {
                self.add_first_descriptor(SlotId(50), input_index, gss_node_id, env);
            }
            // Associativity
            NonterminalId(8) => {
                let mut matched = false;
                // Associativity : . "left"
                if self
                    .scanner
                    .match_any(&FIRST_SET_ASSOCIATIVITY_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(54), input_index, gss_node_id, env);
                }
                // Associativity : . "right"
                if self
                    .scanner
                    .match_any(&FIRST_SET_ASSOCIATIVITY_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(56), input_index, gss_node_id, env);
                }
                // Associativity : . "none"
                if self
                    .scanner
                    .match_any(&FIRST_SET_ASSOCIATIVITY_ALT2, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(58), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(54),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_ASSOCIATIVITY),
                    );
                }
            }
            // Alternative
            NonterminalId(9) => {
                let mut matched = false;
                // Alternative : . Plus_6 Layout Opt_9
                if self
                    .scanner
                    .match_any(&FIRST_SET_ALTERNATIVE_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(60), input_index, gss_node_id, env);
                }
                // Alternative : . "(" Layout ")" Layout Opt_9
                if self
                    .scanner
                    .match_any(&FIRST_SET_ALTERNATIVE_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(64), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(60),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_ALTERNATIVE),
                    );
                }
            }
            // Symbol
            NonterminalId(64) => {
                let mut matched = false;
                // Symbol(p: i32, e: i32) : . [1 & e == 0] Identifier return (0, 0)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(333), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [2 & e == 0] "(" Layout seqs:Plus_7 Layout ")" return (0, 1)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(337), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [4 & e == 0] String return (0, 2)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT2, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(345), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [8 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout
                // sep:Symbol(0, 0) Layout "}" Layout "*" return (0, 3)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT3, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(349), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [16 & e == 0] "{" Layout symbol:Symbol(0, 0) Layout
                // sep:Symbol(0, 0) Layout "}" Layout "+" return (0, 4)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT4, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(361), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [32 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                // 0) || (l_pr >= 3)] Layout "*" return (0, 5)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT5, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(373), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [64 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                // 0) || (l_pr >= 3)] Layout "+" return (0, 6)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT6, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(381), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [128 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                // 0) || (l_pr >= 3)] Layout "?" return (0, 7)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT7, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(389), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [256 & e == 0] [3 >= p] (l_pr, l_label)=Symbol(p, 0) [(l_pr ==
                // 0) || (l_pr >= 3)] [(l_label == NO_LABEL) || ((256 >> l_label) & 1 == 0)] Layout
                // conditions:Plus_8 return (0, 8)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT8, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(397), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [512 & e == 0] conditions:Plus_9 Layout (r_pr,
                // r_label)=Symbol(2, 512) return ((r_pr == 0) ? 2 : min(r_pr, 2), 9)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT9, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(406), input_index, gss_node_id, env);
                }
                // Symbol(p: i32, e: i32) : . [1024 & e == 0] label:Identifier Layout ":" Layout Symbol(1, 0)
                // return (1, 10)
                if self.scanner.match_any(&FIRST_SET_SYMBOL_ALT10, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(412), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(333),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_SYMBOL),
                    );
                }
            }
            // PreCondition : . Identifier Layout "!<<"
            NonterminalId(10) => {
                self.add_first_descriptor(SlotId(70), input_index, gss_node_id, env);
            }
            // PostCondition
            NonterminalId(11) => {
                let mut matched = false;
                // PostCondition : . "\\" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_POST_CONDITION_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(74), input_index, gss_node_id, env);
                }
                // PostCondition : . "!>>" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_POST_CONDITION_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(78), input_index, gss_node_id, env);
                }
                // PostCondition : . "!>>>" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_POST_CONDITION_ALT2, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(82), input_index, gss_node_id, env);
                }
                // PostCondition : . "!" Layout Identifier
                if self
                    .scanner
                    .match_any(&FIRST_SET_POST_CONDITION_ALT3, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(86), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(74),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_POST_CONDITION),
                    );
                }
            }
            // Regex
            NonterminalId(12) => {
                let mut matched = false;
                // Regex : . Regex Layout "+"
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(90), input_index, gss_node_id, env);
                }
                // Regex : . Regex Layout "*"
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(94), input_index, gss_node_id, env);
                }
                // Regex : . Regex Layout "?"
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT2, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(98), input_index, gss_node_id, env);
                }
                // Regex : . "(" Layout seqs:Plus_2 Layout ")"
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT3, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(102), input_index, gss_node_id, env);
                }
                // Regex : . CharClass
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT4, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(108), input_index, gss_node_id, env);
                }
                // Regex : . Char
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT5, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(110), input_index, gss_node_id, env);
                }
                // Regex : . String
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT6, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(112), input_index, gss_node_id, env);
                }
                // Regex : . Identifier
                if self.scanner.match_any(&FIRST_SET_REGEX_ALT7, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(114), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(90),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_REGEX),
                    );
                }
            }
            // CharClass : . neg:Opt_10 Layout "[" Layout Plus_10 Layout "]"
            NonterminalId(13) => {
                self.add_first_descriptor(SlotId(116), input_index, gss_node_id, env);
            }
            // RangeElement
            NonterminalId(14) => {
                let mut matched = false;
                // RangeElement : . Range
                if self
                    .scanner
                    .match_any(&FIRST_SET_RANGE_ELEMENT_ALT0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(124), input_index, gss_node_id, env);
                }
                // RangeElement : . RangeChar
                if self
                    .scanner
                    .match_any(&FIRST_SET_RANGE_ELEMENT_ALT1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(126), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(124),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_RANGE_ELEMENT),
                    );
                }
            }
            // Range : . start:RangeChar Layout "-" Layout end:RangeChar
            NonterminalId(15) => {
                self.add_first_descriptor(SlotId(128), input_index, gss_node_id, env);
            }
            // Layout : . Star_4 !>> WS !>> LineComment
            NonterminalId(16) => {
                self.add_first_descriptor(SlotId(134), input_index, gss_node_id, env);
            }
            // Plus_0
            NonterminalId(17) => {
                let mut matched = false;
                // Plus_0 : . Plus_0 Layout Rule
                if self.scanner.match_any(&FIRST_SET_PLUS_0_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(136), input_index, gss_node_id, env);
                }
                // Plus_0 : . Rule
                if self.scanner.match_any(&FIRST_SET_PLUS_0_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(140), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(136),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_0),
                    );
                }
            }
            // Opt_0
            NonterminalId(18) => {
                let mut matched = false;
                // Opt_0 : . Plus_0
                if self.scanner.match_any(&FIRST_SET_OPT_0_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(142), input_index, gss_node_id, env);
                }
                // Opt_0 : .
                if self.scanner.match_any(&FIRST_SET_OPT_0_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_0, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(144), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(142),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_0),
                    );
                }
            }
            // Star_0 : . Opt_0
            NonterminalId(19) => {
                self.add_first_descriptor(SlotId(145), input_index, gss_node_id, env);
            }
            // Opt_1
            NonterminalId(20) => {
                let mut matched = false;
                // Opt_1 : . Annotation
                if self.scanner.match_any(&FIRST_SET_OPT_1_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(147), input_index, gss_node_id, env);
                }
                // Opt_1 : .
                if self.scanner.match_any(&FIRST_SET_OPT_1_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(149), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(147),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_1),
                    );
                }
            }
            // Plus_1
            NonterminalId(21) => {
                let mut matched = false;
                // Plus_1 : . Plus_1 Layout ">" Layout PriorityLevel
                if self.scanner.match_any(&FIRST_SET_PLUS_1_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(150), input_index, gss_node_id, env);
                }
                // Plus_1 : . PriorityLevel
                if self.scanner.match_any(&FIRST_SET_PLUS_1_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_PLUS_1, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(156), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(150),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_PLUS_1),
                    );
                }
            }
            // Opt_2
            NonterminalId(22) => {
                let mut matched = false;
                // Opt_2 : . Plus_1
                if self.scanner.match_any(&FIRST_SET_OPT_2_ALT0, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_2, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(158), input_index, gss_node_id, env);
                }
                // Opt_2 : .
                if self.scanner.match_any(&FIRST_SET_OPT_2_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_2, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(160), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(158),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_2),
                    );
                }
            }
            // Star_1 : . Opt_2
            NonterminalId(23) => {
                self.add_first_descriptor(SlotId(161), input_index, gss_node_id, env);
            }
            // Opt_3
            NonterminalId(24) => {
                let mut matched = false;
                // Opt_3 : . "@Layout"
                if self.scanner.match_any(&FIRST_SET_OPT_3_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(163), input_index, gss_node_id, env);
                }
                // Opt_3 : .
                if self.scanner.match_any(&FIRST_SET_OPT_3_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_3, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(165), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(163),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_3),
                    );
                }
            }
            // Opt_4
            NonterminalId(25) => {
                let mut matched = false;
                // Opt_4 : . "@Identifier"
                if self.scanner.match_any(&FIRST_SET_OPT_4_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(166), input_index, gss_node_id, env);
                }
                // Opt_4 : .
                if self.scanner.match_any(&FIRST_SET_OPT_4_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_4, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(168), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(166),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_4),
                    );
                }
            }
            // Opt_5
            NonterminalId(26) => {
                let mut matched = false;
                // Opt_5 : . RegexPreCondition
                if self.scanner.match_any(&FIRST_SET_OPT_5_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(169), input_index, gss_node_id, env);
                }
                // Opt_5 : .
                if self.scanner.match_any(&FIRST_SET_OPT_5_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_5, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(171), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(169),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_5),
                    );
                }
            }
            // Plus_3
            NonterminalId(27) => {
                let mut matched = false;
                // Plus_3 : . Plus_3 Layout Regex
                if self.scanner.match_any(&FIRST_SET_PLUS_3_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(172), input_index, gss_node_id, env);
                }
                // Plus_3 : . Regex
                if self.scanner.match_any(&FIRST_SET_PLUS_3_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(176), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(172),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_3),
                    );
                }
            }
            // Plus_2
            NonterminalId(28) => {
                let mut matched = false;
                // Plus_2 : . Plus_2 Layout "|" Layout Plus_3
                if self.scanner.match_any(&FIRST_SET_PLUS_2_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(178), input_index, gss_node_id, env);
                }
                // Plus_2 : . Plus_3
                if self.scanner.match_any(&FIRST_SET_PLUS_2_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(184), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(178),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_2),
                    );
                }
            }
            // Plus_4
            NonterminalId(29) => {
                let mut matched = false;
                // Plus_4 : . Plus_4 Layout RegexPostCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_4_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(186), input_index, gss_node_id, env);
                }
                // Plus_4 : . RegexPostCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_4_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(190), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(186),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_4),
                    );
                }
            }
            // Opt_6
            NonterminalId(30) => {
                let mut matched = false;
                // Opt_6 : . Plus_4
                if self.scanner.match_any(&FIRST_SET_OPT_6_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(192), input_index, gss_node_id, env);
                }
                // Opt_6 : .
                if self.scanner.match_any(&FIRST_SET_OPT_6_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_6, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(194), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(192),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_6),
                    );
                }
            }
            // Star_2 : . Opt_6
            NonterminalId(31) => {
                self.add_first_descriptor(SlotId(195), input_index, gss_node_id, env);
            }
            // Opt_7
            NonterminalId(32) => {
                let mut matched = false;
                // Opt_7 : . Associativity
                if self.scanner.match_any(&FIRST_SET_OPT_7_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(197), input_index, gss_node_id, env);
                }
                // Opt_7 : .
                if self.scanner.match_any(&FIRST_SET_OPT_7_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_7, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(199), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(197),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_7),
                    );
                }
            }
            // Plus_5
            NonterminalId(33) => {
                let mut matched = false;
                // Plus_5 : . Plus_5 Layout "|" Layout Alternative
                if self.scanner.match_any(&FIRST_SET_PLUS_5_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(200), input_index, gss_node_id, env);
                }
                // Plus_5 : . Alternative
                if self.scanner.match_any(&FIRST_SET_PLUS_5_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(206), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(200),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_5),
                    );
                }
            }
            // Opt_8
            NonterminalId(34) => {
                let mut matched = false;
                // Opt_8 : . Plus_5
                if self.scanner.match_any(&FIRST_SET_OPT_8_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(208), input_index, gss_node_id, env);
                }
                // Opt_8 : .
                if self.scanner.match_any(&FIRST_SET_OPT_8_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_8, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(210), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(208),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_8),
                    );
                }
            }
            // Star_3 : . Opt_8
            NonterminalId(35) => {
                self.add_first_descriptor(SlotId(211), input_index, gss_node_id, env);
            }
            // Plus_6
            NonterminalId(36) => {
                let mut matched = false;
                // Plus_6 : . Plus_6 Layout Symbol(0, 0)
                if self.scanner.match_any(&FIRST_SET_PLUS_6_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(213), input_index, gss_node_id, env);
                }
                // Plus_6 : . Symbol(0, 0)
                if self.scanner.match_any(&FIRST_SET_PLUS_6_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(217), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(213),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_6),
                    );
                }
            }
            // Opt_9
            NonterminalId(37) => {
                let mut matched = false;
                // Opt_9 : . Label
                if self.scanner.match_any(&FIRST_SET_OPT_9_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(219), input_index, gss_node_id, env);
                }
                // Opt_9 : .
                if self.scanner.match_any(&FIRST_SET_OPT_9_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_9, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(221), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(219),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_9),
                    );
                }
            }
            // Plus_7
            NonterminalId(38) => {
                let mut matched = false;
                // Plus_7 : . Plus_7 Layout "|" Layout Plus_6
                if self.scanner.match_any(&FIRST_SET_PLUS_7_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(222), input_index, gss_node_id, env);
                }
                // Plus_7 : . Plus_6
                if self.scanner.match_any(&FIRST_SET_PLUS_7_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(228), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(222),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_7),
                    );
                }
            }
            // Plus_8
            NonterminalId(39) => {
                let mut matched = false;
                // Plus_8 : . Plus_8 Layout PostCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_8_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(230), input_index, gss_node_id, env);
                }
                // Plus_8 : . PostCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_8_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(234), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(230),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_8),
                    );
                }
            }
            // Plus_9
            NonterminalId(40) => {
                let mut matched = false;
                // Plus_9 : . Plus_9 Layout PreCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_9_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(236), input_index, gss_node_id, env);
                }
                // Plus_9 : . PreCondition
                if self.scanner.match_any(&FIRST_SET_PLUS_9_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(240), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(236),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_9),
                    );
                }
            }
            // Opt_10
            NonterminalId(41) => {
                let mut matched = false;
                // Opt_10 : . "!"
                if self.scanner.match_any(&FIRST_SET_OPT_10_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(242), input_index, gss_node_id, env);
                }
                // Opt_10 : .
                if self.scanner.match_any(&FIRST_SET_OPT_10_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_10, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(244), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(242),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_10),
                    );
                }
            }
            // Plus_10
            NonterminalId(42) => {
                let mut matched = false;
                // Plus_10 : . Plus_10 Layout RangeElement
                if self.scanner.match_any(&FIRST_SET_PLUS_10_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(245), input_index, gss_node_id, env);
                }
                // Plus_10 : . RangeElement
                if self.scanner.match_any(&FIRST_SET_PLUS_10_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(249), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(245),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_10),
                    );
                }
            }
            // Alt_0
            NonterminalId(43) => {
                let mut matched = false;
                // Alt_0 : . WS
                if self.scanner.match_any(&FIRST_SET_ALT_0_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(251), input_index, gss_node_id, env);
                }
                // Alt_0 : . LineComment
                if self.scanner.match_any(&FIRST_SET_ALT_0_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(253), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(251),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_ALT_0),
                    );
                }
            }
            // Plus_11
            NonterminalId(44) => {
                let mut matched = false;
                // Plus_11 : . Plus_11 Alt_0
                if self.scanner.match_any(&FIRST_SET_PLUS_11_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(255), input_index, gss_node_id, env);
                }
                // Plus_11 : . Alt_0
                if self.scanner.match_any(&FIRST_SET_PLUS_11_ALT1, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(258), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(255),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_11),
                    );
                }
            }
            // Opt_11
            NonterminalId(45) => {
                let mut matched = false;
                // Opt_11 : . Plus_11
                if self.scanner.match_any(&FIRST_SET_OPT_11_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(260), input_index, gss_node_id, env);
                }
                // Opt_11 : .
                if self.scanner.match_any(&FIRST_SET_OPT_11_ALT1, input_index)
                    || self.scanner.match_any(&FOLLOW_SET_OPT_11, input_index)
                {
                    matched = true;
                    self.add_first_descriptor(SlotId(262), input_index, gss_node_id, env);
                }
                if !matched {
                    self.add_failure(
                        input_index,
                        SlotId(260),
                        Some(gss_node_id),
                        GLLFailureKind::UnexpectedToken(&PREDICTION_SET_OPT_11),
                    );
                }
            }
            // Star_4 : . Opt_11
            NonterminalId(46) => {
                self.add_first_descriptor(SlotId(263), input_index, gss_node_id, env);
            }
            // StartGrammar : . Layout start:Grammar Layout
            NonterminalId(47) => {
                self.add_first_descriptor(SlotId(265), input_index, gss_node_id, env);
            }
            // StartRule : . Layout start:Rule Layout
            NonterminalId(48) => {
                self.add_first_descriptor(SlotId(269), input_index, gss_node_id, env);
            }
            // StartSyntaxRule : . Layout start:SyntaxRule Layout
            NonterminalId(49) => {
                self.add_first_descriptor(SlotId(273), input_index, gss_node_id, env);
            }
            // StartAnnotation : . Layout start:Annotation Layout
            NonterminalId(50) => {
                self.add_first_descriptor(SlotId(277), input_index, gss_node_id, env);
            }
            // StartRegexRule : . Layout start:RegexRule Layout
            NonterminalId(51) => {
                self.add_first_descriptor(SlotId(281), input_index, gss_node_id, env);
            }
            // StartRegexPreCondition : . Layout start:RegexPreCondition Layout
            NonterminalId(52) => {
                self.add_first_descriptor(SlotId(285), input_index, gss_node_id, env);
            }
            // StartRegexPostCondition : . Layout start:RegexPostCondition Layout
            NonterminalId(53) => {
                self.add_first_descriptor(SlotId(289), input_index, gss_node_id, env);
            }
            // StartPriorityLevel : . Layout start:PriorityLevel Layout
            NonterminalId(54) => {
                self.add_first_descriptor(SlotId(293), input_index, gss_node_id, env);
            }
            // StartAssociativity : . Layout start:Associativity Layout
            NonterminalId(55) => {
                self.add_first_descriptor(SlotId(297), input_index, gss_node_id, env);
            }
            // StartAlternative : . Layout start:Alternative Layout
            NonterminalId(56) => {
                self.add_first_descriptor(SlotId(301), input_index, gss_node_id, env);
            }
            // StartSymbol : . Layout start:Symbol(0, 0) Layout
            NonterminalId(57) => {
                self.add_first_descriptor(SlotId(305), input_index, gss_node_id, env);
            }
            // StartPreCondition : . Layout start:PreCondition Layout
            NonterminalId(58) => {
                self.add_first_descriptor(SlotId(309), input_index, gss_node_id, env);
            }
            // StartPostCondition : . Layout start:PostCondition Layout
            NonterminalId(59) => {
                self.add_first_descriptor(SlotId(313), input_index, gss_node_id, env);
            }
            // StartRegex : . Layout start:Regex Layout
            NonterminalId(60) => {
                self.add_first_descriptor(SlotId(317), input_index, gss_node_id, env);
            }
            // StartCharClass : . Layout start:CharClass Layout
            NonterminalId(61) => {
                self.add_first_descriptor(SlotId(321), input_index, gss_node_id, env);
            }
            // StartRangeElement : . Layout start:RangeElement Layout
            NonterminalId(62) => {
                self.add_first_descriptor(SlotId(325), input_index, gss_node_id, env);
            }
            // StartRange : . Layout start:Range Layout
            NonterminalId(63) => {
                self.add_first_descriptor(SlotId(329), input_index, gss_node_id, env);
            }
            _ => {
                panic!("Unknown nonterminal id: {nonterminal_id}");
            }
        }
    }
    fn get_gss_node(&self, nonterminal_id: NonterminalId, input_index: u32) -> Option<GssNodeId> {
        self.gss_nodes_index[nonterminal_id.index()]
            .get(&input_index)
            .copied()
    }
    fn add_gss_node(
        &mut self,
        nonterminal_id: NonterminalId,
        input_index: u32,
        gss_node_id: GssNodeId,
    ) {
        let arena = self.vec_arena;
        self.gss_nodes_index[nonterminal_id.index()].insert(input_index, gss_node_id, arena);
    }
    fn new_gss_node(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> GssNodeId {
        let gss_node_id = GssNodeId(self.gss_nodes.len() as u32);
        let gss_node = GSSNode::new(gss_node_id, nonterminal_id, input_index);
        record!(self, GSSNodeCreated, nonterminal_id, input_index);
        self.gss_nodes.push(gss_node);
        gss_node_id
    }
    fn gss_node(&self, id: GssNodeId) -> &GSSNode<'arena> {
        &self.gss_nodes[id.index()]
    }
    fn gss_node_mut(&mut self, id: GssNodeId) -> &mut GSSNode<'arena> {
        self.gss_nodes
            .get_mut(id.index())
            .expect("GSS node id should be valid")
    }
    fn sppf_node(&self, id: SPPFNodeId) -> &SPPFNode {
        &self.sppf_nodes[id.index()]
    }
    fn sppf_node_mut(&mut self, id: SPPFNodeId) -> &mut SPPFNode {
        &mut self.sppf_nodes[id.index()]
    }
    fn add_descriptor(&mut self, descriptor: Descriptor) {
        record!(
            self,
            DescriptorAdded,
            descriptor.input_index,
            descriptor.slot_id,
            descriptor.sppf_node_id(),
            descriptor.gss_node_id
        );
        #[cfg(feature = "instrument")]
        self.increment_descriptor_count();
        self.descriptors.push(descriptor);
        #[cfg(feature = "instrument")]
        {
            if self.descriptors.len() > self.descriptors_peak {
                self.descriptors_peak = self.descriptors.len();
            }
        }
    }
    fn next_descriptor(&mut self) -> Option<Descriptor> {
        self.descriptors.pop()
    }
    fn clear_descriptors(&mut self) {
        self.descriptors.clear();
    }
    fn add_terminal_node(&mut self, terminal_node: TerminalNode) -> SPPFNodeId {
        let terminal_node_id = SPPFNodeId(self.sppf_nodes.len() as u32);
        let arena = self.vec_arena;
        self.terminal_nodes_index[terminal_node.terminal_id.index()].insert(
            terminal_node.span,
            terminal_node_id,
            arena,
        );
        record!(
            self,
            TerminalNodeCreated,
            terminal_node.terminal_id,
            terminal_node.span
        );
        self.sppf_nodes.push(SPPFNode::Terminal(terminal_node));
        terminal_node_id
    }
    fn add_nonterminal_node(&mut self, nonterminal_node: NonterminalNode) -> SPPFNodeId {
        let nonterminal_node_id = SPPFNodeId(self.sppf_nodes.len() as u32);
        record!(
            self,
            NonterminalNodeCreated,
            nonterminal_node.nonterminal_id,
            nonterminal_node.span,
            nonterminal_node.child
        );
        self.sppf_nodes
            .push(SPPFNode::Nonterminal(nonterminal_node));
        nonterminal_node_id
    }
    fn add_intermediate_node(
        &mut self,
        intermediate_node: IntermediateNode,
        env: Option<EnvId>,
        add_to_index: bool,
    ) -> SPPFNodeId {
        let intermediate_node_id = SPPFNodeId(self.sppf_nodes.len() as u32);
        if add_to_index {
            let arena = self.vec_arena;
            let slot_idx = intermediate_node.slot_id.index();
            if slot_idx < 333 {
                self.intermediate_nodes_index[slot_idx].insert(
                    intermediate_node.span,
                    intermediate_node_id,
                    arena,
                );
            } else {
                let idx = slot_idx - 333;
                self.dd_intermediate_nodes_index[idx].insert(
                    (intermediate_node.span, env),
                    intermediate_node_id,
                    arena,
                );
            }
        }
        record!(
            self,
            IntermediateNodeCreated,
            intermediate_node.slot_id,
            intermediate_node.span,
            intermediate_node.child.0,
            intermediate_node.child.1
        );
        self.sppf_nodes
            .push(SPPFNode::Intermediate(intermediate_node));
        intermediate_node_id
    }
    fn input(&self) -> &'i Input {
        self.scanner.input
    }
    fn sppf_nodes(&self) -> &[SPPFNode] {
        &self.sppf_nodes
    }
    #[cfg(feature = "instrument")]
    fn increment_descriptor_count(&mut self) {
        self.descriptors_count += 1;
    }
    #[cfg(feature = "instrument")]
    fn count_descriptors(&self) -> usize {
        self.descriptors_count
    }
    #[cfg(feature = "instrument")]
    fn count_gss_nodes(&self) -> usize {
        self.gss_nodes.len()
    }
    #[cfg(feature = "instrument")]
    fn count_gss_edges(&self) -> usize {
        self.gss_nodes.iter().map(|n| n.edges().len()).sum()
    }
    #[cfg(feature = "instrument")]
    fn count_nonterminal_nodes(&self) -> usize {
        self.sppf_nodes
            .iter()
            .filter(|n| matches!(n, SPPFNode::Nonterminal(_)))
            .count()
    }
    #[cfg(feature = "instrument")]
    fn count_intermediate_nodes(&self) -> usize {
        self.sppf_nodes
            .iter()
            .filter(|n| matches!(n, SPPFNode::Intermediate(_)))
            .count()
    }
    #[cfg(feature = "instrument")]
    fn count_terminal_nodes(&self) -> usize {
        self.sppf_nodes
            .iter()
            .filter(|n| matches!(n, SPPFNode::Terminal(_)))
            .count()
    }
    #[cfg(feature = "instrument")]
    fn count_ambiguous_nodes(&self) -> usize {
        self.sppf_nodes
            .iter()
            .filter(|n| match n {
                SPPFNode::Nonterminal(nn) => nn.ambiguous,
                SPPFNode::Intermediate(in_) => in_.ambiguous,
                SPPFNode::Terminal(_) => false,
            })
            .count()
    }
    fn lookup_intermediate_node(
        &self,
        slot_id: SlotId,
        left_extent: u32,
        right_extent: u32,
        env: Option<EnvId>,
    ) -> Option<SPPFNodeId> {
        let slot_idx = slot_id.index();
        let span = Span::new(left_extent, right_extent);
        if slot_idx < 333 {
            self.intermediate_nodes_index[slot_idx].get(&span).copied()
        } else {
            let idx = slot_idx - 333;
            self.dd_intermediate_nodes_index[idx]
                .get(&(span, env))
                .copied()
        }
    }
    fn lookup_terminal_node(
        &self,
        terminal_id: TerminalId,
        left_extent: u32,
        right_extent: u32,
    ) -> Option<SPPFNodeId> {
        let map = &self.terminal_nodes_index[terminal_id.index()];
        map.get(&Span::new(left_extent, right_extent)).copied()
    }
    fn gss_nodes<'p>(&'p self) -> impl Iterator<Item = &'p GSSNode<'arena>>
    where
        'arena: 'p,
    {
        self.gss_nodes.iter()
    }
    fn add_intermediate_node_child(
        &mut self,
        node: SPPFNodeId,
        child1: SPPFNodeId,
        child2: SPPFNodeId,
    ) {
        self.intermediate_nodes_children
            .push((node, (child1, child2)));
    }
    fn add_nonterminal_node_child(
        &mut self,
        node: SPPFNodeId,
        child: SPPFNodeId,
        return_slot: SlotId,
    ) {
        self.nonterminal_nodes_children
            .push((node, (child, return_slot)));
    }
    fn intermediate_nodes_children_map(
        &self,
    ) -> &FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SPPFNodeId)>> {
        self.intermediate_nodes_children_map.get_or_init(|| {
            let mut map: FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SPPFNodeId)>> =
                FxHashMap::default();
            for (k, v) in &self.intermediate_nodes_children {
                map.entry(*k).or_default().push(*v);
            }
            map
        })
    }
    fn nonterminal_nodes_children_map(&self) -> &FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SlotId)>> {
        self.nonterminal_nodes_children_map.get_or_init(|| {
            let mut map: FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SlotId)>> = FxHashMap::default();
            for (k, v) in &self.nonterminal_nodes_children {
                map.entry(*k).or_default().push(*v);
            }
            map
        })
    }
    #[cfg(feature = "debug-trace")]
    fn enable_trace(&mut self) {
        self.trace_events = Some(Vec::new());
    }
    #[cfg(feature = "debug-trace")]
    fn add_trace_event(&mut self, event: TraceEvent) {
        if let Some(trace_events) = &mut self.trace_events {
            trace_events.push(event);
        }
    }
    #[cfg(feature = "debug-trace")]
    fn trace_events(&self) -> &[TraceEvent] {
        self.trace_events.as_deref().unwrap_or(&[])
    }
    fn start_env(&mut self, start: NonterminalId) -> Option<EnvId> {
        match start {
            NonterminalId(64) => {
                let arena = self.vec_arena;
                let (env_id, env) = self.new_env();
                env.bind(BINDING_P, 0, arena);
                env.bind(BINDING_E, 0, arena);
                Some(env_id)
            }
            _ => None,
        }
    }
    // True if a local ambiguity node was added during parsing. This does not guarantee the
    // ambiguity is reachable from the root, so a tree walk is still needed to confirm it.
    fn ambiguity_node_added(&self) -> bool {
        !self.intermediate_nodes_children.is_empty() || !self.nonterminal_nodes_children.is_empty()
    }
    fn add_start_gss_node(
        &mut self,
        nonterminal_id: NonterminalId,
        input_index: u32,
        gss_node_id: GssNodeId,
    ) {
        match nonterminal_id {
            NonterminalId(64) => self.add_gss_node_symbol(input_index, 0, 0, gss_node_id),
            _ => self.add_gss_node(nonterminal_id, input_index, gss_node_id),
        }
    }
    fn new_env(&mut self) -> (EnvId, &mut Env<'arena>) {
        let id = EnvId(self.envs.len() as u32);
        self.envs.push(Env::default());
        (id, &mut self.envs[id.index()])
    }
    fn lookup(&self, name: BindingId, env_id: EnvId) -> i32 {
        let env = &self.envs[env_id.index()];
        env.get(name)
    }
    fn clone_env(&mut self, source: EnvId) -> (EnvId, &mut Env<'arena>) {
        let bindings = self.envs[source.0 as usize].bindings.clone();
        let (new_id, new_env) = self.new_env();
        new_env.bindings = bindings;
        (new_id, new_env)
    }
    fn bind_return_value(
        &mut self,
        return_slot: SlotId,
        env: Option<EnvId>,
        value: i32,
    ) -> Option<EnvId> {
        let bindings: &[(BindingId, i32)] = match return_slot {
            SlotId(376) => &[
                (BINDING_L_PR, (value) >> 16),
                (BINDING_L_LABEL, (value) & 65535),
            ],
            SlotId(384) => &[
                (BINDING_L_PR, (value) >> 16),
                (BINDING_L_LABEL, (value) & 65535),
            ],
            SlotId(392) => &[
                (BINDING_L_PR, (value) >> 16),
                (BINDING_L_LABEL, (value) & 65535),
            ],
            SlotId(400) => &[
                (BINDING_L_PR, (value) >> 16),
                (BINDING_L_LABEL, (value) & 65535),
            ],
            SlotId(410) => &[
                (BINDING_R_PR, (value) >> 16),
                (BINDING_R_LABEL, (value) & 65535),
            ],
            _ => return env,
        };
        let arena = self.vec_arena;
        let (id, target) = match env {
            Some(id) => self.clone_env(id),
            None => self.new_env(),
        };
        for &(name, value) in bindings {
            target.bind(name, value, arena);
        }
        Some(id)
    }
    fn envs(&self) -> &[Env<'arena>] {
        &self.envs
    }
    #[cfg(feature = "instrument")]
    fn record_stats(&self) -> iguana_runtime::instrument::Stats {
        let mut stats = iguana_runtime::instrument::Stats::new();
        stats.descriptors_count = self.count_descriptors();
        stats.descriptors_peak = self.descriptors_peak;
        stats.envs_count = self.envs.len();
        stats.gss_nodes_count = self.count_gss_nodes();
        stats.gss_edges_count = self.count_gss_edges();
        stats.nonterminal_nodes_count = self.count_nonterminal_nodes();
        stats.intermediate_nodes_count = self.count_intermediate_nodes();
        stats.terminal_nodes_count = self.count_terminal_nodes();
        stats.ambiguous_nodes_count = self.count_ambiguous_nodes();
        for node in self.gss_nodes() {
            stats.record("GssNode::edges: InlineVec", node.edges().len());
            stats.record(
                "GssNode::popped_elements: InlineMap",
                node.popped_elements().len(),
            );
        }
        for env in self.envs() {
            stats.record("Env::bindings: Bindings", env.bindings.len());
        }
        for m in self.intermediate_nodes_index.iter() {
            stats.record("Parser::intermediate_nodes_index: InlineMap", m.len());
        }
        for m in self.dd_intermediate_nodes_index.iter() {
            stats.record("Parser::dd_intermediate_nodes_index: InlineMap", m.len());
        }
        for m in self.terminal_nodes_index.iter() {
            stats.record("Parser::terminal_nodes_index: InlineMap", m.len());
        }
        for m in self.gss_nodes_index.iter() {
            stats.record("Parser::gss_nodes_index: InlineMap", m.len());
        }
        stats.record(
            "Parser::gss_nodes_index_symbol",
            self.gss_nodes_index_symbol.len(),
        );
        for (nt_id, pos) in &self.ll1_call_log {
            let name = Self::Grammar::nonterminal_display_name(*nt_id);
            stats.record_ll1_call(name, *pos);
        }
        stats
    }
    fn post_conditions(
        &mut self,
        slot: SlotId,
        _left_extent: u32,
        right_extent: u32,
    ) -> Option<GLLFailureKind> {
        match slot {
            SlotId(135) => {
                if self
                    .scanner
                    .match_any(&FOLLOW_RESTRICTION_LAYOUT_ALT0_POS0, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow(
                        &FOLLOW_RESTRICTION_LAYOUT_ALT0_POS0,
                    ))
                } else {
                    None
                }
            }
            _ => None,
        }
    }
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            NonterminalId(0) => self.scanner.match_any(&FOLLOW_SET_GRAMMAR, input_index),
            NonterminalId(1) => self.scanner.match_any(&FOLLOW_SET_RULE, input_index),
            NonterminalId(2) => self.scanner.match_any(&FOLLOW_SET_SYNTAX_RULE, input_index),
            NonterminalId(3) => self.scanner.match_any(&FOLLOW_SET_ANNOTATION, input_index),
            NonterminalId(4) => self.scanner.match_any(&FOLLOW_SET_REGEX_RULE, input_index),
            NonterminalId(5) => self
                .scanner
                .match_any(&FOLLOW_SET_REGEX_PRE_CONDITION, input_index),
            NonterminalId(6) => self
                .scanner
                .match_any(&FOLLOW_SET_REGEX_POST_CONDITION, input_index),
            NonterminalId(7) => self
                .scanner
                .match_any(&FOLLOW_SET_PRIORITY_LEVEL, input_index),
            NonterminalId(8) => self
                .scanner
                .match_any(&FOLLOW_SET_ASSOCIATIVITY, input_index),
            NonterminalId(9) => self.scanner.match_any(&FOLLOW_SET_ALTERNATIVE, input_index),
            NonterminalId(64) => self.scanner.match_any(&FOLLOW_SET_SYMBOL, input_index),
            NonterminalId(10) => self
                .scanner
                .match_any(&FOLLOW_SET_PRE_CONDITION, input_index),
            NonterminalId(11) => self
                .scanner
                .match_any(&FOLLOW_SET_POST_CONDITION, input_index),
            NonterminalId(12) => self.scanner.match_any(&FOLLOW_SET_REGEX, input_index),
            NonterminalId(13) => self.scanner.match_any(&FOLLOW_SET_CHAR_CLASS, input_index),
            NonterminalId(14) => self
                .scanner
                .match_any(&FOLLOW_SET_RANGE_ELEMENT, input_index),
            NonterminalId(15) => self.scanner.match_any(&FOLLOW_SET_RANGE, input_index),
            NonterminalId(16) => self.scanner.match_any(&FOLLOW_SET_LAYOUT, input_index),
            NonterminalId(17) => self.scanner.match_any(&FOLLOW_SET_PLUS_0, input_index),
            NonterminalId(18) => self.scanner.match_any(&FOLLOW_SET_OPT_0, input_index),
            NonterminalId(19) => self.scanner.match_any(&FOLLOW_SET_STAR_0, input_index),
            NonterminalId(20) => self.scanner.match_any(&FOLLOW_SET_OPT_1, input_index),
            NonterminalId(21) => self.scanner.match_any(&FOLLOW_SET_PLUS_1, input_index),
            NonterminalId(22) => self.scanner.match_any(&FOLLOW_SET_OPT_2, input_index),
            NonterminalId(23) => self.scanner.match_any(&FOLLOW_SET_STAR_1, input_index),
            NonterminalId(24) => self.scanner.match_any(&FOLLOW_SET_OPT_3, input_index),
            NonterminalId(25) => self.scanner.match_any(&FOLLOW_SET_OPT_4, input_index),
            NonterminalId(26) => self.scanner.match_any(&FOLLOW_SET_OPT_5, input_index),
            NonterminalId(27) => self.scanner.match_any(&FOLLOW_SET_PLUS_3, input_index),
            NonterminalId(28) => self.scanner.match_any(&FOLLOW_SET_PLUS_2, input_index),
            NonterminalId(29) => self.scanner.match_any(&FOLLOW_SET_PLUS_4, input_index),
            NonterminalId(30) => self.scanner.match_any(&FOLLOW_SET_OPT_6, input_index),
            NonterminalId(31) => self.scanner.match_any(&FOLLOW_SET_STAR_2, input_index),
            NonterminalId(32) => self.scanner.match_any(&FOLLOW_SET_OPT_7, input_index),
            NonterminalId(33) => self.scanner.match_any(&FOLLOW_SET_PLUS_5, input_index),
            NonterminalId(34) => self.scanner.match_any(&FOLLOW_SET_OPT_8, input_index),
            NonterminalId(35) => self.scanner.match_any(&FOLLOW_SET_STAR_3, input_index),
            NonterminalId(36) => self.scanner.match_any(&FOLLOW_SET_PLUS_6, input_index),
            NonterminalId(37) => self.scanner.match_any(&FOLLOW_SET_OPT_9, input_index),
            NonterminalId(38) => self.scanner.match_any(&FOLLOW_SET_PLUS_7, input_index),
            NonterminalId(39) => self.scanner.match_any(&FOLLOW_SET_PLUS_8, input_index),
            NonterminalId(40) => self.scanner.match_any(&FOLLOW_SET_PLUS_9, input_index),
            NonterminalId(41) => self.scanner.match_any(&FOLLOW_SET_OPT_10, input_index),
            NonterminalId(42) => self.scanner.match_any(&FOLLOW_SET_PLUS_10, input_index),
            NonterminalId(43) => self.scanner.match_any(&FOLLOW_SET_ALT_0, input_index),
            NonterminalId(44) => self.scanner.match_any(&FOLLOW_SET_PLUS_11, input_index),
            NonterminalId(45) => self.scanner.match_any(&FOLLOW_SET_OPT_11, input_index),
            NonterminalId(46) => self.scanner.match_any(&FOLLOW_SET_STAR_4, input_index),
            NonterminalId(47) => self
                .scanner
                .match_any(&FOLLOW_SET_START_GRAMMAR, input_index),
            NonterminalId(48) => self.scanner.match_any(&FOLLOW_SET_START_RULE, input_index),
            NonterminalId(49) => self
                .scanner
                .match_any(&FOLLOW_SET_START_SYNTAX_RULE, input_index),
            NonterminalId(50) => self
                .scanner
                .match_any(&FOLLOW_SET_START_ANNOTATION, input_index),
            NonterminalId(51) => self
                .scanner
                .match_any(&FOLLOW_SET_START_REGEX_RULE, input_index),
            NonterminalId(52) => self
                .scanner
                .match_any(&FOLLOW_SET_START_REGEX_PRE_CONDITION, input_index),
            NonterminalId(53) => self
                .scanner
                .match_any(&FOLLOW_SET_START_REGEX_POST_CONDITION, input_index),
            NonterminalId(54) => self
                .scanner
                .match_any(&FOLLOW_SET_START_PRIORITY_LEVEL, input_index),
            NonterminalId(55) => self
                .scanner
                .match_any(&FOLLOW_SET_START_ASSOCIATIVITY, input_index),
            NonterminalId(56) => self
                .scanner
                .match_any(&FOLLOW_SET_START_ALTERNATIVE, input_index),
            NonterminalId(57) => self
                .scanner
                .match_any(&FOLLOW_SET_START_SYMBOL, input_index),
            NonterminalId(58) => self
                .scanner
                .match_any(&FOLLOW_SET_START_PRE_CONDITION, input_index),
            NonterminalId(59) => self
                .scanner
                .match_any(&FOLLOW_SET_START_POST_CONDITION, input_index),
            NonterminalId(60) => self.scanner.match_any(&FOLLOW_SET_START_REGEX, input_index),
            NonterminalId(61) => self
                .scanner
                .match_any(&FOLLOW_SET_START_CHAR_CLASS, input_index),
            NonterminalId(62) => self
                .scanner
                .match_any(&FOLLOW_SET_START_RANGE_ELEMENT, input_index),
            NonterminalId(63) => self.scanner.match_any(&FOLLOW_SET_START_RANGE, input_index),
            _ => true,
        }
    }
    fn follow_set(&self, nonterminal_id: NonterminalId) -> &'static TerminalSet {
        match nonterminal_id {
            NonterminalId(0) => &FOLLOW_SET_GRAMMAR,
            NonterminalId(1) => &FOLLOW_SET_RULE,
            NonterminalId(2) => &FOLLOW_SET_SYNTAX_RULE,
            NonterminalId(3) => &FOLLOW_SET_ANNOTATION,
            NonterminalId(4) => &FOLLOW_SET_REGEX_RULE,
            NonterminalId(5) => &FOLLOW_SET_REGEX_PRE_CONDITION,
            NonterminalId(6) => &FOLLOW_SET_REGEX_POST_CONDITION,
            NonterminalId(7) => &FOLLOW_SET_PRIORITY_LEVEL,
            NonterminalId(8) => &FOLLOW_SET_ASSOCIATIVITY,
            NonterminalId(9) => &FOLLOW_SET_ALTERNATIVE,
            NonterminalId(64) => &FOLLOW_SET_SYMBOL,
            NonterminalId(10) => &FOLLOW_SET_PRE_CONDITION,
            NonterminalId(11) => &FOLLOW_SET_POST_CONDITION,
            NonterminalId(12) => &FOLLOW_SET_REGEX,
            NonterminalId(13) => &FOLLOW_SET_CHAR_CLASS,
            NonterminalId(14) => &FOLLOW_SET_RANGE_ELEMENT,
            NonterminalId(15) => &FOLLOW_SET_RANGE,
            NonterminalId(16) => &FOLLOW_SET_LAYOUT,
            NonterminalId(17) => &FOLLOW_SET_PLUS_0,
            NonterminalId(18) => &FOLLOW_SET_OPT_0,
            NonterminalId(19) => &FOLLOW_SET_STAR_0,
            NonterminalId(20) => &FOLLOW_SET_OPT_1,
            NonterminalId(21) => &FOLLOW_SET_PLUS_1,
            NonterminalId(22) => &FOLLOW_SET_OPT_2,
            NonterminalId(23) => &FOLLOW_SET_STAR_1,
            NonterminalId(24) => &FOLLOW_SET_OPT_3,
            NonterminalId(25) => &FOLLOW_SET_OPT_4,
            NonterminalId(26) => &FOLLOW_SET_OPT_5,
            NonterminalId(27) => &FOLLOW_SET_PLUS_3,
            NonterminalId(28) => &FOLLOW_SET_PLUS_2,
            NonterminalId(29) => &FOLLOW_SET_PLUS_4,
            NonterminalId(30) => &FOLLOW_SET_OPT_6,
            NonterminalId(31) => &FOLLOW_SET_STAR_2,
            NonterminalId(32) => &FOLLOW_SET_OPT_7,
            NonterminalId(33) => &FOLLOW_SET_PLUS_5,
            NonterminalId(34) => &FOLLOW_SET_OPT_8,
            NonterminalId(35) => &FOLLOW_SET_STAR_3,
            NonterminalId(36) => &FOLLOW_SET_PLUS_6,
            NonterminalId(37) => &FOLLOW_SET_OPT_9,
            NonterminalId(38) => &FOLLOW_SET_PLUS_7,
            NonterminalId(39) => &FOLLOW_SET_PLUS_8,
            NonterminalId(40) => &FOLLOW_SET_PLUS_9,
            NonterminalId(41) => &FOLLOW_SET_OPT_10,
            NonterminalId(42) => &FOLLOW_SET_PLUS_10,
            NonterminalId(43) => &FOLLOW_SET_ALT_0,
            NonterminalId(44) => &FOLLOW_SET_PLUS_11,
            NonterminalId(45) => &FOLLOW_SET_OPT_11,
            NonterminalId(46) => &FOLLOW_SET_STAR_4,
            NonterminalId(47) => &FOLLOW_SET_START_GRAMMAR,
            NonterminalId(48) => &FOLLOW_SET_START_RULE,
            NonterminalId(49) => &FOLLOW_SET_START_SYNTAX_RULE,
            NonterminalId(50) => &FOLLOW_SET_START_ANNOTATION,
            NonterminalId(51) => &FOLLOW_SET_START_REGEX_RULE,
            NonterminalId(52) => &FOLLOW_SET_START_REGEX_PRE_CONDITION,
            NonterminalId(53) => &FOLLOW_SET_START_REGEX_POST_CONDITION,
            NonterminalId(54) => &FOLLOW_SET_START_PRIORITY_LEVEL,
            NonterminalId(55) => &FOLLOW_SET_START_ASSOCIATIVITY,
            NonterminalId(56) => &FOLLOW_SET_START_ALTERNATIVE,
            NonterminalId(57) => &FOLLOW_SET_START_SYMBOL,
            NonterminalId(58) => &FOLLOW_SET_START_PRE_CONDITION,
            NonterminalId(59) => &FOLLOW_SET_START_POST_CONDITION,
            NonterminalId(60) => &FOLLOW_SET_START_REGEX,
            NonterminalId(61) => &FOLLOW_SET_START_CHAR_CLASS,
            NonterminalId(62) => &FOLLOW_SET_START_RANGE_ELEMENT,
            NonterminalId(63) => &FOLLOW_SET_START_RANGE,
            _ => unreachable!("no FOLLOW set for nonterminal {nonterminal_id}"),
        }
    }
    fn failures(&self) -> impl Iterator<Item = &GLLFailure> {
        self.failures.iter()
    }
    #[inline(never)]
    fn add_failure(
        &mut self,
        input_index: u32,
        slot_id: SlotId,
        gss_node_id: Option<GssNodeId>,
        kind: GLLFailureKind,
    ) {
        if self.suppress_failures {
            return;
        }
        let level = self.failures.first().map_or(0, |e| e.input_index);
        record!(self, GLLFailure, input_index, slot_id, gss_node_id, kind);
        if input_index < level {
            return;
        }
        if input_index > level {
            self.failures.clear();
        }
        self.failures.push(
            GLLFailure {
                input_index,
                slot_id,
                gss_node_id,
                kind,
            },
            self.vec_arena,
        );
    }
    fn match_token(&mut self, terminal_id: TerminalId, input_index: u32) -> Option<u32> {
        self.scanner.match_token(terminal_id, input_index)
    }
    fn vec_arena(&self) -> &'arena Arena {
        self.vec_arena
    }
}
pub struct IggyParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: IggyScanner<'i, 'arena>,
    descriptors: ArenaVec<'arena, Descriptor>,
    gss_nodes: ArenaVec<'arena, GSSNode<'arena>>,
    // Per-nonterminal GSS-node index keyed by input position.
    gss_nodes_index: [InlineMap<'arena, u32, GssNodeId>; 65],
    // GSS index for nonterminal Symbol
    gss_nodes_index_symbol: InlineMap<'arena, (u32, i32, i32), GssNodeId>,
    sppf_nodes: ArenaVec<'arena, SPPFNode>,
    #[cfg(feature = "instrument")]
    descriptors_count: usize,
    #[cfg(feature = "instrument")]
    descriptors_peak: usize,
    #[cfg(feature = "instrument")]
    ll1_call_log: Vec<(NonterminalId, u32)>,
    // Per-slot Span-keyed intermediate-node index, for slots in non-parameterized nonterminals.
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 333],
    // Per-slot (Span, env)-keyed intermediate-node index, for slots in parameterized
    // nonterminals; env separates calls made with different parameter values.
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 87],
    terminal_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 40],
    // Epsilon nodes keyed by input position; SPPFNodeId::NONE marks an empty slot.
    epsilon_nodes: ArenaVec<'arena, SPPFNodeId>,
    // An intermediate node keeps its first child inline. Children of intermediate nodes are
    // pairs: (left_child, right_child). Extra children, when there is ambiguity, are stored here
    // as (parent node, (left child, right child)).
    intermediate_nodes_children: ArenaVec<'arena, (SPPFNodeId, (SPPFNodeId, SPPFNodeId))>,
    // intermediate_nodes_children grouped by parent node, built lazily for tree construction.
    intermediate_nodes_children_map: OnceCell<FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SPPFNodeId)>>>,
    // Extra children of ambiguous nonterminal nodes, the counterpart to
    // intermediate_nodes_children: each entry is (parent node, (child, return slot)), a single
    // child plus its return slot rather than a pair.
    nonterminal_nodes_children: ArenaVec<'arena, (SPPFNodeId, (SPPFNodeId, SlotId))>,
    // nonterminal_nodes_children grouped by parent node, built lazily like
    // intermediate_nodes_children_map.
    nonterminal_nodes_children_map: OnceCell<FxHashMap<SPPFNodeId, Vec<(SPPFNodeId, SlotId)>>>,
    envs: ArenaVec<'arena, Env<'arena>>,
    failures: InlineVec<'arena, GLLFailure, 8>,
    // When true, `add_failure` is a no-op. The one user is the layout match of a `!>>>`
    // restriction: that parse is speculative, so its failure must not become the reported error.
    suppress_failures: bool,
    layout_memo: ArenaVec<'arena, Option<SPPFNodeId>>,
    #[cfg(feature = "debug-trace")]
    pub trace_events: Option<Vec<TraceEvent>>,
}
impl<'i, 'arena> IggyParser<'i, 'arena> {
    /// Creates a new parser.
    /// A reference to `input` is stored in the parser for scanning.
    /// `parser_arena` is where the parser's internal objects (GSS, SPPF nodes,
    /// etc.) are allocated.
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        init_logger();
        let vec_arena = parser_arena;
        Self {
            vec_arena,
            scanner: IggyScanner::new(input, vec_arena),
            gss_nodes_index: [const { InlineMap::Empty }; 65],
            gss_nodes_index_symbol: InlineMap::Empty,
            descriptors: vec_arena.vec_with_capacity(
                input.len() as usize / DESCRIPTORS_CAPACITY_DIVISOR + DESCRIPTORS_CAPACITY_FLOOR,
            ),
            gss_nodes: vec_arena.vec_with_capacity(input.len() as usize * GSS_CAPACITY_MULTIPLIER),
            sppf_nodes: vec_arena
                .vec_with_capacity(input.len() as usize * SPPF_CAPACITY_MULTIPLIER),
            intermediate_nodes_index: [const { InlineMap::Empty }; 333],
            dd_intermediate_nodes_index: [const { InlineMap::Empty }; 87],
            terminal_nodes_index: [const { InlineMap::Empty }; 40],
            epsilon_nodes: {
                let mut v = vec_arena.vec_with_capacity(input.len() as usize + 1);
                v.resize(input.len() as usize + 1, SPPFNodeId::NONE);
                v
            },
            #[cfg(feature = "instrument")]
            descriptors_count: 0,
            #[cfg(feature = "instrument")]
            descriptors_peak: 0,
            #[cfg(feature = "instrument")]
            ll1_call_log: vec![],
            intermediate_nodes_children: vec_arena.vec(),
            intermediate_nodes_children_map: OnceCell::new(),
            nonterminal_nodes_children: vec_arena.vec(),
            nonterminal_nodes_children_map: OnceCell::new(),
            envs: vec_arena.vec_with_capacity(input.len() as usize * ENVS_CAPACITY_MULTIPLIER),
            failures: InlineVec::Empty,
            suppress_failures: false,
            layout_memo: {
                let mut v = vec_arena.vec_with_capacity(input.len() as usize + 1);
                v.resize(input.len() as usize + 1, None);
                v
            },
            #[cfg(feature = "debug-trace")]
            trace_events: None,
        }
    }
    /// Parses the input from `start` and builds the parse tree in the given
    /// `tree_arena`. The parser is consumed. The tree lives as long as the
    /// arena lives. `start` is normally the start wrapper of a declared
    /// nonterminal, `grammar::START_<NAME>`, which allows layout around the
    /// input; a bare nonterminal id parses without it.
    pub fn parse<'a>(
        self,
        start: NonterminalId,
        tree_arena: &'a Arena,
    ) -> Result<ParseSuccess<ParseTree<'a>>, ParseError> {
        Parser::parse(self, start, tree_arena)
    }
    pub fn parse_grammar(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartGrammar<'_>>, ParseError> {
        Ok(self
            .parse(START_GRAMMAR, tree_arena)?
            .map(ParseTree::unwrap_start_grammar))
    }
    pub fn parse_rule(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRule<'_>>, ParseError> {
        Ok(self
            .parse(START_RULE, tree_arena)?
            .map(ParseTree::unwrap_start_rule))
    }
    pub fn parse_syntax_rule(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartSyntaxRule<'_>>, ParseError> {
        Ok(self
            .parse(START_SYNTAX_RULE, tree_arena)?
            .map(ParseTree::unwrap_start_syntax_rule))
    }
    pub fn parse_annotation(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartAnnotation<'_>>, ParseError> {
        Ok(self
            .parse(START_ANNOTATION, tree_arena)?
            .map(ParseTree::unwrap_start_annotation))
    }
    pub fn parse_regex_rule(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRegexRule<'_>>, ParseError> {
        Ok(self
            .parse(START_REGEX_RULE, tree_arena)?
            .map(ParseTree::unwrap_start_regex_rule))
    }
    pub fn parse_regex_pre_condition(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRegexPreCondition<'_>>, ParseError> {
        Ok(self
            .parse(START_REGEX_PRE_CONDITION, tree_arena)?
            .map(ParseTree::unwrap_start_regex_pre_condition))
    }
    pub fn parse_regex_post_condition(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRegexPostCondition<'_>>, ParseError> {
        Ok(self
            .parse(START_REGEX_POST_CONDITION, tree_arena)?
            .map(ParseTree::unwrap_start_regex_post_condition))
    }
    pub fn parse_priority_level(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartPriorityLevel<'_>>, ParseError> {
        Ok(self
            .parse(START_PRIORITY_LEVEL, tree_arena)?
            .map(ParseTree::unwrap_start_priority_level))
    }
    pub fn parse_associativity(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartAssociativity<'_>>, ParseError> {
        Ok(self
            .parse(START_ASSOCIATIVITY, tree_arena)?
            .map(ParseTree::unwrap_start_associativity))
    }
    pub fn parse_alternative(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartAlternative<'_>>, ParseError> {
        Ok(self
            .parse(START_ALTERNATIVE, tree_arena)?
            .map(ParseTree::unwrap_start_alternative))
    }
    pub fn parse_symbol(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartSymbol<'_>>, ParseError> {
        Ok(self
            .parse(START_SYMBOL, tree_arena)?
            .map(ParseTree::unwrap_start_symbol))
    }
    pub fn parse_pre_condition(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartPreCondition<'_>>, ParseError> {
        Ok(self
            .parse(START_PRE_CONDITION, tree_arena)?
            .map(ParseTree::unwrap_start_pre_condition))
    }
    pub fn parse_post_condition(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartPostCondition<'_>>, ParseError> {
        Ok(self
            .parse(START_POST_CONDITION, tree_arena)?
            .map(ParseTree::unwrap_start_post_condition))
    }
    pub fn parse_regex(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRegex<'_>>, ParseError> {
        Ok(self
            .parse(START_REGEX, tree_arena)?
            .map(ParseTree::unwrap_start_regex))
    }
    pub fn parse_char_class(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartCharClass<'_>>, ParseError> {
        Ok(self
            .parse(START_CHAR_CLASS, tree_arena)?
            .map(ParseTree::unwrap_start_char_class))
    }
    pub fn parse_range_element(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRangeElement<'_>>, ParseError> {
        Ok(self
            .parse(START_RANGE_ELEMENT, tree_arena)?
            .map(ParseTree::unwrap_start_range_element))
    }
    pub fn parse_range(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartRange<'_>>, ParseError> {
        Ok(self
            .parse(START_RANGE, tree_arena)?
            .map(ParseTree::unwrap_start_range))
    }
    pub fn parse_layout(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::Layout<'_>>, ParseError> {
        Ok(self
            .parse(LAYOUT, tree_arena)?
            .map(ParseTree::unwrap_layout))
    }
    #[allow(clippy::too_many_arguments)]
    fn create_symbol(
        &mut self,
        sppf_node_id: Option<SPPFNodeId>,
        gss_node_id: GssNodeId,
        return_slot: SlotId,
        env: Option<EnvId>,
        p: i32,
        e: i32,
    ) {
        record!(self, Call, sppf_node_id, gss_node_id, return_slot);
        let left_child = sppf_node_id.map(|id| {
            let node = self.sppf_node(id);
            (id, node.left_extent())
        });
        let gss_node = self.gss_node(gss_node_id);
        let i = match left_child {
            Some((id, _)) => self.sppf_node(id).right_extent(),
            None => gss_node.index,
        };
        // If there is already a GSS node for this call, add an edge.
        if let Some(existing_gss_node_id) = self.get_gss_node_symbol(i, p, e) {
            record!(self, GSSNodeFound, NonterminalId(64), i);
            self.add_edge_to_existing_gss_node(
                existing_gss_node_id,
                gss_node_id,
                left_child,
                return_slot,
                env,
            );
        } else {
            record!(self, GSSNodeNotFound, NonterminalId(64), i);
            let new_gss_node_id = self.new_gss_node(NonterminalId(64), i);
            self.add_gss_edge(new_gss_node_id, gss_node_id, sppf_node_id, return_slot, env);
            let arena = self.vec_arena;
            let (env_id, env) = self.new_env();
            env.bind(BINDING_P, p, arena);
            env.bind(BINDING_E, e, arena);
            self.add_first_descriptors(NonterminalId(64), i, new_gss_node_id, Some(env_id));
            self.add_gss_node_symbol(i, p, e, new_gss_node_id);
        }
    }
    fn parse_annotation_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(3), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self.scanner.longest_match(&FIRST_SET_ANNOTATION, i)?;
            match matched {
                TerminalId(14) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(14), start, SlotId(19), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(3),
                        return_slot: SlotId(19),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                TerminalId(15) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(15), start, SlotId(21), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(3),
                        return_slot: SlotId(21),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_ANNOTATION),
                );
            }
        }
        result
    }
    fn parse_regex_pre_condition_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(5), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self
                .scanner
                .longest_match(&FIRST_SET_REGEX_PRE_CONDITION, i)?;
            match matched {
                TerminalId(2) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(39), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let mut current = right_child;
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(40), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(40),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(19), start, SlotId(41), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(41),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(5),
                        return_slot: SlotId(41),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_REGEX_PRE_CONDITION),
                );
            }
        }
        result
    }
    fn parse_regex_post_condition_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(6), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self
                .scanner
                .longest_match(&FIRST_SET_REGEX_POST_CONDITION, i)?;
            match matched {
                TerminalId(20) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(20), start, SlotId(43), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let mut current = right_child;
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(44), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(44),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(45), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(45),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(6),
                        return_slot: SlotId(45),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                TerminalId(21) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(21), start, SlotId(47), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let mut current = right_child;
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(48), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(48),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(49), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(49),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(6),
                        return_slot: SlotId(49),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_REGEX_POST_CONDITION),
                );
            }
        }
        result
    }
    fn parse_associativity_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(8), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self.scanner.longest_match(&FIRST_SET_ASSOCIATIVITY, i)?;
            match matched {
                TerminalId(22) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(22), start, SlotId(55), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(8),
                        return_slot: SlotId(55),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                TerminalId(23) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(23), start, SlotId(57), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(8),
                        return_slot: SlotId(57),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                TerminalId(24) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(24), start, SlotId(59), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(8),
                        return_slot: SlotId(59),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_ASSOCIATIVITY),
                );
            }
        }
        result
    }
    fn parse_pre_condition_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(10), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self.scanner.longest_match(&FIRST_SET_PRE_CONDITION, i)?;
            match matched {
                TerminalId(2) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(71), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let mut current = right_child;
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(72), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(72),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(19), start, SlotId(73), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(73),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(10),
                        return_slot: SlotId(73),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_PRE_CONDITION),
                );
            }
        }
        result
    }
    fn parse_range_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(15), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self.scanner.longest_match(&FIRST_SET_RANGE, i)?;
            match matched {
                TerminalId(6) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(6), start, SlotId(129), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let mut current = right_child;
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(130), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(130),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(37), start, SlotId(131), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(131),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let node = self.parse_layout_ll1(start, Some((SlotId(132), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(132),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(6), start, SlotId(133), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                        SlotId(133),
                        left_extent,
                        j,
                        current,
                        right_child,
                    );
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(15),
                        return_slot: SlotId(133),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_RANGE),
                );
            }
        }
        result
    }
    fn parse_layout_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(16), i));
        if let Some(memo) = self.layout_memo[i as usize] {
            return Some(memo);
        }
        let result = (|| -> Option<SPPFNodeId> {
            let mut j = i;
            let right_child = {
                let start = j;
                let node = self.parse_star_4_ll1(start, Some((SlotId(135), None)))?;
                let end = self.sppf_node(node).right_extent();
                j = end;
                if let Some(failure) = self.post_conditions(SlotId(135), start, end) {
                    self.add_failure(end, SlotId(135), None, failure);
                    return None;
                }
                node
            };
            let left_extent = self.sppf_node(right_child).left_extent();
            let current = right_child;
            Some(self.add_nonterminal_node(NonterminalNode {
                nonterminal_id: NonterminalId(16),
                return_slot: SlotId(135),
                span: Span {
                    left_extent,
                    right_extent: j,
                },
                child: current,
                ambiguous: false,
            }))
        })();
        if let Some(node) = result {
            self.layout_memo[i as usize] = Some(node);
        }
        result
    }
    fn parse_opt_1_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(20), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_1, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(20),
                    return_slot: SlotId(149),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(15) | TerminalId(14) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let node = self.parse_annotation_ll1(start, Some((SlotId(148), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(20),
                    return_slot: SlotId(148),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_opt_3_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(24), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_3, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(24),
                    return_slot: SlotId(165),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(15) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let (end, node) =
                        self.match_terminal(TerminalId(15), start, SlotId(164), None)?;
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(24),
                    return_slot: SlotId(164),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_opt_4_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(25), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_4, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(25),
                    return_slot: SlotId(168),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(16) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let (end, node) =
                        self.match_terminal(TerminalId(16), start, SlotId(167), None)?;
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(25),
                    return_slot: SlotId(167),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_plus_4_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(29), i));
        let result = (|| -> Option<SPPFNodeId> {
            let mut j = i;
            let (body_node, body_end) =
                (self.parse_regex_post_condition_ll1(j, None).map(|node| {
                    let end = self.sppf_node(node).right_extent();
                    (node, end)
                }))?;
            j = body_end;
            let left_extent = i;
            let mut current = self.add_nonterminal_node(NonterminalNode {
                nonterminal_id: NonterminalId(29),
                return_slot: SlotId(191),
                span: Span {
                    left_extent,
                    right_extent: j,
                },
                child: body_node,
                ambiguous: false,
            });
            #[allow(clippy::while_let_loop)]
            loop {
                let Some((node_0, pos_0)) = self.parse_layout_ll1(j, None).map(|node| {
                    let end = self.sppf_node(node).right_extent();
                    (node, end)
                }) else {
                    break;
                };
                let Some((node_1, pos_1)) =
                    self.parse_regex_post_condition_ll1(pos_0, None)
                        .map(|node| {
                            let end = self.sppf_node(node).right_extent();
                            (node, end)
                        })
                else {
                    break;
                };
                j = pos_1;
                current = self.create_intermediate_node_ll1(
                    SlotId(188),
                    left_extent,
                    pos_0,
                    current,
                    node_0,
                );
                current = self.create_intermediate_node_ll1(
                    SlotId(189),
                    left_extent,
                    pos_1,
                    current,
                    node_1,
                );
                current = self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(29),
                    return_slot: SlotId(189),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                });
            }
            Some(current)
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_4),
                );
            }
        }
        result
    }
    fn parse_opt_6_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(30), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_6, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(30),
                    return_slot: SlotId(194),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(21) | TerminalId(20) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let node = self.parse_plus_4_ll1(start, Some((SlotId(193), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(30),
                    return_slot: SlotId(193),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_star_2_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(31), i));
        let mut j = i;
        let right_child = {
            let start = j;
            let node = self.parse_opt_6_ll1(start, Some((SlotId(196), None)))?;
            let end = self.sppf_node(node).right_extent();
            j = end;
            node
        };
        let left_extent = self.sppf_node(right_child).left_extent();
        let current = right_child;
        Some(self.add_nonterminal_node(NonterminalNode {
            nonterminal_id: NonterminalId(31),
            return_slot: SlotId(196),
            span: Span {
                left_extent,
                right_extent: j,
            },
            child: current,
            ambiguous: false,
        }))
    }
    fn parse_opt_9_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(37), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_9, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(37),
                    return_slot: SlotId(221),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(7) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let (end, node) =
                        self.match_terminal(TerminalId(7), start, SlotId(220), None)?;
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(37),
                    return_slot: SlotId(220),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_opt_10_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(41), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_10, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(41),
                    return_slot: SlotId(244),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(34) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let (end, node) =
                        self.match_terminal(TerminalId(34), start, SlotId(243), None)?;
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(41),
                    return_slot: SlotId(243),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_alt_0_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(43), i));
        let result = (|| -> Option<SPPFNodeId> {
            let matched = self.scanner.longest_match(&FIRST_SET_ALT_0, i)?;
            match matched {
                TerminalId(8) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(8), start, SlotId(252), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(43),
                        return_slot: SlotId(252),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                TerminalId(10) => {
                    let mut j = i;
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(10), start, SlotId(254), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
                    let current = right_child;
                    Some(self.add_nonterminal_node(NonterminalNode {
                        nonterminal_id: NonterminalId(43),
                        return_slot: SlotId(254),
                        span: Span {
                            left_extent,
                            right_extent: j,
                        },
                        child: current,
                        ambiguous: false,
                    }))
                }
                _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
            }
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_ALT_0),
                );
            }
        }
        result
    }
    fn parse_plus_11_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(44), i));
        let result = (|| -> Option<SPPFNodeId> {
            let mut j = i;
            let (body_node, body_end) = (self.parse_alt_0_ll1(j, None).map(|node| {
                let end = self.sppf_node(node).right_extent();
                (node, end)
            }))?;
            j = body_end;
            let left_extent = i;
            let mut current = self.add_nonterminal_node(NonterminalNode {
                nonterminal_id: NonterminalId(44),
                return_slot: SlotId(259),
                span: Span {
                    left_extent,
                    right_extent: j,
                },
                child: body_node,
                ambiguous: false,
            });
            #[allow(clippy::while_let_loop)]
            loop {
                let Some((node_0, pos_0)) = self.parse_alt_0_ll1(j, None).map(|node| {
                    let end = self.sppf_node(node).right_extent();
                    (node, end)
                }) else {
                    break;
                };
                j = pos_0;
                current = self.create_intermediate_node_ll1(
                    SlotId(257),
                    left_extent,
                    pos_0,
                    current,
                    node_0,
                );
                current = self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(44),
                    return_slot: SlotId(257),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                });
            }
            Some(current)
        })();
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    i,
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_PLUS_11),
                );
            }
        }
        result
    }
    fn parse_opt_11_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(45), i));
        let Some(matched) = self.scanner.longest_match(&FIRST_SET_OPT_11, i) else {
            return {
                let epsilon_node_id = self.get_or_create_epsilon_node(i);
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(45),
                    return_slot: SlotId(262),
                    span: Span {
                        left_extent: i,
                        right_extent: i,
                    },
                    child: epsilon_node_id,
                    ambiguous: false,
                }))
            };
        };
        match matched {
            TerminalId(10) | TerminalId(8) => {
                let mut j = i;
                let right_child = {
                    let start = j;
                    let node = self.parse_plus_11_ll1(start, Some((SlotId(261), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
                let left_extent = self.sppf_node(right_child).left_extent();
                let current = right_child;
                Some(self.add_nonterminal_node(NonterminalNode {
                    nonterminal_id: NonterminalId(45),
                    return_slot: SlotId(261),
                    span: Span {
                        left_extent,
                        right_extent: j,
                    },
                    child: current,
                    ambiguous: false,
                }))
            }
            _ => unreachable!("LL(1) dispatch covers every terminal in FIRST_SET"),
        }
    }
    fn parse_star_4_ll1(
        &mut self,
        i: u32,
        _failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(46), i));
        let mut j = i;
        let right_child = {
            let start = j;
            let node = self.parse_opt_11_ll1(start, Some((SlotId(264), None)))?;
            let end = self.sppf_node(node).right_extent();
            j = end;
            node
        };
        let left_extent = self.sppf_node(right_child).left_extent();
        let current = right_child;
        Some(self.add_nonterminal_node(NonterminalNode {
            nonterminal_id: NonterminalId(46),
            return_slot: SlotId(264),
            span: Span {
                left_extent,
                right_extent: j,
            },
            child: current,
            ambiguous: false,
        }))
    }
    fn get_gss_node_symbol(&self, input_index: u32, p: i32, e: i32) -> Option<GssNodeId> {
        self.gss_nodes_index_symbol
            .get(&(input_index, p, e))
            .copied()
    }
    fn add_gss_node_symbol(&mut self, input_index: u32, p: i32, e: i32, gss_node_id: GssNodeId) {
        let arena = self.vec_arena;
        self.gss_nodes_index_symbol
            .insert((input_index, p, e), gss_node_id, arena);
    }
    fn get_or_create_epsilon_node(&mut self, i: u32) -> SPPFNodeId {
        let existing = self.epsilon_nodes[i as usize];
        if existing != SPPFNodeId::NONE {
            record!(self, TerminalNodeFound, existing);
            return existing;
        }
        let span = Span::new(i, i);
        let terminal_id = TerminalId(38);
        let node_id = SPPFNodeId(self.sppf_nodes.len() as u32);
        record!(self, TerminalNodeCreated, terminal_id, span);
        self.sppf_nodes
            .push(SPPFNode::Terminal(TerminalNode { terminal_id, span }));
        self.epsilon_nodes[i as usize] = node_id;
        node_id
    }
}
