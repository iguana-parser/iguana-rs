...
use parse_tree::DeepPriorityFullParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    let mut parser = DeepPriorityFullParser::new(input, grammar_data::S);
    match parser.run() {
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<DeepPriorityFullParser>(input, &error))
        }
    }
}
