...
use crate::{
    grammar::*,
    parse_tree::{self, DeepPriorityFullParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for DeepPriorityFullParser<'i, 'arena> {
    type Grammar = DeepPriorityFullGrammar;
    type ConcreteParser<'input, 'parser_arena> = DeepPriorityFullParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        DeepPriorityFullParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = DeepPriorityFullParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // (r_pr == 0) ? 5 : min(r_pr, 5)
            SlotId(16) => {
                self.create_e(result, gss_node_id, SlotId(17), env, 6);
            }
            // E(p: i32) : [5 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 5)] WS "*" WS r_pr=E(6) . return
            // (r_pr == 0) ? 5 : min(r_pr, 5)
            SlotId(17) => {
            ...
            // (r_pr == 0) ? 4 : min(r_pr, 4)
            SlotId(25) => {
                self.create_e(result, gss_node_id, SlotId(26), env, 5);
            }
            // E(p: i32) : [4 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 4)] WS "+" WS r_pr=E(5) . return
            // (r_pr == 0) ? 4 : min(r_pr, 4)
            SlotId(26) => {
            ...
            SlotId(28) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(28), Some(gss_node_id))
                {
                    // E(p: i32) : "-" . WS r_pr=E(3) return (r_pr == 0) ? 3 : min(r_pr, 3)
            ...
            // E(p: i32) : "-" WS . r_pr=E(3) return (r_pr == 0) ? 3 : min(r_pr, 3)
            SlotId(30) => {
                self.create_e(result, gss_node_id, SlotId(31), env, 3);
            }
            // E(p: i32) : "-" WS r_pr=E(3) . return (r_pr == 0) ? 3 : min(r_pr, 3)
            SlotId(31) => {
            ...
            SlotId(33) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(4), input_index, SlotId(33), Some(gss_node_id))
                {
                    // E(p: i32) : "if" . WS E(0) WS "then" WS E(0) WS "else" WS E(2) return 2
            ...
            // E(p: i32) : "if" WS . E(0) WS "then" WS E(0) WS "else" WS E(2) return 2
            SlotId(35) => {
                self.create_e(result, gss_node_id, SlotId(36), env, 0);
            }
            // E(p: i32) : "if" WS E(0) . WS "then" WS E(0) WS "else" WS E(2) return 2
            SlotId(36) => {
                if let Some((_, right_child)) =
            ...
            SlotId(37) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(5), input_index, SlotId(37), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            // E(p: i32) : "if" WS E(0) WS "then" WS . E(0) WS "else" WS E(2) return 2
            SlotId(39) => {
                self.create_e(result, gss_node_id, SlotId(40), env, 0);
            }
            // E(p: i32) : "if" WS E(0) WS "then" WS E(0) . WS "else" WS E(2) return 2
            SlotId(40) => {
                if let Some((_, right_child)) =
            ...
            SlotId(41) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(6), input_index, SlotId(41), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(42) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(0), input_index, SlotId(42), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            // E(p: i32) : "if" WS E(0) WS "then" WS E(0) WS "else" WS . E(2) return 2
            SlotId(43) => {
                self.create_e(result, gss_node_id, SlotId(44), env, 2);
            }
            // E(p: i32) : "if" WS E(0) WS "then" WS E(0) WS "else" WS E(2) . return 2
            SlotId(44) => {
            ...
            SlotId(50) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(7), input_index, SlotId(50), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] WS ";" WS . E(1) return 1
            SlotId(52) => {
                self.create_e(result, gss_node_id, SlotId(53), env, 1);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] WS ";" WS E(1) . return 1
            SlotId(53) => {
                self.execute(input_index, SlotId(54), result, gss_node_id, env);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] WS ";" WS E(1) return 1.
            SlotId(54) => {
                let Some(result) = result else {
            ...
            SlotId(55) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(8), input_index, SlotId(55), Some(gss_node_id))
                {
                    // E(p: i32) : "a" . return 0
    ...
    fn bind_return_value(
        ...
        let bindings: &[(BindingId, i32)] = match return_slot {
            ...
            SlotId(17) => &[(BINDING_R_PR, value)],
            SlotId(21) => &[(BINDING_L_PR, value)],
            SlotId(26) => &[(BINDING_R_PR, value)],
            SlotId(31) => &[(BINDING_R_PR, value)],
            SlotId(48) => &[(BINDING_L_PR, value)],
            _ => return env,
        };
        let arena = self.vec_arena;
...
pub struct DeepPriorityFullParser<'i, 'arena> {
    ...
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 48],
    terminal_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 11],
...
impl<'i, 'arena> DeepPriorityFullParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: DeepPriorityFullScanner::new(input, vec_arena),
            ...
            dd_intermediate_nodes_index: [const { InlineMap::Empty }; 48],
