...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the AmbiguousExprUnsafe parser in REPL mode"]
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbiguousExprUnsafeParseTreeBuilder::new(&ctx);
                let mut parser = AmbiguousExprUnsafeParser::new(&input, start_nonterminal_id);
