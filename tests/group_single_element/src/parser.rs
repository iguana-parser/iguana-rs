...
use crate::{
    grammar::*,
    parse_tree::{self, GroupSingleElementParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::GroupSingleElementScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for GroupSingleElementParser<'i, 'arena> {
    type Grammar = GroupSingleElementGrammar;
    type ConcreteParser<'input, 'parser_arena> = GroupSingleElementParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        GroupSingleElementParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = GroupSingleElementParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(5) => {
                        if let Some(right_child) =
                            self.parse_num_ll1(input_index, Some((SlotId(5), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(7) => {
                        if let Some(right_child) =
                            self.parse_d_ll1(input_index, Some((SlotId(7), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartD : start:D.
                            input_index = j;
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_num_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartNum : start:Num.
                            input_index = j;
...
impl<'i, 'arena> GroupSingleElementParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: GroupSingleElementScanner::new(input, vec_arena),
            ...
            match matched {
                TerminalId(1) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(1), start, SlotId(3), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_num_ll1(start, Some((SlotId(6), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
