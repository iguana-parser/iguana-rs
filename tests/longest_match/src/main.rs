...
use longest_match::{
    ...
    parse_tree::{LongestMatchParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the LongestMatch grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the LongestMatch parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = LongestMatchParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = LongestMatchParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser =
                        LongestMatchParser::new(&input, start_nonterminal_id, &vec_arena);
