...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, StarWithSepParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for StarWithSepParser<'i, 'arena> {
    type Grammar = StarWithSepGrammar;
    type ConcreteParser<'input, 'parser_arena> = StarWithSepParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        StarWithSepParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = StarWithSepParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(0) => {
                        if let Some(right_child) =
                            self.parse_star_0_ll1(input_index, Some((SlotId(0), Some(gss_node_id))))
                        {
                    ...
                    SlotId(13) => {
                        if let Some(right_child) =
                            self.parse_opt_0_ll1(input_index, Some((SlotId(13), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Star_0 : Opt_0.
                            input_index = j;
...
impl<'i, 'arena> StarWithSepParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: StarWithSepScanner::new(input, vec_arena),
    ...
    fn parse_s_ll1(
        ...
        let right_child = {
            let start = j;
            let node = self.parse_star_0_ll1(start, Some((SlotId(1), None)))?;
    ...
    fn parse_plus_0_ll1(
        ...
        let result = (|| -> Option<SPPFNodeId> {
            let mut j = i;
            let (body_node, body_end) = (self.parse_a_ll1(j, None).map(|node| {
                let end = self.sppf_node(node).right_extent();
                (node, end)
            }))?;
            j = body_end;
            ...
            loop {
                ...
                let Some((node_1, pos_1)) = self.parse_a_ll1(pos_0, None).map(|node| {
                    let end = self.sppf_node(node).right_extent();
                    (node, end)
                }) else {
                    break;
    ...
    fn parse_opt_0_ll1(
        ...
        match matched {
            TerminalId(1) => {
                ...
                let right_child = {
                    let start = j;
                    let node = self.parse_plus_0_ll1(start, Some((SlotId(11), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
    ...
    fn parse_star_0_ll1(
        ...
        let right_child = {
            let start = j;
            let node = self.parse_opt_0_ll1(start, Some((SlotId(14), None)))?;
            let end = self.sppf_node(node).right_extent();
            j = end;
            node
        };
