...
impl<'i> Parser<'i> for StarWithSepParser<'i> {
    ...
    fn add_first_descriptors(
        ...
        match nonterminal_id {
            ...
            // S_Opt_0
            NonterminalId(3) => {
                self.try_alternatives(ALTERNATIVES_S_OPT_0, input_index, gss_node_id, env);
            }
            // S_Star_0 : . S_Opt_0
            NonterminalId(4) => {
...
impl<'i> StarWithSepParser<'i> {
    ...
    fn parse_s_opt_0_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        let matched = self.scanner.longest_match(FIRST_SET_S_OPT_0, i)?;
        match matched {
            TerminalId(1) => {
                let mut j = i;
                let right_child = {
    ...
    fn parse_s_star_0_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        let matched = self.scanner.longest_match(FIRST_SET_S_STAR_0, i)?;
        match matched {
            TerminalId(1) | TerminalId(3) => {
                let mut j = i;
                let right_child = {
