...
use star_with_sep::{StarWithSepGrammar, StarWithSepParser};
use std::{
    fs::{self, File},
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in StarWithSepGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: StarWithSepGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: StarWithSepGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: StarWithSepGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = StarWithSepParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(tree, StarWithSepGrammar::LAYOUT_NAME, display_options)
                    }
            ...
            match parser.run(start_nonterminal_id) {
                GLLResult::Success(success) => {
                    ...
                    cli::ReplOutcome::Parsed {
                        tree: to_sexpr_with(tree, StarWithSepGrammar::LAYOUT_NAME, display_options),
                        ambiguous,
                    }
                }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, StarWithSepGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
                ...
                && args.trace.is_none()
            {
                if let Some(ref parse_tree) = parse_tree_opt {
                    ...
                    println!(
                        ...
                        to_sexpr_with(
                            *parse_tree,
                            StarWithSepGrammar::LAYOUT_NAME,
                            display_options
                        )
                    );
                }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    StarWithSepGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
