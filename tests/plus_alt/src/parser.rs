...
impl<'i, 'arena> Parser<'i, 'arena> for PlusAltParser<'i, 'arena> {
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PlusAltParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // Alt_0 : . Num
            SlotId(8) => {
                if let Some(right_child) = self.parse_num_ll1(input_index) {
                    let j = self.sppf_node(right_child).right_extent();
                    // Alt_0 : Num.
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(2) => self.scanner.match_any(&FOLLOW_SET_NUM, input_index),
            ...
            NonterminalId(7) => self.scanner.match_any(&FOLLOW_SET_START_NUM, input_index),
            _ => true,
        }
    }
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(2) => FOLLOW_SET_NUM.terminals.to_vec(),
            ...
            NonterminalId(7) => FOLLOW_SET_START_NUM.terminals.to_vec(),
            _ => vec![],
        }
    }
...
impl<'i, 'arena> PlusAltParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PlusAltScanner::new(input, vec_arena),
    ...
    pub fn parse_num(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartNum<'_>>, ParseError> {
        Ok(self
            .parse(START_NUM, tree_arena)?
            .map(ParseTree::unwrap_start_num))
    }
    ...
    fn parse_num_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(2), i));
        let matched = self.scanner.longest_match(&FIRST_SET_NUM, i)?;
        match matched {
            TerminalId(1) => {
                let mut j = i;
                let right_child = {
