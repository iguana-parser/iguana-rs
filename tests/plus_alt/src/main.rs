...
use plus_alt::{
    ...
    parse_tree::{PlusAltParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the PlusAlt grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the PlusAlt parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = PlusAltParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = PlusAltParser::new(&input, start_nonterminal_id, &vec_arena);
