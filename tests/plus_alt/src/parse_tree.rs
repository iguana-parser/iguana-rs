...
use crate::{grammar, parser::PlusAltParser};
use iguana_runtime::{
    arena::Arena,
...
pub type StartNum<'a> = Start<&'a Num<'a>, ()>;
#[derive(Debug, Clone, Copy)]
...
impl<'a> ParseTree<'a> {
    ...
    pub(crate) fn unwrap_num(self) -> &'a Num<'a> {
        match self {
    ...
    pub(crate) fn unwrap_start_num(self) -> &'a Start<&'a Num<'a>, ()> {
        match self {
    ...
    fn new_ambiguity_node(
        ...
        match parent {
            ...
            grammar::NUM => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_num()));
            ...
            grammar::START_NUM => {
                ...
                let inner = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_start_num().node));
...
pub fn create_parse_tree<'a>(
    ...
    match root.nonterminal_id {
        ...
        grammar::START_NUM => ParseTree::StartNum(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_num(),
        ),
