...
use crate::{grammar, parser::LabeledFollowRestrictionParser};
use iguana_runtime::{
    arena::Arena,
...
#[derive(Debug)]
pub enum S<'a> {
    Alt0 { x: Token, tail: Token, span: Span },
    Amb(&'a [&'a S<'a>]),
}
impl<'a> S<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 { x, tail, .. } => match index {
                0 => Some(ParseTree::Token(*x)),
...
pub struct LabeledFollowRestrictionParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> LabeledFollowRestrictionParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for LabeledFollowRestrictionParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = x:Name Tail
                SlotId(2) => {
                    let [x, tail] = children.into_array::<2usize>();
...
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &LabeledFollowRestrictionParser,
    builder: &LabeledFollowRestrictionParseTreeBuilder<'a>,
) -> ParseTree<'a> {
