...
use crate::{
    grammar::*,
    parse_tree::{self, ExceptLongestMatchParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExceptLongestMatchScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExceptLongestMatchParser<'i, 'arena> {
    type Grammar = ExceptLongestMatchGrammar;
    type ConcreteParser<'input, 'parser_arena> = ExceptLongestMatchParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExceptLongestMatchParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExceptLongestMatchParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> ExceptLongestMatchParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExceptLongestMatchScanner::new(input, vec_arena),
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(0), start, SlotId(2), None)?;
                        j = end;
                        node
                    };
