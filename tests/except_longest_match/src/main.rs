...
use except_longest_match::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{ExceptLongestMatchParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::ExceptLongestMatchParser,
};
...
#[derive(ClapParser)]
#[command(about = "Parser for the ExceptLongestMatch grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the ExceptLongestMatch parser in REPL mode"]
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = ExceptLongestMatchParseTreeBuilder::new(&ctx);
                let mut parser = ExceptLongestMatchParser::new(&input, start_nonterminal_id);
