...
use simple_alt::{
    ...
    parser::{NONTERMINALS, SLOTS, SimpleAltParser, TERMINALS},
    types::Nonterminal,
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = SimpleAltParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = SimpleAltParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = SimpleAltParseTreeBuilder;
