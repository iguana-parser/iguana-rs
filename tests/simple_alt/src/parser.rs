...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, SimpleAltParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for SimpleAltParser<'i, 'arena> {
    type Grammar = SimpleAltGrammar;
    type ConcreteParser<'input, 'parser_arena> = SimpleAltParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        SimpleAltParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = SimpleAltParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(0) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(0), Some(gss_node_id))))
                        {
                    ...
                    SlotId(1) => {
                        if let Some(right_child) =
                            self.parse_alt_0_ll1(input_index, Some((SlotId(1), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(11) => {
                        if let Some(right_child) =
                            self.parse_d_ll1(input_index, Some((SlotId(11), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Alt_0 : D.
                            input_index = j;
                    ...
                    SlotId(13) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(13), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartA : start:A.
                            input_index = j;
                    ...
                    SlotId(15) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(15), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartB : start:B.
                            input_index = j;
                    ...
                    SlotId(17) => {
                        if let Some(right_child) =
                            self.parse_c_ll1(input_index, Some((SlotId(17), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartC : start:C.
                            input_index = j;
                    ...
                    SlotId(19) => {
                        if let Some(right_child) =
                            self.parse_d_ll1(input_index, Some((SlotId(19), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartD : start:D.
                            input_index = j;
...
pub struct SimpleAltParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 21],
...
impl<'i, 'arena> SimpleAltParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: SimpleAltScanner::new(input, vec_arena),
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_b_ll1(start, Some((SlotId(1), None)))?;
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_alt_0_ll1(start, Some((SlotId(2), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(0), start, SlotId(4), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
            ...
            match matched {
                TerminalId(1) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(1), start, SlotId(6), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
            ...
            match matched {
                TerminalId(2) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(8), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                ...
                TerminalId(2) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_d_ll1(start, Some((SlotId(12), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
