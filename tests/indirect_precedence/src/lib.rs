...
pub fn parse_s<'a>(
    ...
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::S, &vec_arena);
    match parser.run() {
...
pub fn parse_e<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::E, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_f<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::F, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_k<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::K, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = IndirectPrecedenceParseTreeBuilder::new(tree_arena);
            let tree =
                parse_tree::create_parse_tree_k(success.sppf_node_id, &parser, &parse_tree_builder);
            let tree_construction_duration = tree_start.elapsed();
            let ambiguity_node_added = parser.ambiguity_node_added();
            Ok(ParseSuccess {
                tree,
