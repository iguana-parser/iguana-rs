...
use parse_tree::IndirectPrecedenceParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    ...
    match parser.run() {
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<IndirectPrecedenceParser>(input, &error))
        }
    }
}
pub fn parse_f(input: &Input) -> Result<ParseSuccess<parse_tree::F>, ParseError> {
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::F);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
            let tree = parse_tree::create_parse_tree_f(
                success.sppf_node_id,
                &parser,
                &IndirectPrecedenceParseTreeBuilder,
            );
...
pub fn parse_k(input: &Input) -> Result<ParseSuccess<parse_tree::K>, ParseError> {
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::K);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
            let tree = parse_tree::create_parse_tree_k(
                success.sppf_node_id,
                &parser,
                &IndirectPrecedenceParseTreeBuilder,
            );
