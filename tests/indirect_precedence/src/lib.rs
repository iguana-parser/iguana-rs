...
pub fn parse_s<'a>(
    ...
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_e<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::START_E, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_f<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::START_F, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_k<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a K<'a>, ()>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = IndirectPrecedenceParser::new(input, grammar_data::START_K, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = IndirectPrecedenceParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_k(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
