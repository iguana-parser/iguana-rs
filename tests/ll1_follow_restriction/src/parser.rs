...
impl<'i, 'arena> Parser<'i, 'arena> for Ll1FollowRestrictionParser<'i, 'arena> {
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = Ll1FollowRestrictionParseTreeBuilder::new(tree_arena);
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(3) => {
                if self
                    .scanner
                    .match_any(&FOLLOW_RESTRICTION_A_ALT0_POS0, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow {
                        forbidden: FOLLOW_RESTRICTION_A_ALT0_POS0.terminals.to_vec(),
                    })
                } else {
                    None
                }
    ...
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(3) => FOLLOW_SET_START_A.terminals.to_vec(),
            _ => vec![],
        }
    }
...
impl<'i, 'arena> Ll1FollowRestrictionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: Ll1FollowRestrictionScanner::new(input, vec_arena),
