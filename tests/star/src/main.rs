...
use star::{
    ...
    parse_tree::{StarParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::StarParser,
};
use std::{
    fs::{self, File},
...
#[derive(ClapParser)]
#[command(about = "Parser for the Star grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Star parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = StarParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = StarParser::new(&input, start_nonterminal_id, &vec_arena);
