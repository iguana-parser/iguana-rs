...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, StarParseTreeBuilder, create_parse_tree},
    scanner::StarScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for StarParser<'i, 'arena> {
    type Grammar = StarGrammar;
    type ConcreteParser<'input, 'parser_arena> = StarParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        StarParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = StarParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(5) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(5), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(7) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(7), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_0 : A.
                            input_index = j;
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_plus_0_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_0 : Plus_0.
                            input_index = j;
                    ...
                    SlotId(12) => {
                        if let Some(right_child) =
                            self.parse_opt_0_ll1(input_index, Some((SlotId(12), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Star_0 : Opt_0.
                            input_index = j;
...
impl<'i, 'arena> StarParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: StarScanner::new(input, vec_arena),
    ...
    fn parse_plus_0_ll1(
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            loop {
                let Some((node_0, pos_0)) = self.parse_a_ll1(j, None).map(|node| {
                    let end = self.sppf_node(node).right_extent();
                    (node, end)
                }) else {
                    break;
    ...
    fn parse_opt_0_ll1(
        ...
        match matched {
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let node = self.parse_plus_0_ll1(start, Some((SlotId(10), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
    ...
    fn parse_star_0_ll1(
        ...
        let right_child = {
            let start = j;
            let node = self.parse_opt_0_ll1(start, Some((SlotId(13), None)))?;
            let end = self.sppf_node(node).right_extent();
            j = end;
            node
        };
