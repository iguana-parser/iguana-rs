...
use crate::{
    grammar::*,
    parse_tree::{self, ExactKeywordContextualParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExactKeywordContextualScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExactKeywordContextualParser<'i, 'arena> {
    type Grammar = ExactKeywordContextualGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        ExactKeywordContextualParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExactKeywordContextualParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExactKeywordContextualParseTreeBuilder::new(tree_arena);
                ...
                if !matched {
                    self.add_failure(input_index, SlotId(0), Some(gss_node_id), || {
                        GLLFailureKind::UnexpectedToken {
                            expected: FIRST_SET_DECL.terminals.to_vec(),
                        }
                    });
                }
            }
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(1) => {
                if self
                    .scanner
                    .match_any(&FOLLOW_RESTRICTION_DECL_ALT0_POS0, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow {
                        forbidden: FOLLOW_RESTRICTION_DECL_ALT0_POS0.terminals.to_vec(),
                    })
                } else {
                    None
                }
    ...
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            NonterminalId(0) => FOLLOW_SET_DECL.terminals.to_vec(),
            NonterminalId(1) => FOLLOW_SET_START_DECL.terminals.to_vec(),
            _ => vec![],
        }
    }
...
impl<'i, 'arena> ExactKeywordContextualParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExactKeywordContextualScanner::new(input, vec_arena),
    ...
    pub fn parse_decl(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartDecl<'_>>, ParseError> {
        Ok(self
            .parse(START_DECL, tree_arena)?
            .map(ParseTree::unwrap_start_decl))
    }
}
