...
use exact_keyword_contextual::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{
        ExactKeywordContextualParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = ExactKeywordContextualParseTreeBuilder::new(&tree_arena);
                let vec_arena = Arena::new();
                let mut parser =
                    ExactKeywordContextualParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    let tree_arena = Arena::new();
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordContextualParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
