...
use parse_tree::*;
use parser::ExactKeywordContextualParser;
use std::error::Error;
...
/// Parses `input` starting from `Decl`.
///
...
pub fn parse_decl<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Decl<'a>, Token>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordContextualParser::new(input, grammar_data::START_DECL, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            ...
            let tree = parse_tree::create_parse_tree_start_decl(
