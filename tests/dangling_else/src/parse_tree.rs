...
use crate::{grammar, parser::DanglingElseParser};
use iguana_runtime::{
    arena::Arena,
...
impl<'a> ParseTree<'a> {
    ...
    pub fn is_amb(&self) -> bool {
        match self {
            ...
            ParseTree::Statement(statement) => matches!(statement, Statement::Amb(_)),
    ...
    pub fn node_id(&self) -> Option<usize> {
        match self {
            ...
            ParseTree::Statement(statement) => Some(*statement as *const _ as usize),
    ...
    pub(crate) fn unwrap_statement(self) -> &'a Statement<'a> {
        match self {
    ...
    pub(crate) fn unwrap_start_s(self) -> &'a Start<&'a S<'a>, &'a Layout<'a>> {
        match self {
...
impl<'a> Statement<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            ...
            Statement::Amb(alts) => alts.get(index).copied().map(ParseTree::Statement),
        }
    }
    pub fn child_count(&self) -> usize {
        match self {
    ...
    pub fn span(&self) -> Span {
        match self {
            ...
            Statement::Amb(alts) => alts[0].span(),
        }
    }
    pub fn display_name(&self) -> &'static str {
        match self {
...
pub struct DanglingElseParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> DanglingElseParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for DanglingElseParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = Statement+
                SlotId(1) => {
                    let [statements] = children.into_array::<1usize>();
            ...
            NonterminalId(1) => match nonterminal_node.return_slot {
                ...
                // Statement = Id Layout ";" #Simple
                SlotId(29) => {
                    let [id, layout, lit_2] = children.into_array::<3usize>();
            ...
            NonterminalId(3) => match nonterminal_node.return_slot {
                // Plus_0 = Statement+ Layout Statement
                SlotId(35) => {
                    let [statements_0, layout, statement_2] = children.into_array::<3usize>();
                ...
                // Plus_0 = Statement
                SlotId(37) => {
                    let [statement] = children.into_array::<1usize>();
    ...
    fn new_ambiguity_node(
        ...
        match parent {
            ...
            grammar::STATEMENT => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_statement()));
                ParseTree::Statement(self.arena.alloc(Statement::Amb(slice)))
            }
            grammar::LAYOUT => {
                let slice = self
                    .arena
            ...
            grammar::START_STATEMENT => {
                let first = alternatives[0].unwrap_start_statement();
                let inner = self.arena.alloc_slice(
                    alternatives
                        .into_iter()
                        .map(|a| a.unwrap_start_statement().node),
                );
                let node = &*self.arena.alloc(Statement::Amb(inner));
                ParseTree::StartStatement(self.arena.alloc(Start {
                    before: first.before,
                    node,
                    after: first.after,
...
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &DanglingElseParser,
    builder: &DanglingElseParseTreeBuilder<'a>,
) -> ParseTree<'a> {
    ...
    match root.nonterminal_id {
        ...
        grammar::STATEMENT => ParseTree::Statement(
        ...
        grammar::START_STATEMENT => ParseTree::StartStatement(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
