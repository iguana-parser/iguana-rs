...
use crate::{
    grammar::*,
    parse_tree::{self, EscapedLitParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::EscapedLitScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for EscapedLitParser<'i, 'arena> {
    type Grammar = EscapedLitGrammar;
    type ConcreteParser<'input, 'parser_arena> = EscapedLitParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        EscapedLitParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = EscapedLitParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> EscapedLitParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: EscapedLitScanner::new(input, vec_arena),
            ...
            match matched {
                TerminalId(2) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(5), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(3), start, SlotId(7), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(1), start, SlotId(8), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(1), start, SlotId(10), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
