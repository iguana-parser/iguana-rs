...
use parse_tree::FollowRestrictionParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    let mut parser = FollowRestrictionParser::new(input, grammar_data::S);
    match parser.run() {
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<FollowRestrictionParser>(input, &error))
        }
    }
}
pub fn parse_t(input: &Input) -> Result<ParseSuccess<parse_tree::T>, ParseError> {
    let mut parser = FollowRestrictionParser::new(input, grammar_data::T);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
            let tree = parse_tree::create_parse_tree_t(
                success.sppf_node_id,
                &parser,
                &FollowRestrictionParseTreeBuilder,
            );
...
pub fn parse_id(input: &Input) -> Result<ParseSuccess<parse_tree::Id>, ParseError> {
    let mut parser = FollowRestrictionParser::new(input, grammar_data::ID);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
