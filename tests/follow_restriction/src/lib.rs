...
pub fn parse_s<'a>(
    ...
    let mut parser = FollowRestrictionParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_t<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a Start<&'a T<'a>, Token>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionParser::new(input, grammar_data::START_T, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_id<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Id<'a>, Token>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionParser::new(input, grammar_data::START_ID, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
