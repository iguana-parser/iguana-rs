...
pub fn parse_s<'a>(
    ...
    let mut parser = FollowRestrictionParser::new(input, grammar_data::S, &vec_arena);
    match parser.run() {
...
pub fn parse_t<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionParser::new(input, grammar_data::T, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = FollowRestrictionParseTreeBuilder::new(tree_arena);
            let tree =
                parse_tree::create_parse_tree_t(success.sppf_node_id, &parser, &parse_tree_builder);
            let tree_construction_duration = tree_start.elapsed();
            let ambiguity_node_added = parser.ambiguity_node_added();
            Ok(ParseSuccess {
                tree,
...
pub fn parse_id<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionParser::new(input, grammar_data::ID, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
