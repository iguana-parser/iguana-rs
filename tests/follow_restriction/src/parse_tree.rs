...
impl<'a> ParseTreeRef<'a> {
    pub fn children(&self) -> Vec<ParseTreeRef<'a>> {
        match self {
            ...
            ParseTreeRef::StartT(start_t) => {
                (0..start_t.child_count()).filter_map(|i| start_t.child(i)).collect()
            }
...
pub enum SPlus0 {
    //Id+ Layout Id
    Alt0 { ids: Box<SPlus0>, layout: Token, id_2: Box<Id>, span: Span },
    //Id
...
impl<'a> ListNode<'a> for SPlus0 {
    fn iter(&'a self) -> IntoIter<ParseTreeRef<'a>> {
        ...
        loop {
            match current {
                SPlus0::Alt0 { ids: rest, layout: layout, id_2: item, .. } => {
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(3) => {
                match nonterminal_node.return_slot {
                    //Id+ : Id+ Layout Id.
                    SlotId(9) => {
                        let [ids, layout, id_2] = <[ParseTree; 3usize]>::try_from(
                                children,
                            )
                            .unwrap();
                        SPlus0::Alt0 {
...
pub fn create_parse_tree_start_t(
    ...
    let node = parser.sppf_node(root_id);
    visit_sppf(node, parser, builder).unwrap_one().unwrap_start_t()
}
