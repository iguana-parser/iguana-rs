...
use crate::{grammar, parser::FollowRestrictionParser};
use iguana_runtime::{
    arena::Arena,
...
pub type StartT<'a> = Start<&'a T<'a>, Token>;
pub type StartId<'a> = Start<&'a Id<'a>, Token>;
#[derive(Debug, Clone, Copy)]
...
impl<'a> ParseTree<'a> {
    ...
    pub(crate) fn unwrap_start_t(self) -> &'a Start<&'a T<'a>, Token> {
        match self {
            ParseTree::StartT(start_t) => start_t,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_start_id(self) -> &'a Start<&'a Id<'a>, Token> {
        match self {
