// @generated by iguana. Do not edit manually.

use iguana_runtime::{
    grammar::{Grammar, Nonterminal, Slot, Terminal},
    ids::{NonterminalId, TerminalId},
    scanner::TerminalSet,
};
pub const S: NonterminalId = NonterminalId(0);
pub const T: NonterminalId = NonterminalId(1);
pub const ID: NonterminalId = NonterminalId(2);
pub const PLUS_0: NonterminalId = NonterminalId(3);
pub const PLUS_1: NonterminalId = NonterminalId(4);
pub const START_S: NonterminalId = NonterminalId(5);
pub const START_T: NonterminalId = NonterminalId(6);
pub const START_ID: NonterminalId = NonterminalId(7);
pub struct FollowRestrictionGrammar;
impl Grammar for FollowRestrictionGrammar {
    const NAME: &'static str = "FollowRestriction";
    const NONTERMINALS: &'static [Nonterminal] = &[
        Nonterminal {
            name: "S",
            display_name: "S",
        },
        Nonterminal {
            name: "T",
            display_name: "T",
        },
        Nonterminal {
            name: "Id",
            display_name: "Id",
        },
        Nonterminal {
            name: "Plus_0",
            display_name: "Id+",
        },
        Nonterminal {
            name: "Plus_1",
            display_name: "Char+",
        },
        Nonterminal {
            name: "StartS",
            display_name: "S",
        },
        Nonterminal {
            name: "StartT",
            display_name: "T",
        },
        Nonterminal {
            name: "StartId",
            display_name: "Id",
        },
    ];
    const DISPLAY_ORDER: &'static [&'static str] = &["S", "T", "Id"];
    const TERMINALS: &'static [Terminal] = &[
        Terminal { name: "Char" },
        Terminal { name: "WS" },
        Terminal { name: "Epsilon" },
        Terminal { name: "EOF" },
    ];
    const SLOTS: &'static [Slot] = &[
        Slot {
            display_name: "S : . Id+",
        },
        Slot {
            display_name: "S : Id+.",
        },
        Slot {
            display_name: "T : . Char !>> Char",
        },
        Slot {
            display_name: "T : Char !>> Char.",
        },
        Slot {
            display_name: "Id : . Char+ !>> Char",
        },
        Slot {
            display_name: "Id : Char+ !>> Char.",
        },
        Slot {
            display_name: "Id+ : . Id+ WS Id",
        },
        Slot {
            display_name: "Id+ : Id+ . WS Id",
        },
        Slot {
            display_name: "Id+ : Id+ WS . Id",
        },
        Slot {
            display_name: "Id+ : Id+ WS Id.",
        },
        Slot {
            display_name: "Id+ : . Id",
        },
        Slot {
            display_name: "Id+ : Id.",
        },
        Slot {
            display_name: "Char+ : . Char+ Char",
        },
        Slot {
            display_name: "Char+ : Char+ . Char",
        },
        Slot {
            display_name: "Char+ : Char+ Char.",
        },
        Slot {
            display_name: "Char+ : . Char",
        },
        Slot {
            display_name: "Char+ : Char.",
        },
        Slot {
            display_name: "S : . WS start:S WS",
        },
        Slot {
            display_name: "S : WS . start:S WS",
        },
        Slot {
            display_name: "S : WS start:S . WS",
        },
        Slot {
            display_name: "S : WS start:S WS.",
        },
        Slot {
            display_name: "T : . WS start:T WS",
        },
        Slot {
            display_name: "T : WS . start:T WS",
        },
        Slot {
            display_name: "T : WS start:T . WS",
        },
        Slot {
            display_name: "T : WS start:T WS.",
        },
        Slot {
            display_name: "Id : . WS start:Id WS",
        },
        Slot {
            display_name: "Id : WS . start:Id WS",
        },
        Slot {
            display_name: "Id : WS start:Id . WS",
        },
        Slot {
            display_name: "Id : WS start:Id WS.",
        },
    ];
    const LAYOUT_NAME: Option<&'static str> = Some("WS");
    const LAYOUT_TERMINALS: &'static [TerminalId] = &[TerminalId(1)];
    fn nonterminal_id(name: &str) -> Option<NonterminalId> {
        match name {
            "S" => Some(S),
            "T" => Some(T),
            "Id" => Some(ID),
            "Plus_0" => Some(PLUS_0),
            "Plus_1" => Some(PLUS_1),
            "StartS" => Some(START_S),
            "StartT" => Some(START_T),
            "StartId" => Some(START_ID),
            _ => None,
        }
    }
}
// S { WS, EOF }
pub static FOLLOW_SET_S: TerminalSet = TerminalSet {
    id: 0,
    terminals: &[TerminalId(1), TerminalId(3)],
};
// S : . Plus_0 { Char }
pub static FIRST_SET_S_ALT0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// T { WS, EOF }
pub static FOLLOW_SET_T: TerminalSet = TerminalSet {
    id: 0,
    terminals: &[TerminalId(1), TerminalId(3)],
};
// T : . Char !>> Char { Char }
pub static FIRST_SET_T_ALT0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// T : . Char !>> Char !>> { Char }
pub static FOLLOW_RESTRICTION_T_ALT0_POS0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Id { Char, WS, EOF }
pub static FOLLOW_SET_ID: TerminalSet = TerminalSet {
    id: 2,
    terminals: &[TerminalId(0), TerminalId(1), TerminalId(3)],
};
// Id : . Plus_1 !>> Char { Char }
pub static FIRST_SET_ID_ALT0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Id : . Plus_1 !>> Char !>> { Char }
pub static FOLLOW_RESTRICTION_ID_ALT0_POS0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Plus_0 { Char, WS, EOF }
pub static FOLLOW_SET_PLUS_0: TerminalSet = TerminalSet {
    id: 2,
    terminals: &[TerminalId(0), TerminalId(1), TerminalId(3)],
};
// Plus_0 : . Plus_0 WS Id { Char }
pub static FIRST_SET_PLUS_0_ALT0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Plus_0 : . Id { Char }
pub static FIRST_SET_PLUS_0_ALT1: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Plus_1 { Char, WS, EOF }
pub static FOLLOW_SET_PLUS_1: TerminalSet = TerminalSet {
    id: 2,
    terminals: &[TerminalId(0), TerminalId(1), TerminalId(3)],
};
// Plus_1 : . Plus_1 Char { Char }
pub static FIRST_SET_PLUS_1_ALT0: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// Plus_1 : . Char { Char }
pub static FIRST_SET_PLUS_1_ALT1: TerminalSet = TerminalSet {
    id: 1,
    terminals: &[TerminalId(0)],
};
// StartS { EOF }
pub static FOLLOW_SET_START_S: TerminalSet = TerminalSet {
    id: 3,
    terminals: &[TerminalId(3)],
};
// StartS : . WS start:S WS { Char, WS }
pub static FIRST_SET_START_S_ALT0: TerminalSet = TerminalSet {
    id: 4,
    terminals: &[TerminalId(0), TerminalId(1)],
};
// StartT { EOF }
pub static FOLLOW_SET_START_T: TerminalSet = TerminalSet {
    id: 3,
    terminals: &[TerminalId(3)],
};
// StartT : . WS start:T WS { Char, WS }
pub static FIRST_SET_START_T_ALT0: TerminalSet = TerminalSet {
    id: 4,
    terminals: &[TerminalId(0), TerminalId(1)],
};
// StartId { EOF }
pub static FOLLOW_SET_START_ID: TerminalSet = TerminalSet {
    id: 3,
    terminals: &[TerminalId(3)],
};
// StartId : . WS start:Id WS { Char, WS }
pub static FIRST_SET_START_ID_ALT0: TerminalSet = TerminalSet {
    id: 4,
    terminals: &[TerminalId(0), TerminalId(1)],
};
// S { Char }
pub static FIRST_SET_S: TerminalSet = TerminalSet {
    id: 5,
    terminals: &[TerminalId(0)],
};
// T { Char }
pub static FIRST_SET_T: TerminalSet = TerminalSet {
    id: 5,
    terminals: &[TerminalId(0)],
};
// Id { Char }
pub static FIRST_SET_ID: TerminalSet = TerminalSet {
    id: 5,
    terminals: &[TerminalId(0)],
};
// Plus_0 { Char }
pub static FIRST_SET_PLUS_0: TerminalSet = TerminalSet {
    id: 5,
    terminals: &[TerminalId(0)],
};
// Plus_1 { Char }
pub static FIRST_SET_PLUS_1: TerminalSet = TerminalSet {
    id: 5,
    terminals: &[TerminalId(0)],
};
// StartS { Char, WS }
pub static FIRST_SET_START_S: TerminalSet = TerminalSet {
    id: 6,
    terminals: &[TerminalId(0), TerminalId(1)],
};
// StartT { Char, WS }
pub static FIRST_SET_START_T: TerminalSet = TerminalSet {
    id: 6,
    terminals: &[TerminalId(0), TerminalId(1)],
};
// StartId { Char, WS }
pub static FIRST_SET_START_ID: TerminalSet = TerminalSet {
    id: 6,
    terminals: &[TerminalId(0), TerminalId(1)],
};
