...
use layout_aware_follow_required_layout::{
    ...
    parse_tree::{
        LayoutAwareFollowRequiredLayoutParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
fn main() -> Result<(), io::Error> {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder =
                    LayoutAwareFollowRequiredLayoutParseTreeBuilder::new(&tree_arena);
                let vec_arena = Arena::new();
                let mut parser = LayoutAwareFollowRequiredLayoutParser::new(
                    &input,
    ...
    let vec_arena = Arena::new();
    let mut parser =
        LayoutAwareFollowRequiredLayoutParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = LayoutAwareFollowRequiredLayoutParseTreeBuilder::new(&tree_arena);
    let result = parser.run();
    #[cfg(feature = "debug-trace")]
