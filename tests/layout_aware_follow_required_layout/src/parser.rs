...
use crate::{
    grammar::*,
    parse_tree::{
        self, LayoutAwareFollowRequiredLayoutParseTreeBuilder, ParseTree, create_parse_tree,
    },
    scanner::LayoutAwareFollowRequiredLayoutScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutAwareFollowRequiredLayoutParser<'i, 'arena> {
    type Grammar = LayoutAwareFollowRequiredLayoutGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        LayoutAwareFollowRequiredLayoutParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LayoutAwareFollowRequiredLayoutParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LayoutAwareFollowRequiredLayoutParseTreeBuilder::new(tree_arena);
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(1) => {
                if (self.scanner.match_token(TerminalId(2), right_extent)).is_some_and(|end| {
                    self.scanner
                        .match_any(&LAYOUT_AWARE_FOLLOW_RESTRICTION_S_ALT0_POS0, end)
                }) {
                    Some(GLLFailureKind::ForbiddenFollow {
                        forbidden: LAYOUT_AWARE_FOLLOW_RESTRICTION_S_ALT0_POS0
                            .terminals
                            .to_vec(),
                    })
                } else {
...
pub struct LayoutAwareFollowRequiredLayoutParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: LayoutAwareFollowRequiredLayoutScanner<'i, 'arena>,
...
impl<'i, 'arena> LayoutAwareFollowRequiredLayoutParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LayoutAwareFollowRequiredLayoutScanner::new(input, vec_arena),
