...
use crate::{grammar, parser::AmbListParser};
use iguana_runtime::{
    arena::Arena,
...
impl<'a> S<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 { as_0, as_1, .. } => match index {
                0 => Some(ParseTree::Star0(as_0)),
                1 => Some(ParseTree::Star0(as_1)),
                _ => None,
            },
    ...
    pub fn as_0(&self) -> &'a Star0<'a> {
        match self {
            S::Alt0 { as_0, .. } => as_0,
            S::Amb(_) => panic!("S is ambiguous"),
        }
    }
    pub fn as_1(&self) -> &'a Star0<'a> {
        match self {
            S::Alt0 { as_1, .. } => as_1,
            S::Amb(_) => panic!("S is ambiguous"),
        }
...
pub struct AmbListParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> AmbListParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for AmbListParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = A* A*
                SlotId(2) => {
                    let [as_0, as_1] = children.into_array::<2usize>();
                    ParseTree::S(self.arena.alloc(S::Alt0 {
                        as_0: as_0.unwrap_star_0(),
                        as_1: as_1.unwrap_star_0(),
...
pub fn create_parse_tree<'a>(
    ...
    builder: &AmbListParseTreeBuilder<'a>,
) -> ParseTree<'a> {
