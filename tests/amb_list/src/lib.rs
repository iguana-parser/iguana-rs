...
pub fn parse_s<'a>(
    ...
    let mut parser = AmbListParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_a<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = AmbListParser::new(input, grammar_data::START_A, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
