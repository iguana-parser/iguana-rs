...
use crate::{
    grammar::*,
    parse_tree::{self, AmbListParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::AmbListScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for AmbListParser<'i, 'arena> {
    type Grammar = AmbListGrammar;
    type ConcreteParser<'input, 'parser_arena> = AmbListParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AmbListParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbListParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> AmbListParser<'i, 'arena> {
