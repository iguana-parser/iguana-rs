use amb_list::{
    ...
    parse_tree::{AmbListParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::AmbListParser,
};
...
#[derive(ClapParser)]
#[command(about = "Parser for the AmbList grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the AmbList parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbListParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = AmbListParser::new(&input, start_nonterminal_id, &vec_arena);
