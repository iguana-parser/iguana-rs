...
use crate::{
    grammar::*,
    parse_tree::{self, AssocTestParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for AssocTestParser<'i, 'arena> {
    type Grammar = AssocTestGrammar;
    type ConcreteParser<'input, 'parser_arena> = AssocTestParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AssocTestParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AssocTestParseTreeBuilder::new(tree_arena);
        ...
        match slot_id {
            ...
            SlotId(34) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(4), input_index, SlotId(34), Some(gss_node_id))
                {
                    // E(p: i32) : "a" . return 0
...
pub struct AssocTestParser<'i, 'arena> {
    ...
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 31],
...
impl<'i, 'arena> AssocTestParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: AssocTestScanner::new(input, vec_arena),
