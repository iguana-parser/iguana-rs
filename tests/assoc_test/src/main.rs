use assoc_test::{
    ...
    parse_tree::{AssocTestParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the AssocTest grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the AssocTest parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AssocTestParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = AssocTestParser::new(&input, start_nonterminal_id, &vec_arena);
