use assoc_test::{
    parse_tree::{AssocTestParseTreeBuilder, create_parse_tree, to_json, to_sexpr},
    parser::{AssocTestParser, NONTERMINALS, SLOTS, TERMINALS},
};
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = AssocTestParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = AssocTestParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = AssocTestParseTreeBuilder;
