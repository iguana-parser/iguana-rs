...
use crate::{
    grammar::*,
    parse_tree::{self, KeywordLabelsParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::KeywordLabelsScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for KeywordLabelsParser<'i, 'arena> {
    type Grammar = KeywordLabelsGrammar;
    type ConcreteParser<'input, 'parser_arena> = KeywordLabelsParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        KeywordLabelsParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = KeywordLabelsParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> KeywordLabelsParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: KeywordLabelsScanner::new(input, vec_arena),
    ...
    fn parse_s_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        ...
        match matched {
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(2), start, SlotId(3), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
