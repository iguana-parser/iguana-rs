...
use crate::{grammar, parser::KeywordLabelsParser};
use iguana_runtime::{
    arena::Arena,
...
impl<'a> S<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 {
                ...
                0 => Some(ParseTree::Token(*r#do)),
                1 => Some(ParseTree::Token(*r#type)),
                2 => Some(ParseTree::Token(*r#gen)),
                _ => None,
            },
    ...
    pub fn r#type(&self) -> Token {
        match self {
            S::Alt0 { r#type, .. } => *r#type,
            S::Amb(_) => panic!("S is ambiguous"),
        }
    }
    pub fn r#gen(&self) -> Token {
        match self {
            S::Alt0 { r#gen, .. } => *r#gen,
            S::Amb(_) => panic!("S is ambiguous"),
        }
...
pub struct KeywordLabelsParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> KeywordLabelsParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for KeywordLabelsParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = do:"a" type:"b" gen:"c"
                SlotId(3) => {
                    let [r#do, r#type, r#gen] = children.into_array::<3usize>();
                    ParseTree::S(self.arena.alloc(S::Alt0 {
                        r#do: r#do.unwrap_token(),
                        r#type: r#type.unwrap_token(),
                        r#gen: r#gen.unwrap_token(),
...
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &KeywordLabelsParser,
    builder: &KeywordLabelsParseTreeBuilder<'a>,
) -> ParseTree<'a> {
