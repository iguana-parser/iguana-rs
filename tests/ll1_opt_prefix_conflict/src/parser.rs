...
use crate::{
    grammar::*,
    parse_tree::{self, Ll1OptPrefixConflictParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::Ll1OptPrefixConflictScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for Ll1OptPrefixConflictParser<'i, 'arena> {
    type Grammar = Ll1OptPrefixConflictGrammar;
    type ConcreteParser<'input, 'parser_arena> = Ll1OptPrefixConflictParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        Ll1OptPrefixConflictParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = Ll1OptPrefixConflictParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> Ll1OptPrefixConflictParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: Ll1OptPrefixConflictScanner::new(input, vec_arena),
