...
use ll1_opt_prefix_conflict::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{Ll1OptPrefixConflictParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the Ll1OptPrefixConflict grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Ll1OptPrefixConflict parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser =
                    Ll1OptPrefixConflictParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    let tree_arena = Bump::new();
    let vec_arena = Bump::new();
    let mut parser = Ll1OptPrefixConflictParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
