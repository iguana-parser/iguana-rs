...
impl<'i, 'arena> Parser<'i, 'arena> for AmbRootNestedParser<'i, 'arena> {
    type Grammar = AmbRootNestedGrammar;
    type ConcreteParser<'input, 'parser_arena> = AmbRootNestedParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AmbRootNestedParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbRootNestedParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> AmbRootNestedParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: AmbRootNestedScanner::new(input, vec_arena),
