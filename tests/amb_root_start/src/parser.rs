...
use crate::{
    grammar::*,
    parse_tree::{self, AmbRootStartParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::AmbRootStartScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for AmbRootStartParser<'i, 'arena> {
    type Grammar = AmbRootStartGrammar;
    type ConcreteParser<'input, 'parser_arena> = AmbRootStartParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AmbRootStartParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbRootStartParseTreeBuilder::new(tree_arena);
        ...
        match slot_id {
            ...
            SlotId(12) => {
                if let Some(right_child) = self.parse_type_ll1(input_index) {
                    ...
                    self.add_failure(input_index, SlotId(12), Some(gss_node_id), || {
            ...
            SlotId(42) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(2), input_index, SlotId(42), Some(gss_node_id))
                {
                    // E(p: i32) : "(" . WS E(0) WS ")" return 0
            ...
            SlotId(43) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(1), input_index, SlotId(43), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(45) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(1), input_index, SlotId(45), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(46) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(46), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            // E(p: i32) : "(" WS E(0) WS ")" . return 0
            SlotId(47) => {
                self.execute(input_index, SlotId(48), result, gss_node_id, env);
            }
            // E(p: i32) : "(" WS E(0) WS ")" return 0.
            SlotId(48) => {
                let Some(result) = result else {
            ...
            // E(p: i32) : Id . return 0
            SlotId(50) => {
                self.execute(input_index, SlotId(51), result, gss_node_id, env);
            }
            // E(p: i32) : Id return 0.
            SlotId(51) => {
                let Some(result) = result else {
...
pub struct AmbRootStartParser<'i, 'arena> {
    ...
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 42],
...
impl<'i, 'arena> AmbRootStartParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: AmbRootStartScanner::new(input, vec_arena),
            ...
            dd_intermediate_nodes_index: [const { InlineMap::Empty }; 42],
