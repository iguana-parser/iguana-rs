...
use crate::{
    grammar::*,
    parse_tree::{self, ExceptFollowRestrictionParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExceptFollowRestrictionScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExceptFollowRestrictionParser<'i, 'arena> {
    type Grammar = ExceptFollowRestrictionGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        ExceptFollowRestrictionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExceptFollowRestrictionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExceptFollowRestrictionParseTreeBuilder::new(tree_arena);
        ...
        match slot_id {
            ...
            SlotId(1) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(1), Some(gss_node_id))
                {
...
impl<'i, 'arena> ExceptFollowRestrictionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExceptFollowRestrictionScanner::new(input, vec_arena),
    ...
    fn parse_s_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        ...
        match matched {
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(3), start, SlotId(2), None)?;
                    j = end;
                    node
                };
