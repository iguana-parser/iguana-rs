...
use crate::{
    grammar::*,
    parse_tree::{self, AmbParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::AmbScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for AmbParser<'i, 'arena> {
    type Grammar = AmbGrammar;
    type ConcreteParser<'input, 'parser_arena> = AmbParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AmbParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // E(p: i32) : [2 >= p] l_pr=lhs:E(p) [(l_pr == 0) || (l_pr >= 2)] "*" . rhs:E(3) return 2
            SlotId(13) => {
                self.create_e(result, gss_node_id, SlotId(14), env, 3);
            }
            // E(p: i32) : [2 >= p] l_pr=lhs:E(p) [(l_pr == 0) || (l_pr >= 2)] "*" rhs:E(3) . return 2
            SlotId(14) => {
            ...
            // E(p: i32) : [1 >= p] l_pr=lhs:E(p) [(l_pr == 0) || (l_pr >= 1)] "+" . rhs:E(2) return 1
            SlotId(20) => {
                self.create_e(result, gss_node_id, SlotId(21), env, 2);
            }
            // E(p: i32) : [1 >= p] l_pr=lhs:E(p) [(l_pr == 0) || (l_pr >= 1)] "+" rhs:E(2) . return 1
            SlotId(21) => {
...
impl<'i, 'arena> AmbParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: AmbScanner::new(input, vec_arena),
