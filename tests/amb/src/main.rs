use amb::{
    ...
    parse_tree::{AmbParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::AmbParser,
};
...
#[derive(ClapParser)]
#[command(about = "Parser for the Amb grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Amb parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = AmbParser::new(&input, start_nonterminal_id, &vec_arena);
