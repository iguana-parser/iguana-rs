...
use layout_aware_follow_same_position::{
    LayoutAwareFollowSamePositionGrammar, LayoutAwareFollowSamePositionParser,
};
#[cfg(feature = "profile")]
...
#[derive(ClapParser)]
#[command(about = "Parser for the LayoutAwareFollowSamePosition grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the LayoutAwareFollowSamePosition parser in REPL mode
    ///
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in LayoutAwareFollowSamePositionGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: LayoutAwareFollowSamePositionGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: LayoutAwareFollowSamePositionGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: LayoutAwareFollowSamePositionGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = LayoutAwareFollowSamePositionParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            LayoutAwareFollowSamePositionGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    LayoutAwareFollowSamePositionGrammar :: nonterminal_id (& format ! ("Start{}" , name)) . ok_or_else (|| io :: Error :: new (io :: ErrorKind :: InvalidInput , format ! ("Unknown start nonterminal: '{}'. Use --list-nonterminals to see the valid names." , name ,) ,))
}
fn run_batch(
    dir: &Path,
    ext: Option<&str>,
