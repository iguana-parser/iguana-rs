...
use layout_aware_follow_same_position::{
    ...
    parse_tree::{
        LayoutAwareFollowSamePositionParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
fn main() -> Result<(), io::Error> {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder =
                    LayoutAwareFollowSamePositionParseTreeBuilder::new(&tree_arena);
                let vec_arena = Arena::new();
                let mut parser = LayoutAwareFollowSamePositionParser::new(
                    &input,
    ...
    if args.repl {
        ...
        cli::run_repl(display_options, |text, display_options| {
            ...
            let vec_arena = Arena::new();
            let mut parser =
                LayoutAwareFollowSamePositionParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
                ParseResult::Success(success) => {
    ...
    let parse_tree_builder = LayoutAwareFollowSamePositionParseTreeBuilder::new(&tree_arena);
    let result = parser.run();
    #[cfg(feature = "debug-trace")]
