...
use crate::{
    grammar::*,
    parse_tree::{
        self, LayoutAwareFollowSamePositionParseTreeBuilder, ParseTree, create_parse_tree,
    },
    scanner::LayoutAwareFollowSamePositionScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutAwareFollowSamePositionParser<'i, 'arena> {
    type Grammar = LayoutAwareFollowSamePositionGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        LayoutAwareFollowSamePositionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LayoutAwareFollowSamePositionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LayoutAwareFollowSamePositionParseTreeBuilder::new(tree_arena);
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(1) => {
                ...
                } else if (self.scanner.match_token(TerminalId(3), right_extent)).is_some_and(
                    |end| {
                        self.scanner
...
pub struct LayoutAwareFollowSamePositionParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: LayoutAwareFollowSamePositionScanner<'i, 'arena>,
...
impl<'i, 'arena> LayoutAwareFollowSamePositionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LayoutAwareFollowSamePositionScanner::new(input, vec_arena),
