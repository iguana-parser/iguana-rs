...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, PrecedeRestrictionParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for PrecedeRestrictionParser<'i, 'arena> {
    type Grammar = PrecedeRestrictionGrammar;
    type ConcreteParser<'input, 'parser_arena> = PrecedeRestrictionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        PrecedeRestrictionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PrecedeRestrictionParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(12) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(2), SlotId(12), gss_node_id);
            ...
            // StartS : WS . start:S WS
            SlotId(14) => {
                self.create(NonterminalId(0), result, gss_node_id, SlotId(15), env);
            }
            // StartS : WS start:S . WS
            SlotId(15) => {
                if let Some((_, right_child)) =
            ...
            SlotId(16) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(3), SlotId(16), gss_node_id);
            ...
            SlotId(20) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(4), SlotId(20), gss_node_id);
...
impl<'i, 'arena> PrecedeRestrictionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PrecedeRestrictionScanner::new(input, vec_arena),
