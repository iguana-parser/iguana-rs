...
use crate::{grammar, parser::PrecedeFollowRestrictionParser};
use iguana_runtime::{
    arena::Arena,
...
impl<'a> S<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 { num, id, tail, .. } => match index {
                0 => Some(ParseTree::Token(*num)),
                1 => Some(ParseTree::Id(id)),
                2 => Some(ParseTree::Token(*tail)),
                _ => None,
            },
            S::Alt1 { id, tail, .. } => match index {
                0 => Some(ParseTree::Id(id)),
                1 => Some(ParseTree::Token(*tail)),
                _ => None,
            },
...
pub struct PrecedeFollowRestrictionParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> PrecedeFollowRestrictionParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for PrecedeFollowRestrictionParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = Num Id Tail
                SlotId(3) => {
                    let [num, id, tail] = children.into_array::<3usize>();
                    ParseTree::S(self.arena.alloc(S::Alt0 {
                        num: num.unwrap_token(),
                        id: id.unwrap_id(),
                        tail: tail.unwrap_token(),
...
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &PrecedeFollowRestrictionParser,
    builder: &PrecedeFollowRestrictionParseTreeBuilder<'a>,
) -> ParseTree<'a> {
