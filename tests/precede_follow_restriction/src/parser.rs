...
impl<'i, 'arena> Parser<'i, 'arena> for PrecedeFollowRestrictionParser<'i, 'arena> {
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(5) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(4), input_index, SlotId(5), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(8) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(1), SlotId(8), gss_node_id);
    ...
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(3) => FOLLOW_SET_START_ID.terminals.to_vec(),
            _ => vec![],
        }
    }
    ...
    fn parse_s_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        ...
        match matched {
            TerminalId(2) => {
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(4), start, SlotId(3), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
            ...
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(4), start, SlotId(6), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
