...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, PrecedeFollowRestrictionParseTreeBuilder, create_parse_tree},
    scanner::PrecedeFollowRestrictionScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for PrecedeFollowRestrictionParser<'i, 'arena> {
    type Grammar = PrecedeFollowRestrictionGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        PrecedeFollowRestrictionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        PrecedeFollowRestrictionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PrecedeFollowRestrictionParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(1) => {
                        if let Some(right_child) =
                            self.parse_id_ll1(input_index, Some((SlotId(1), Some(gss_node_id))))
                        {
                    ...
                    SlotId(4) => {
                        if let Some(right_child) =
                            self.parse_id_ll1(input_index, Some((SlotId(4), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // S : Id . Tail
                            input_index = j;
                    ...
                    SlotId(7) => {
                        if input_index == 0
                            || self
                                .scanner
                                .match_token(TerminalId(1), input_index - 1)
                                .is_none()
                        {
                            if let Some((j, right_child)) = self.match_terminal(
                                TerminalId(0),
                                ...
                                if let Some(failure) =
                                    self.post_conditions(SlotId(8), input_index, j)
                                {
                                    self.add_failure(j, SlotId(8), Some(gss_node_id), failure);
                                } else {
                                    // Id : Digit !<< Name !>> Eq.
                                    input_index = j;
                                    result = Some(right_child);
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartS : start:S.
                            input_index = j;
                    ...
                    SlotId(11) => {
                        if let Some(right_child) =
                            self.parse_id_ll1(input_index, Some((SlotId(11), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartId : start:Id.
                            input_index = j;
...
pub struct PrecedeFollowRestrictionParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 13],
...
impl<'i, 'arena> PrecedeFollowRestrictionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PrecedeFollowRestrictionScanner::new(input, vec_arena),
            ...
            match matched {
                TerminalId(2) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(2), start, SlotId(1), None)?;
                        j = end;
                        node
                    };
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_id_ll1(start, Some((SlotId(2), None)))?;
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(4), start, SlotId(3), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
                ...
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_id_ll1(start, Some((SlotId(5), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(4), start, SlotId(6), None)?;
                        j = end;
                        node
                    };
                    current = self.create_intermediate_node_ll1(
    ...
    fn parse_id_ll1(
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    let mut j = i;
                    if !(j == 0 || self.scanner.match_token(TerminalId(1), j - 1).is_none()) {
                        return None;
                    }
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(0), start, SlotId(8), None)?;
                        if let Some(failure) = self.post_conditions(SlotId(8), start, end) {
                            self.add_failure(end, SlotId(8), None, failure);
                            return None;
                        }
                        j = end;
                        node
