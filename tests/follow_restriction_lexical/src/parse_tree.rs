...
impl<'a> ParseTreeRef<'a> {
    pub fn children(&self) -> Vec<ParseTreeRef<'a>> {
        match self {
            ...
            ParseTreeRef::Element(element) => {
                (0..element.child_count()).filter_map(|i| element.child(i)).collect()
            }
            ...
            ParseTreeRef::StartElement(start_element) => {
                (0..start_element.child_count())
...
pub enum SPlus0 {
    //Element+ WS Element
    Alt0 { elements: Box<SPlus0>, ws: Token, element_2: Box<Element>, span: Span },
    //Element
...
impl SPlus0 {
    pub fn child(&self, index: usize) -> Option<ParseTreeRef<'_>> {
        match self {
            SPlus0::Alt0 { elements, ws, element_2, .. } => {
                match index {
...
impl<'a> ListNode<'a> for SPlus0 {
    fn iter(&'a self) -> IntoIter<ParseTreeRef<'a>> {
        ...
        loop {
            match current {
                SPlus0::Alt0 { elements: rest, ws: layout, element_2: item, .. } => {
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(0) => {
                match nonterminal_node.return_slot {
                    //S : Element+.
                    SlotId(1) => {
                        let [elements] = <[ParseTree; 1usize]>::try_from(children)
                            .unwrap();
                        S {
            ...
            NonterminalId(2) => {
                match nonterminal_node.return_slot {
                    ...
                    SlotId(9) => {
                        let [elements, ws, element_2] = <[ParseTree; 3usize]>::try_from(
                                children,
                            )
                            .unwrap();
                        SPlus0::Alt0 {
                    ...
                    //Element+ : Element.
                    SlotId(11) => {
                        let [element] = <[ParseTree; 1usize]>::try_from(children)
                            .unwrap();
                        SPlus0::Alt1 {
...
pub fn create_parse_tree(
    ...
    match name {
        ...
        "StartElement" => {
            ParseTree::StartElement(
                create_parse_tree_start_element(root_id, parser, builder),
            )
        }
        _ => panic!(),
    }
...
pub fn create_parse_tree_element(
    ...
    let node = parser.sppf_node(root_id);
    visit_sppf(node, parser, builder).unwrap_one().unwrap_element()
}
...
pub fn create_parse_tree_start_element(
    ...
    let node = parser.sppf_node(root_id);
    visit_sppf(node, parser, builder).unwrap_one().unwrap_start_element()
}
