...
use crate::{
    grammar::*,
    parse_tree::{self, FollowRestrictionLexicalParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for FollowRestrictionLexicalParser<'i, 'arena> {
    type Grammar = FollowRestrictionLexicalGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        FollowRestrictionLexicalParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        FollowRestrictionLexicalParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = FollowRestrictionLexicalParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(12) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(12), Some(gss_node_id))
                {
                    // StartS : WS . start:S WS
            ...
            // StartS : WS . start:S WS
            SlotId(13) => {
                self.create(NonterminalId(0), result, gss_node_id, SlotId(14), env);
            }
            // StartS : WS start:S . WS
            SlotId(14) => {
                if let Some((_, right_child)) =
            ...
            SlotId(15) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(3), SlotId(15), gss_node_id);
                self.pop(gss_node_id, SlotId(15), nonterminal_node_id, None);
            }
            // StartElement : . WS start:Element WS
            SlotId(16) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(16), Some(gss_node_id))
                {
                    // StartElement : WS . start:Element WS
...
impl<'i, 'arena> FollowRestrictionLexicalParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: FollowRestrictionLexicalScanner::new(input, vec_arena),
    ...
    pub fn parse_element(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartElement<'_>>, ParseError> {
        Ok(self
            .parse(START_ELEMENT, tree_arena)?
            .map(ParseTree::unwrap_start_element))
    }
