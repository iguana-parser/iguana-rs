...
use crate::{
    grammar::*,
    parse_tree::{self, AmbRootParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::AmbRootScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for AmbRootParser<'i, 'arena> {
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbRootParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] WS "-" WS . E(2) return 1
            SlotId(30) => {
                self.create_e(result, gss_node_id, SlotId(31), env, 2);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] WS "-" WS E(2) . return 1
            SlotId(31) => {
