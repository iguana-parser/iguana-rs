    ...
    fn add_first_descriptors(
        ...
        match nonterminal_id {
            ...
            NonterminalId(3) => {
                let mut matched = false;
                // Id_Alt_0 : . Alpha
                if self.scanner.match_any(FIRST_SET_ID_ALT_0_ALT0, input_index) {
                    matched = true;
                    self.add_first_descriptor(SlotId(9), input_index, gss_node_id, env);
                }
                // Id_Alt_0 : . Digit
                if self.scanner.match_any(FIRST_SET_ID_ALT_0_ALT1, input_index) {
                    matched = true;
...
impl<'i> FollowRestrictionMultipleParser<'i> {
    ...
    fn parse_id_alt_0_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        let matched = self.scanner.longest_match(FIRST_SET_ID_ALT_0, i)?;
        match matched {
            TerminalId(0) => {
                let mut j = i;
                let right_child = {
