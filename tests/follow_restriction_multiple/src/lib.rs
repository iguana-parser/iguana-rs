...
pub fn parse_s<'a>(
    ...
    let mut parser = FollowRestrictionMultipleParser::new(input, grammar_data::S, &vec_arena);
    match parser.run() {
...
pub fn parse_id<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionMultipleParser::new(input, grammar_data::ID, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
