...
use parse_tree::FollowRestrictionMultipleParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    let mut parser = FollowRestrictionMultipleParser::new(input, grammar_data::S);
    match parser.run() {
        ParseResult::Success(success) => {
            ...
            let tree = parse_tree::create_parse_tree_s(
                ...
                &parser,
                &FollowRestrictionMultipleParseTreeBuilder,
            );
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<FollowRestrictionMultipleParser>(input, &error))
        }
    }
}
pub fn parse_id(input: &Input) -> Result<ParseSuccess<parse_tree::Id>, ParseError> {
    let mut parser = FollowRestrictionMultipleParser::new(input, grammar_data::ID);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
