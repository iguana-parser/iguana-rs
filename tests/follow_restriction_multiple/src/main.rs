...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = FollowRestrictionMultipleParser::nonterminal_id(
    ...
    if let Some(iterations) = cli.profile {
        let parse_tree_builder = FollowRestrictionMultipleParseTreeBuilder;
            ...
            match &error.kind {
                ParseErrorKind::UnexpectedToken { expected } => {
                    let names: Vec<_> = expected
                        .iter()
                        .map(|t| FollowRestrictionMultipleParser::terminal_name(*t))
                        .collect();
                    println!(
