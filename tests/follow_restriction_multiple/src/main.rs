...
#[derive(ClapParser)]
#[command(about = "Parser for the FollowRestrictionMultiple grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the FollowRestrictionMultiple parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder =
                    FollowRestrictionMultipleParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser =
                    FollowRestrictionMultipleParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser = FollowRestrictionMultipleParser::new(
                        &input,
                        start_nonterminal_id,
                        &vec_arena,
                    );
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let tree_arena = Bump::new();
            let parse_tree_builder = FollowRestrictionMultipleParseTreeBuilder::new(&tree_arena);
            let vec_arena = Bump::new();
            let mut parser =
                FollowRestrictionMultipleParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
    ...
    let tree_arena = Bump::new();
    let vec_arena = Bump::new();
    let mut parser = FollowRestrictionMultipleParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
