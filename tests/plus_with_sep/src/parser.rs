...
impl<'i, 'arena> Parser<'i, 'arena> for PlusWithSepParser<'i, 'arena> {
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PlusWithSepParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(10) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(10), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartS : start:S.
                            input_index = j;
...
impl<'i, 'arena> PlusWithSepParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PlusWithSepScanner::new(input, vec_arena),
