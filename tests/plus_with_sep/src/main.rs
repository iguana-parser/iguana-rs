...
use plus_with_sep::{
    ...
    parse_tree::{PlusWithSepParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the PlusWithSep grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the PlusWithSep parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = PlusWithSepParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = PlusWithSepParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser =
                        PlusWithSepParser::new(&input, start_nonterminal_id, &vec_arena);
