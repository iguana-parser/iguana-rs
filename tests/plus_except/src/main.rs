...
use plus_except::{
    ...
    parse_tree::{PlusExceptParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the PlusExcept grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the PlusExcept parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = PlusExceptParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser =
                        PlusExceptParser::new(&input, start_nonterminal_id, &vec_arena);
