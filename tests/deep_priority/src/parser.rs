...
use crate::{grammar_data::*, scanner::DeepPriorityScanner};
...
impl<'i, 'arena> Parser<'i, 'arena> for DeepPriorityParser<'i, 'arena> {
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // : min(r, 2)
            SlotId(19) => {
                self.create_e(result, gss_node_id, SlotId(20), env, Some(BINDING_R), 2);
            }
            // E(p: i32) : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS "+" WS r=E(2) . return (r == 0) ? 2
            // : min(r, 2)
            SlotId(20) => {
            ...
            // E(p: i32) : "if" WS E(0) WS "then" WS . E(0) WS "else" WS E(1) return 1
            SlotId(28) => {
                self.create_e(result, gss_node_id, SlotId(29), env, None, 0);
            }
            // E(p: i32) : "if" WS E(0) WS "then" WS E(0) . WS "else" WS E(1) return 1
            SlotId(29) => {
                if let Some((_, right_child)) =
