...
pub fn parse_s<'a>(
    ...
    let mut parser = ExceptNonterminalParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_id<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = ExceptNonterminalParser::new(input, grammar_data::START_ID, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
/// Parses `input` starting from `Name`.
///
/// `tree_arena` holds the constructed parse tree: the returned tree borrows it
/// and lives until the arena is reset or dropped. Once the tree goes out of
/// scope, the arena can be reset with `tree_arena.reset()` and reused for the
...
pub fn parse_name<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Name<'a>, ()>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = ExceptNonterminalParser::new(input, grammar_data::START_NAME, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = ExceptNonterminalParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_name(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
