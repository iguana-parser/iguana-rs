...
use crate::{
    grammar::*,
    parse_tree::{self, ExceptNonterminalParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for ExceptNonterminalParser<'i, 'arena> {
    type Grammar = ExceptNonterminalGrammar;
    type ConcreteParser<'input, 'parser_arena> = ExceptNonterminalParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExceptNonterminalParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExceptNonterminalParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(2) => {
                        if let Some(right_child) =
                            self.parse_name_ll1(input_index, Some((SlotId(2), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(6) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(6), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartS : start:S.
                            input_index = j;
                    ...
                    SlotId(10) => {
                        if let Some(right_child) =
                            self.parse_name_ll1(input_index, Some((SlotId(10), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartName : start:Name.
                            input_index = j;
...
pub struct ExceptNonterminalParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 12],
...
impl<'i, 'arena> ExceptNonterminalParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExceptNonterminalScanner::new(input, vec_arena),
    ...
    pub fn parse_name(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartName<'_>>, ParseError> {
        Ok(self
            .parse(START_NAME, tree_arena)?
            .map(ParseTree::unwrap_start_name))
    }
    fn parse_s_ll1(
        &mut self,
        i: u32,
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_name_ll1(start, Some((SlotId(3), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
