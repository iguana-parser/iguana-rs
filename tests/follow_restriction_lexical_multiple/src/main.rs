...
use follow_restriction_lexical_multiple::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{
        FollowRestrictionLexicalMultipleParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
#[derive(ClapParser)]
#[command(about = "Parser for the FollowRestrictionLexicalMultiple grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the FollowRestrictionLexicalMultiple parser in REPL mode"]
    ///
...
fn main() -> Result<(), io::Error> {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder =
                    FollowRestrictionLexicalMultipleParseTreeBuilder::new(&ctx);
                let mut parser =
                    FollowRestrictionLexicalMultipleParser::new(&input, start_nonterminal_id);
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let ctx = ParseContext::new();
            let parse_tree_builder = FollowRestrictionLexicalMultipleParseTreeBuilder::new(&ctx);
            let mut parser =
                FollowRestrictionLexicalMultipleParser::new(&input, start_nonterminal_id);
            match parser.run() {
                ParseResult::Success(success) => {
    ...
    let ctx = ParseContext::new();
    let mut parser = FollowRestrictionLexicalMultipleParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
