...
use follow_restriction_lexical_multiple::{
    FollowRestrictionLexicalMultipleGrammar, FollowRestrictionLexicalMultipleParser,
};
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in FollowRestrictionLexicalMultipleGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: FollowRestrictionLexicalMultipleGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: FollowRestrictionLexicalMultipleGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: FollowRestrictionLexicalMultipleGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = FollowRestrictionLexicalMultipleParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            FollowRestrictionLexicalMultipleGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    FollowRestrictionLexicalMultipleGrammar :: nonterminal_id (& format ! ("Start{}" , name)) . ok_or_else (|| io :: Error :: new (io :: ErrorKind :: InvalidInput , format ! ("Unknown start nonterminal: '{}'. Use --list-nonterminals to see the valid names." , name ,) ,))
}
fn run_batch(
    dir: &Path,
    ext: Option<&str>,
