...
use crate::{
    grammar::*,
    parse_tree::{
        self, FollowRestrictionLexicalMultipleParseTreeBuilder, ParseTree, create_parse_tree,
    },
...
impl<'i, 'arena> Parser<'i, 'arena> for FollowRestrictionLexicalMultipleParser<'i, 'arena> {
    ...
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        FollowRestrictionLexicalMultipleParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = FollowRestrictionLexicalMultipleParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(12) => {
                        if let Some(right_child) = self
                            .parse_element_ll1(input_index, Some((SlotId(12), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_0 : Element.
                            input_index = j;
                    ...
                    SlotId(19) => {
                        if let Some(right_child) = self
                            .parse_element_ll1(input_index, Some((SlotId(19), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
...
pub struct FollowRestrictionLexicalMultipleParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: FollowRestrictionLexicalMultipleScanner<'i, 'arena>,
...
impl<'i, 'arena> FollowRestrictionLexicalMultipleParser<'i, 'arena> {
            ...
            match matched {
                ...
                TerminalId(3) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(3), start, SlotId(5), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
