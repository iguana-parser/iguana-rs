// @generated by iguana. Do not edit manually.

use crate::{grammar, parser::FollowRestrictionLexicalMultipleParser};
use iguana_runtime::{
    arena::Arena,
    ids::{NonterminalId, SlotId, TerminalId},
    input::Span,
    parse_tree::{NodeKind, OneOrMany, Origin, ParseTreeBuilder, ParseTreeNode, visit_sppf},
    sppf::{NonterminalNode, SPPFNodeId, TerminalNode},
};
use std::vec::IntoIter;
#[derive(Debug, Clone, Copy)]
pub enum TokenKind {
    // Num
    T0,
    // Alpha
    T1,
    // Dot
    T2,
    // Word
    T3,
    // WS
    T4,
}
impl TokenKind {
    pub fn name(&self) -> &'static str {
        match self {
            TokenKind::T0 => "Num",
            TokenKind::T1 => "Alpha",
            TokenKind::T2 => "Dot",
            TokenKind::T3 => "Word",
            TokenKind::T4 => "WS",
        }
    }
}
#[derive(Debug)]
pub struct Start<T, L> {
    pub before: L,
    pub node: T,
    pub after: L,
    pub span: Span,
}
pub type StartS<'a> = Start<&'a S<'a>, Token>;
pub type StartElement<'a> = Start<&'a Element<'a>, Token>;
#[derive(Debug, Clone, Copy)]
pub enum ParseTree<'a> {
    S(&'a S<'a>),
    Element(&'a Element<'a>),
    // Element+
    Plus0(&'a Plus0<'a>),
    // S
    StartS(&'a Start<&'a S<'a>, Token>),
    // Element
    StartElement(&'a Start<&'a Element<'a>, Token>),
    Token(Token),
}
impl<'a> ParseTree<'a> {
    pub fn children(&self) -> Vec<ParseTree<'a>> {
        match self {
            ParseTree::S(s) => (0..s.child_count()).filter_map(|i| s.child(i)).collect(),
            ParseTree::Element(element) => (0..element.child_count())
                .filter_map(|i| element.child(i))
                .collect(),
            ParseTree::Plus0(plus_0) => match plus_0 {
                Plus0::Amb(alts) => alts.iter().copied().map(ParseTree::Plus0).collect(),
                _ => plus_0.iter().collect(),
            },
            ParseTree::StartS(start_s) => (0..start_s.child_count())
                .filter_map(|i| start_s.child(i))
                .collect(),
            ParseTree::StartElement(start_element) => (0..start_element.child_count())
                .filter_map(|i| start_element.child(i))
                .collect(),
            ParseTree::Token(_) => vec![],
        }
    }
    pub fn display_name(&self) -> &'static str {
        match self {
            ParseTree::S(s) => s.display_name(),
            ParseTree::Element(element) => element.display_name(),
            ParseTree::Plus0(plus_0) => plus_0.display_name(),
            ParseTree::StartS(start_s) => start_s.display_name(),
            ParseTree::StartElement(start_element) => start_element.display_name(),
            ParseTree::Token(token) => token.kind.name(),
        }
    }
    pub fn child_count(&self) -> usize {
        match self {
            ParseTree::S(s) => s.child_count(),
            ParseTree::Element(element) => element.child_count(),
            ParseTree::Plus0(plus_0) => plus_0.child_count(),
            ParseTree::StartS(start_s) => start_s.child_count(),
            ParseTree::StartElement(start_element) => start_element.child_count(),
            ParseTree::Token(_) => 0,
        }
    }
    pub fn span(&self) -> Span {
        match self {
            ParseTree::S(s) => s.span(),
            ParseTree::Element(element) => element.span(),
            ParseTree::Plus0(plus_0) => plus_0.span(),
            ParseTree::StartS(start_s) => start_s.span(),
            ParseTree::StartElement(start_element) => start_element.span(),
            ParseTree::Token(token) => token.span(),
        }
    }
    ///True when this node is an ambiguity cluster (any `*::Amb` variant). The
    ///uniform way to detect ambiguity without matching each nonterminal's enum.
    pub fn is_amb(&self) -> bool {
        match self {
            ParseTree::S(s) => matches!(s, S::Amb(_)),
            ParseTree::Element(element) => matches!(element, Element::Amb(_)),
            ParseTree::Plus0(plus_0) => matches!(plus_0, Plus0::Amb(_)),
            ParseTree::StartS(_) => false,
            ParseTree::StartElement(_) => false,
            ParseTree::Token(_) => false,
        }
    }
    ///Pointer identity of the underlying node, or `None` for tokens (by-value
    ///leaves that are never shared). Two parse trees with the same `node_id` are
    ///the same allocation, i.e. a node shared between parents in the ambiguity DAG.
    pub fn node_id(&self) -> Option<usize> {
        match self {
            ParseTree::S(s) => Some(*s as *const _ as usize),
            ParseTree::Element(element) => Some(*element as *const _ as usize),
            ParseTree::Plus0(plus_0) => Some(*plus_0 as *const _ as usize),
            ParseTree::StartS(start_s) => Some(*start_s as *const _ as usize),
            ParseTree::StartElement(start_element) => Some(*start_element as *const _ as usize),
            ParseTree::Token(_) => None,
        }
    }
    pub fn origin(&self) -> Option<Origin> {
        match self {
            ParseTree::S(s) => s.origin(),
            ParseTree::Element(element) => element.origin(),
            ParseTree::Plus0(plus_0) => plus_0.origin(),
            ParseTree::StartS(start_s) => start_s.origin(),
            ParseTree::StartElement(start_element) => start_element.origin(),
            ParseTree::Token(_) => None,
        }
    }
    pub(crate) fn unwrap_s(self) -> &'a S<'a> {
        match self {
            ParseTree::S(s) => s,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_element(self) -> &'a Element<'a> {
        match self {
            ParseTree::Element(element) => element,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_plus_0(self) -> &'a Plus0<'a> {
        match self {
            ParseTree::Plus0(plus_0) => plus_0,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_start_s(self) -> &'a Start<&'a S<'a>, Token> {
        match self {
            ParseTree::StartS(start_s) => start_s,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_start_element(self) -> &'a Start<&'a Element<'a>, Token> {
        match self {
            ParseTree::StartElement(start_element) => start_element,
            _ => panic!(),
        }
    }
    fn unwrap_token(self) -> Token {
        match self {
            ParseTree::Token(t) => t,
            _ => panic!(),
        }
    }
}
pub trait ListNode<'a> {
    fn iter(&'a self) -> IntoIter<ParseTree<'a>>;
}
pub trait OptNode {
    type Inner;
    fn value(&self) -> Option<&Self::Inner>;
}
// S = Element+
#[derive(Debug)]
pub enum S<'a> {
    Alt0 { elements: &'a Plus0<'a>, span: Span },
    Amb(&'a [&'a S<'a>]),
}
#[derive(Debug)]
pub enum Element<'a> {
    // Element = Num
    Alt0 { num: Token, span: Span },
    // Element = Word
    Alt1 { word: Token, span: Span },
    // Element = Dot
    Alt2 { dot: Token, span: Span },
    Amb(&'a [&'a Element<'a>]),
}
// Element+
#[derive(Debug)]
pub enum Plus0<'a> {
    // Plus_0 = Element+ WS Element
    Alt0 {
        elements_0: &'a Plus0<'a>,
        ws: Token,
        element_2: &'a Element<'a>,
        span: Span,
    },
    // Plus_0 = Element
    Alt1 {
        element: &'a Element<'a>,
        span: Span,
    },
    Amb(&'a [&'a Plus0<'a>]),
}
impl<'a> S<'a> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::S(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 { elements, .. } => match index {
                0 => Some(ParseTree::Plus0(elements)),
                _ => None,
            },
            S::Amb(alts) => alts.get(index).copied().map(ParseTree::S),
        }
    }
    pub fn child_count(&self) -> usize {
        match self {
            S::Alt0 { .. } => 1usize,
            S::Amb(alts) => alts.len(),
        }
    }
    pub fn span(&self) -> Span {
        match self {
            S::Alt0 { span, .. } => *span,
            S::Amb(alts) => alts[0].span(),
        }
    }
    pub fn display_name(&self) -> &'static str {
        match self {
            S::Amb(_) => "Amb",
            _ => "S",
        }
    }
    pub fn origin(&self) -> Option<Origin> {
        None
    }
    pub fn elements(&self) -> &'a Plus0<'a> {
        match self {
            S::Alt0 { elements, .. } => elements,
            S::Amb(_) => panic!("S is ambiguous"),
        }
    }
}
impl<'a> Element<'a> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::Element(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            Element::Alt0 { num, .. } => match index {
                0 => Some(ParseTree::Token(*num)),
                _ => None,
            },
            Element::Alt1 { word, .. } => match index {
                0 => Some(ParseTree::Token(*word)),
                _ => None,
            },
            Element::Alt2 { dot, .. } => match index {
                0 => Some(ParseTree::Token(*dot)),
                _ => None,
            },
            Element::Amb(alts) => alts.get(index).copied().map(ParseTree::Element),
        }
    }
    pub fn child_count(&self) -> usize {
        match self {
            Element::Alt0 { .. } => 1usize,
            Element::Alt1 { .. } => 1usize,
            Element::Alt2 { .. } => 1usize,
            Element::Amb(alts) => alts.len(),
        }
    }
    pub fn span(&self) -> Span {
        match self {
            Element::Alt0 { span, .. } => *span,
            Element::Alt1 { span, .. } => *span,
            Element::Alt2 { span, .. } => *span,
            Element::Amb(alts) => alts[0].span(),
        }
    }
    pub fn display_name(&self) -> &'static str {
        match self {
            Element::Amb(_) => "Amb",
            _ => "Element",
        }
    }
    pub fn origin(&self) -> Option<Origin> {
        None
    }
}
impl<'a> Plus0<'a> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::Plus0(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            Plus0::Alt0 {
                elements_0,
                ws,
                element_2,
                ..
            } => match index {
                0 => Some(ParseTree::Plus0(elements_0)),
                1 => Some(ParseTree::Token(*ws)),
                2 => Some(ParseTree::Element(element_2)),
                _ => None,
            },
            Plus0::Alt1 { element, .. } => match index {
                0 => Some(ParseTree::Element(element)),
                _ => None,
            },
            Plus0::Amb(alts) => alts.get(index).copied().map(ParseTree::Plus0),
        }
    }
    pub fn child_count(&self) -> usize {
        match self {
            Plus0::Alt0 { .. } => 3usize,
            Plus0::Alt1 { .. } => 1usize,
            Plus0::Amb(alts) => alts.len(),
        }
    }
    pub fn span(&self) -> Span {
        match self {
            Plus0::Alt0 { span, .. } => *span,
            Plus0::Alt1 { span, .. } => *span,
            Plus0::Amb(alts) => alts[0].span(),
        }
    }
    pub fn display_name(&self) -> &'static str {
        match self {
            Plus0::Amb(_) => "Amb",
            _ => "Element+",
        }
    }
    pub fn origin(&self) -> Option<Origin> {
        match self {
            Plus0::Amb(_) => None,
            _ => Some(Origin::List),
        }
    }
    pub fn elements(&'a self) -> impl Iterator<Item = &'a Element<'a>> {
        self.iter().filter_map(|node| match node {
            ParseTree::Element(r) => Some(r),
            other if other.is_amb() => panic!("Plus_0 is ambiguous"),
            _ => None,
        })
    }
}
impl<'a> Start<&'a S<'a>, Token> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::StartS(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match index {
            0 => Some(ParseTree::Token(self.before)),
            1 => Some(ParseTree::S(self.node)),
            2 => Some(ParseTree::Token(self.after)),
            _ => None,
        }
    }
    pub fn child_count(&self) -> usize {
        3usize
    }
    pub fn span(&self) -> Span {
        self.span
    }
    pub fn display_name(&self) -> &'static str {
        "Start"
    }
    pub fn origin(&self) -> Option<Origin> {
        Some(Origin::Start)
    }
}
impl<'a> Start<&'a Element<'a>, Token> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::StartElement(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match index {
            0 => Some(ParseTree::Token(self.before)),
            1 => Some(ParseTree::Element(self.node)),
            2 => Some(ParseTree::Token(self.after)),
            _ => None,
        }
    }
    pub fn child_count(&self) -> usize {
        3usize
    }
    pub fn span(&self) -> Span {
        self.span
    }
    pub fn display_name(&self) -> &'static str {
        "Start"
    }
    pub fn origin(&self) -> Option<Origin> {
        Some(Origin::Start)
    }
}
impl<'a> ListNode<'a> for Plus0<'a> {
    fn iter(&'a self) -> IntoIter<ParseTree<'a>> {
        let mut items = vec![];
        let mut current = self;
        loop {
            match current {
                Plus0::Alt0 {
                    elements_0: rest,
                    ws,
                    element_2: item,
                    ..
                } => {
                    items.push(item.as_parse_tree());
                    items.push(ws.as_parse_tree());
                    current = rest;
                }
                Plus0::Alt1 { element: item, .. } => {
                    items.push(item.as_parse_tree());
                    break;
                }
                Plus0::Amb(_) => {
                    items.push(ParseTree::Plus0(current));
                    break;
                }
            }
        }
        items.reverse();
        items.into_iter()
    }
}
impl<'a> Element<'a> {
    pub fn as_num(&self) -> Option<&Token> {
        match self {
            Element::Alt0 { num, .. } => Some(num),
            _ => None,
        }
    }
    pub fn as_word(&self) -> Option<&Token> {
        match self {
            Element::Alt1 { word, .. } => Some(word),
            _ => None,
        }
    }
    pub fn as_dot(&self) -> Option<&Token> {
        match self {
            Element::Alt2 { dot, .. } => Some(dot),
            _ => None,
        }
    }
}
#[derive(Debug, Clone, Copy)]
pub struct Token {
    pub kind: TokenKind,
    span: Span,
}
impl Token {
    pub fn as_parse_tree<'a>(&self) -> ParseTree<'a> {
        ParseTree::Token(*self)
    }
    pub fn span(&self) -> Span {
        self.span
    }
}
fn token_kind(terminal_id: TerminalId) -> TokenKind {
    match terminal_id {
        // Num
        TerminalId(0) => TokenKind::T0,
        // Alpha
        TerminalId(1) => TokenKind::T1,
        // Dot
        TerminalId(2) => TokenKind::T2,
        // Word
        TerminalId(3) => TokenKind::T3,
        // WS
        TerminalId(4) => TokenKind::T4,
        _ => unreachable!("Unknown TerminalId: {:?}", terminal_id),
    }
}
pub struct FollowRestrictionLexicalMultipleParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> FollowRestrictionLexicalMultipleParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for FollowRestrictionLexicalMultipleParseTreeBuilder<'a> {
    fn new_nonterminal_node(
        &self,
        nonterminal_node: &NonterminalNode,
        children: OneOrMany<ParseTree<'a>>,
    ) -> ParseTree<'a> {
        match nonterminal_node.nonterminal_id {
            // S
            NonterminalId(0) => match nonterminal_node.return_slot {
                // S = Element+
                SlotId(1) => {
                    let [elements] = children.into_array::<1usize>();
                    ParseTree::S(self.arena.alloc(S::Alt0 {
                        elements: elements.unwrap_plus_0(),
                        span: nonterminal_node.span,
                    }))
                }
                _ => unreachable!(),
            },
            // Element
            NonterminalId(1) => match nonterminal_node.return_slot {
                // Element = Num
                SlotId(3) => {
                    let [num] = children.into_array::<1usize>();
                    ParseTree::Element(self.arena.alloc(Element::Alt0 {
                        num: num.unwrap_token(),
                        span: nonterminal_node.span,
                    }))
                }
                // Element = Word
                SlotId(5) => {
                    let [word] = children.into_array::<1usize>();
                    ParseTree::Element(self.arena.alloc(Element::Alt1 {
                        word: word.unwrap_token(),
                        span: nonterminal_node.span,
                    }))
                }
                // Element = Dot
                SlotId(7) => {
                    let [dot] = children.into_array::<1usize>();
                    ParseTree::Element(self.arena.alloc(Element::Alt2 {
                        dot: dot.unwrap_token(),
                        span: nonterminal_node.span,
                    }))
                }
                _ => unreachable!(),
            },
            // Plus_0
            NonterminalId(2) => match nonterminal_node.return_slot {
                // Plus_0 = Element+ WS Element
                SlotId(11) => {
                    let [elements_0, ws, element_2] = children.into_array::<3usize>();
                    ParseTree::Plus0(self.arena.alloc(Plus0::Alt0 {
                        elements_0: elements_0.unwrap_plus_0(),
                        ws: ws.unwrap_token(),
                        element_2: element_2.unwrap_element(),
                        span: nonterminal_node.span,
                    }))
                }
                // Plus_0 = Element
                SlotId(13) => {
                    let [element] = children.into_array::<1usize>();
                    ParseTree::Plus0(self.arena.alloc(Plus0::Alt1 {
                        element: element.unwrap_element(),
                        span: nonterminal_node.span,
                    }))
                }
                _ => unreachable!(),
            },
            // StartS
            NonterminalId(3) => match nonterminal_node.return_slot {
                // StartS = WS start:S WS
                SlotId(17) => {
                    let [ws_0, start, ws_2] = children.into_array::<3usize>();
                    ParseTree::StartS(self.arena.alloc(Start {
                        before: ws_0.unwrap_token(),
                        node: start.unwrap_s(),
                        after: ws_2.unwrap_token(),
                        span: nonterminal_node.span,
                    }))
                }
                _ => unreachable!(),
            },
            // StartElement
            NonterminalId(4) => match nonterminal_node.return_slot {
                // StartElement = WS start:Element WS
                SlotId(21) => {
                    let [ws_0, start, ws_2] = children.into_array::<3usize>();
                    ParseTree::StartElement(self.arena.alloc(Start {
                        before: ws_0.unwrap_token(),
                        node: start.unwrap_element(),
                        after: ws_2.unwrap_token(),
                        span: nonterminal_node.span,
                    }))
                }
                _ => unreachable!(),
            },
            _ => unreachable!(),
        }
    }
    fn new_token(&self, terminal_node: &TerminalNode) -> ParseTree<'a> {
        ParseTree::Token(Token {
            kind: token_kind(terminal_node.terminal_id),
            span: terminal_node.span,
        })
    }
    fn new_ambiguity_node(
        &self,
        parent: NonterminalId,
        alternatives: Vec<ParseTree<'a>>,
    ) -> ParseTree<'a> {
        match parent {
            grammar::S => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_s()));
                ParseTree::S(self.arena.alloc(S::Amb(slice)))
            }
            grammar::ELEMENT => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_element()));
                ParseTree::Element(self.arena.alloc(Element::Amb(slice)))
            }
            grammar::PLUS_0 => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_plus_0()));
                ParseTree::Plus0(self.arena.alloc(Plus0::Amb(slice)))
            }
            grammar::START_S => {
                let first = alternatives[0].unwrap_start_s();
                let inner = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_start_s().node));
                let node = &*self.arena.alloc(S::Amb(inner));
                ParseTree::StartS(self.arena.alloc(Start {
                    before: first.before,
                    node,
                    after: first.after,
                    span: first.span,
                }))
            }
            grammar::START_ELEMENT => {
                let first = alternatives[0].unwrap_start_element();
                let inner = self.arena.alloc_slice(
                    alternatives
                        .into_iter()
                        .map(|a| a.unwrap_start_element().node),
                );
                let node = &*self.arena.alloc(Element::Amb(inner));
                ParseTree::StartElement(self.arena.alloc(Start {
                    before: first.before,
                    node,
                    after: first.after,
                    span: first.span,
                }))
            }
            _ => unreachable!("nonterminal cannot be ambiguous"),
        }
    }
}
// The parse tree of a finished parse, from the nonterminal node that spans the input.
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &FollowRestrictionLexicalMultipleParser,
    builder: &FollowRestrictionLexicalMultipleParseTreeBuilder<'a>,
) -> ParseTree<'a> {
    let iguana_runtime::sppf::SPPFNode::Nonterminal(root) =
        iguana_runtime::parser::Parser::sppf_node(parser, root_id)
    else {
        panic!("the SPPF root is not a nonterminal node")
    };
    match root.nonterminal_id {
        grammar::S => ParseTree::S(visit_sppf(root_id, parser, builder).unwrap_one().unwrap_s()),
        grammar::ELEMENT => ParseTree::Element(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_element(),
        ),
        grammar::PLUS_0 => ParseTree::Plus0(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_plus_0(),
        ),
        grammar::START_S => ParseTree::StartS(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_s(),
        ),
        grammar::START_ELEMENT => ParseTree::StartElement(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_element(),
        ),
        _ => panic!(),
    }
}
impl<'a> ParseTreeNode for ParseTree<'a> {
    fn children(&self) -> Vec<Self> {
        ParseTree::children(self)
    }
    fn display_name(&self) -> &'static str {
        ParseTree::display_name(self)
    }
    fn span(&self) -> Span {
        ParseTree::span(self)
    }
    fn kind(&self) -> NodeKind {
        if self.is_amb() {
            NodeKind::Amb
        } else if matches!(self, ParseTree::Token(_)) {
            NodeKind::Token
        } else {
            NodeKind::Nonterminal
        }
    }
    fn node_id(&self) -> Option<usize> {
        ParseTree::node_id(self)
    }
    fn origin(&self) -> Option<Origin> {
        ParseTree::origin(self)
    }
}
