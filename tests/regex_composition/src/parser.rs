...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, RegexCompositionParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for RegexCompositionParser<'i, 'arena> {
    type Grammar = RegexCompositionGrammar;
    type ConcreteParser<'input, 'parser_arena> = RegexCompositionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        RegexCompositionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = RegexCompositionParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(15) => {
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(15), Some(gss_node_id))
                {
                    // StartS : WS . start:S WS
            ...
            SlotId(17) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(17), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(18) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(5), SlotId(18), gss_node_id);
            ...
            SlotId(22) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(6), SlotId(22), gss_node_id);
...
impl<'i, 'arena> RegexCompositionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: RegexCompositionScanner::new(input, vec_arena),
