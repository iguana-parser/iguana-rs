...
use exclude_by_label::{
    parse_tree::{ExcludeByLabelParseTreeBuilder, create_parse_tree, to_json, to_sexpr},
    parser::{ExcludeByLabelParser, NONTERMINALS, SLOTS, TERMINALS},
};
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = ExcludeByLabelParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = ExcludeByLabelParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = ExcludeByLabelParseTreeBuilder;
