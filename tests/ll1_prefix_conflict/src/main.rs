...
use ll1_prefix_conflict::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{Ll1PrefixConflictParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the Ll1PrefixConflict grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Ll1PrefixConflict parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = Ll1PrefixConflictParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser =
                    Ll1PrefixConflictParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let parse_tree_builder = Ll1PrefixConflictParseTreeBuilder::new(&tree_arena);
            let vec_arena = Bump::new();
            let mut parser = Ll1PrefixConflictParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
                ParseResult::Success(success) => {
