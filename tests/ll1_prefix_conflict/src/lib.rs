...
use parse_tree::*;
use parser::Ll1PrefixConflictParser;
use std::error::Error;
...
pub fn parse_t<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a T<'a>, ()>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = Ll1PrefixConflictParser::new(input, grammar_data::START_T, &vec_arena);
    match parser.run() {
...
pub fn parse_s<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = Ll1PrefixConflictParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
