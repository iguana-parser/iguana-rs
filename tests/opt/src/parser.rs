...
use crate::{
    grammar::*,
    parse_tree::{self, OptParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::OptScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for OptParser<'i, 'arena> {
    type Grammar = OptGrammar;
    type ConcreteParser<'input, 'parser_arena> = OptParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        OptParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = OptParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(0) => {
                        if let Some(right_child) =
                            self.parse_opt_0_ll1(input_index, Some((SlotId(0), Some(gss_node_id))))
                        {
                    ...
                    SlotId(4) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(4), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_0 : A.
                            input_index = j;
                    ...
                    SlotId(7) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(7), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartS : start:S.
                            input_index = j;
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartA : start:A.
                            input_index = j;
    ...
    fn follow_set(&self, nonterminal_id: NonterminalId) -> &'static TerminalSet {
        match nonterminal_id {
            ...
            NonterminalId(2) => &FOLLOW_SET_OPT_0,
...
impl<'i, 'arena> OptParser<'i, 'arena> {
    ...
    fn parse_s_ll1(
        ...
        let right_child = {
            let start = j;
            let node = self.parse_opt_0_ll1(start, Some((SlotId(1), None)))?;
    ...
    fn parse_opt_0_ll1(
        ...
        match matched {
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let node = self.parse_a_ll1(start, Some((SlotId(5), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
