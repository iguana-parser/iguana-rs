...
use opt::{
    ...
    parse_tree::{OptParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::OptParser,
};
#[cfg(feature = "profile")]
...
#[derive(ClapParser)]
#[command(about = "Parser for the Opt grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Opt parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = OptParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = OptParser::new(&input, start_nonterminal_id, &vec_arena);
