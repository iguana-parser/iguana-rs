...
use opt::{
    ...
    parser::{NONTERMINALS, OptParser, SLOTS, TERMINALS},
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id =
        OptParser::nonterminal_id(&start_nonterminal_name).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("Unknown nonterminal: '{}'", start_nonterminal_name),
            )
        })?;
    let mut parser = OptParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = OptParseTreeBuilder;
