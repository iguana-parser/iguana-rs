...
use ll1_call_error::{
    ...
    parse_tree::{Ll1CallErrorParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the Ll1CallError grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Ll1CallError parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = Ll1CallErrorParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = Ll1CallErrorParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser =
                        Ll1CallErrorParser::new(&input, start_nonterminal_id, &vec_arena);
