...
use ll1_plus_follow_prefix::{Ll1PlusFollowPrefixGrammar, Ll1PlusFollowPrefixParser};
#[cfg(feature = "profile")]
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in Ll1PlusFollowPrefixGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: Ll1PlusFollowPrefixGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: Ll1PlusFollowPrefixGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: Ll1PlusFollowPrefixGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = Ll1PlusFollowPrefixParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            Ll1PlusFollowPrefixGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, Ll1PlusFollowPrefixGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    Ll1PlusFollowPrefixGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
