...
use ll1_plus_follow_prefix::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{Ll1PlusFollowPrefixParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::Ll1PlusFollowPrefixParser,
};
#[cfg(feature = "profile")]
...
#[derive(ClapParser)]
#[command(about = "Parser for the Ll1PlusFollowPrefix grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Ll1PlusFollowPrefix parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = Ll1PlusFollowPrefixParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser =
                    Ll1PlusFollowPrefixParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    let tree_arena = Bump::new();
    let vec_arena = Bump::new();
    let mut parser = Ll1PlusFollowPrefixParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
