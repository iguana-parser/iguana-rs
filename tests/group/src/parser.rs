...
use crate::{
    grammar::*,
    parse_tree::{self, GroupParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for GroupParser<'i, 'arena> {
    type Grammar = GroupGrammar;
    type ConcreteParser<'input, 'parser_arena> = GroupParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        GroupParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = GroupParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(0) => {
                        if let Some(right_child) = self
                            .parse_group_0_ll1(input_index, Some((SlotId(0), Some(gss_node_id))))
                        {
                    ...
                    SlotId(8) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(8), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Group_0 : B . C D
                            input_index = j;
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_c_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(10) => {
                        if let Some(right_child) =
                            self.parse_d_ll1(input_index, Some((SlotId(10), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                    ...
                    SlotId(12) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(12), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartA : start:A.
                            input_index = j;
                    ...
                    SlotId(14) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(14), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartB : start:B.
                            input_index = j;
                    ...
                    SlotId(16) => {
                        if let Some(right_child) =
                            self.parse_c_ll1(input_index, Some((SlotId(16), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartC : start:C.
                            input_index = j;
                    ...
                    SlotId(18) => {
                        if let Some(right_child) =
                            self.parse_d_ll1(input_index, Some((SlotId(18), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartD : start:D.
                            input_index = j;
...
pub struct GroupParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 20],
    ...
    terminal_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 5],
...
impl<'i, 'arena> GroupParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: GroupScanner::new(input, vec_arena),
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_group_0_ll1(start, Some((SlotId(1), None)))?;
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_b_ll1(start, Some((SlotId(9), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_c_ll1(start, Some((SlotId(10), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_d_ll1(start, Some((SlotId(11), None)))?;
                        let end = self.sppf_node(node).right_extent();
                        j = end;
                        node
                    };
        ...
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    ...
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_GROUP_0),
                );
            }
        }
        result
