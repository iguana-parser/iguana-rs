...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, PlusParseTreeBuilder, create_parse_tree},
    scanner::PlusScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for PlusParser<'i, 'arena> {
    type Grammar = PlusGrammar;
    type ConcreteParser<'input, 'parser_arena> = PlusParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        PlusParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PlusParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(11) => {
                        if let Some(right_child) =
                            self.parse_a_ll1(input_index, Some((SlotId(11), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartA : start:A.
                            input_index = j;
    ...
    fn follow_set(&self, nonterminal_id: NonterminalId) -> &'static TerminalSet {
        match nonterminal_id {
            ...
            NonterminalId(4) => &FOLLOW_SET_START_A,
...
impl<'i, 'arena> PlusParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PlusScanner::new(input, vec_arena),
