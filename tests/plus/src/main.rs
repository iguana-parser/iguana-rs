...
use plus::{
    ...
    parse_tree::{PlusParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::PlusParser,
};
#[cfg(feature = "profile")]
...
#[derive(ClapParser)]
#[command(about = "Parser for the Plus grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Plus parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = PlusParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = PlusParser::new(&input, start_nonterminal_id, &vec_arena);
