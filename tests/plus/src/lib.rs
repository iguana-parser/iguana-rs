...
use parse_tree::PlusParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    let mut parser = PlusParser::new(input, grammar_data::S);
    match parser.run() {
        ...
        ParseResult::Failure(error) => Err(to_parse_error::<PlusParser>(input, &error)),
    }
}
pub fn parse_a(input: &Input) -> Result<ParseSuccess<parse_tree::A>, ParseError> {
    let mut parser = PlusParser::new(input, grammar_data::A);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
