...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, PrefixPostfixPriorityParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for PrefixPostfixPriorityParser<'i, 'arena> {
    type Grammar = PrefixPostfixPriorityGrammar;
    type ConcreteParser<'input, 'parser_arena> = PrefixPostfixPriorityParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        PrefixPostfixPriorityParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PrefixPostfixPriorityParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // E(p: i32) : . [4 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 4)] "!" return 0
            SlotId(9) => {
            ...
            // E(p: i32) : "-" . E(3) return 3
            SlotId(16) => {
                self.create_e(result, gss_node_id, SlotId(17), env, 3);
            }
            // E(p: i32) : "-" E(3) . return 3
            SlotId(17) => {
            ...
            // E(p: i32) : [2 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] "*" . E(2) return 2
            SlotId(23) => {
                self.create_e(result, gss_node_id, SlotId(24), env, 2);
            }
            // E(p: i32) : [2 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] "*" E(2) . return 2
            SlotId(24) => {
...
pub struct PrefixPostfixPriorityParser<'i, 'arena> {
    ...
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 27],
...
impl<'i, 'arena> PrefixPostfixPriorityParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PrefixPostfixPriorityScanner::new(input, vec_arena),
            ...
            dd_intermediate_nodes_index: [const { InlineMap::Empty }; 27],
