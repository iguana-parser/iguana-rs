...
use prefix_postfix_priority::{
    parse_tree::{PrefixPostfixPriorityParseTreeBuilder, create_parse_tree, to_json, to_sexpr},
    parser::{NONTERMINALS, PrefixPostfixPriorityParser, SLOTS, TERMINALS},
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = PrefixPostfixPriorityParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = PrefixPostfixPriorityParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = PrefixPostfixPriorityParseTreeBuilder;
