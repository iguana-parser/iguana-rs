...
use prefix_postfix_priority::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{
        PrefixPostfixPriorityParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
#[derive(ClapParser)]
#[command(about = "Parser for the PrefixPostfixPriority grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the PrefixPostfixPriority parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = PrefixPostfixPriorityParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser =
                    PrefixPostfixPriorityParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    let tree_arena = Bump::new();
    let vec_arena = Bump::new();
    let mut parser = PrefixPostfixPriorityParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
