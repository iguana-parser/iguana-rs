...
use except_regex_reference::{ExceptRegexReferenceGrammar, ExceptRegexReferenceParser};
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in ExceptRegexReferenceGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: ExceptRegexReferenceGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: ExceptRegexReferenceGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: ExceptRegexReferenceGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = ExceptRegexReferenceParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            ExceptRegexReferenceGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, ExceptRegexReferenceGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    ExceptRegexReferenceGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
