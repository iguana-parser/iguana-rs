...
use crate::{
    grammar::*,
    parse_tree::{self, ExceptRegexReferenceParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExceptRegexReferenceScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExceptRegexReferenceParser<'i, 'arena> {
    type Grammar = ExceptRegexReferenceGrammar;
    type ConcreteParser<'input, 'parser_arena> = ExceptRegexReferenceParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExceptRegexReferenceParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExceptRegexReferenceParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> ExceptRegexReferenceParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExceptRegexReferenceScanner::new(input, vec_arena),
