...
use parse_tree::*;
use parser::LayoutAwareFollowNoLayoutParser;
use std::error::Error;
...
pub fn parse_s<'a>(
    ...
    let mut parser = LayoutAwareFollowNoLayoutParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
/// Parses `input` starting from `Word`.
///
/// `tree_arena` holds the constructed parse tree: the returned tree borrows it
/// and lives until the arena is reset or dropped. Once the tree goes out of
/// scope, the arena can be reset with `tree_arena.reset()` and reused for the
...
pub fn parse_word<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Word<'a>, Token>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser =
        LayoutAwareFollowNoLayoutParser::new(input, grammar_data::START_WORD, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = LayoutAwareFollowNoLayoutParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_word(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
