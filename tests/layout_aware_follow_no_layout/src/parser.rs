...
use crate::{
    grammar::*,
    parse_tree::{self, LayoutAwareFollowNoLayoutParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::LayoutAwareFollowNoLayoutScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutAwareFollowNoLayoutParser<'i, 'arena> {
    type Grammar = LayoutAwareFollowNoLayoutGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        LayoutAwareFollowNoLayoutParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LayoutAwareFollowNoLayoutParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LayoutAwareFollowNoLayoutParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // Plus_0 : Plus_0 WS . Word
            SlotId(6) => {
                if let Some(right_child) = self.parse_word_ll1(input_index) {
                    if let Some((j, new_node)) =
                    ...
                    self.add_failure(input_index, SlotId(6), Some(gss_node_id), || {
                        GLLFailureKind::UnexpectedToken {
                            expected: FIRST_SET_WORD.terminals.to_vec(),
                        }
                    });
                }
            }
            ...
            SlotId(14) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(3), SlotId(14), gss_node_id);
            ...
            SlotId(18) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(4), SlotId(18), gss_node_id);
            ...
            SlotId(22) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(5), SlotId(22), gss_node_id);
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(3) => {
                if self
                    .scanner
                    .match_any(&FOLLOW_RESTRICTION_WORD_ALT0_POS0, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow {
                        forbidden: FOLLOW_RESTRICTION_WORD_ALT0_POS0.terminals.to_vec(),
                    })
                } else {
                    None
                }
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(5) => self.scanner.match_any(&FOLLOW_SET_START_WORD, input_index),
            _ => true,
        }
    }
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(1) => FOLLOW_SET_WORD.terminals.to_vec(),
            NonterminalId(2) => FOLLOW_SET_PLUS_0.terminals.to_vec(),
            NonterminalId(3) => FOLLOW_SET_PLUS_1.terminals.to_vec(),
            NonterminalId(4) => FOLLOW_SET_START_S.terminals.to_vec(),
            NonterminalId(5) => FOLLOW_SET_START_WORD.terminals.to_vec(),
            _ => vec![],
        }
    }
...
pub struct LayoutAwareFollowNoLayoutParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: LayoutAwareFollowNoLayoutScanner<'i, 'arena>,
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 23],
...
impl<'i, 'arena> LayoutAwareFollowNoLayoutParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LayoutAwareFollowNoLayoutScanner::new(input, vec_arena),
    ...
    pub fn parse_word(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartWord<'_>>, ParseError> {
        Ok(self
            .parse(START_WORD, tree_arena)?
            .map(ParseTree::unwrap_start_word))
    }
    fn parse_word_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        #[cfg(feature = "instrument")]
        self.ll1_call_log.push((NonterminalId(1), i));
        let matched = self.scanner.longest_match(&FIRST_SET_WORD, i)?;
        match matched {
            TerminalId(0) => {
                let mut j = i;
                let right_child = {
    ...
    fn parse_plus_1_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        ...
        loop {
            ...
            j = pos_0;
            current =
                self.create_intermediate_node_ll1(SlotId(12), left_extent, pos_0, current, node_0);
