...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutAwareFollowNoLayoutParser<'i, 'arena> {
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(6) => {
                        if let Some(right_child) =
                            self.parse_word_ll1(input_index, Some((SlotId(6), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
                    ...
                    SlotId(8) => {
                        if let Some(right_child) =
                            self.parse_word_ll1(input_index, Some((SlotId(8), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_0 : Word.
                            input_index = j;
                    ...
                    SlotId(10) => {
                        if let Some(right_child) = self
                            .parse_plus_1_ll1(input_index, Some((SlotId(10), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_1 : Plus_1 . Char
                            input_index = j;
                    ...
                    SlotId(20) => {
                        if let Some(right_child) =
                            self.parse_word_ll1(input_index, Some((SlotId(20), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
            ...
            SlotId(3) => {
                    ...
                    .match_any(&FOLLOW_RESTRICTION_WORD_ALT0_POS0, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow(
                        &FOLLOW_RESTRICTION_WORD_ALT0_POS0,
                    ))
                } else {
                    None
                }
    ...
    fn follow_set(&self, nonterminal_id: NonterminalId) -> &'static TerminalSet {
        match nonterminal_id {
            ...
            NonterminalId(5) => &FOLLOW_SET_START_WORD,
        ...
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    ...
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_WORD),
                );
            }
        }
        result
