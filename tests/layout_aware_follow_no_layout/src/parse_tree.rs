...
use crate::{grammar, parser::LayoutAwareFollowNoLayoutParser};
use iguana_runtime::{
    arena::Arena,
...
pub type StartWord<'a> = Start<&'a Word<'a>, Token>;
#[derive(Debug, Clone, Copy)]
...
impl<'a> ParseTree<'a> {
    ...
    pub(crate) fn unwrap_word(self) -> &'a Word<'a> {
        match self {
    ...
    pub(crate) fn unwrap_start_word(self) -> &'a Start<&'a Word<'a>, Token> {
        match self {
...
pub fn create_parse_tree<'a>(
    ...
    match root.nonterminal_id {
        ...
        grammar::START_WORD => ParseTree::StartWord(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_word(),
        ),
