...
pub fn parse_e(input: &Input) -> Result<ParseSuccess<parse_tree::E>, ParseError> {
    let mut parser = ExpressionParser::new(input, grammar_data::E);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = std::time::Instant::now();
            let tree = parse_tree::create_parse_tree_e(
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<ExpressionParser>(input, &error))
        }
    }
}
