...
use crate::{
    grammar::*,
    parse_tree::{self, ExpressionParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for ExpressionParser<'i, 'arena> {
    type Grammar = ExpressionGrammar;
    type ConcreteParser<'input, 'parser_arena> = ExpressionParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExpressionParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExpressionParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> ExpressionParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExpressionScanner::new(input, vec_arena),
