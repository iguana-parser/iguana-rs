...
use crate::{
    grammar::*,
    parse_tree::{self, ExactKeywordPrecedeParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExactKeywordPrecedeScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExactKeywordPrecedeParser<'i, 'arena> {
    type Grammar = ExactKeywordPrecedeGrammar;
    type ConcreteParser<'input, 'parser_arena> = ExactKeywordPrecedeParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExactKeywordPrecedeParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExactKeywordPrecedeParseTreeBuilder::new(tree_arena);
        ...
        match slot_id {
            ...
            SlotId(2) => {
                if !(input_index == 0
                    || self
                        .scanner
                        .match_token(TerminalId(5), input_index - 1)
                        .is_none())
                {
                    return;
                }
                if let Some((j, right_child)) =
                    self.match_terminal(TerminalId(3), input_index, SlotId(2), Some(gss_node_id))
                {
            ...
            SlotId(14) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(2), input_index, SlotId(14), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
            ...
            SlotId(15) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(1), SlotId(15), gss_node_id);
    ...
    fn post_conditions(
        ...
        match slot {
            SlotId(3) => {
                if self
                    .scanner
                    .match_any(&FOLLOW_RESTRICTION_S_ALT0_POS2, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow {
                        forbidden: FOLLOW_RESTRICTION_S_ALT0_POS2.terminals.to_vec(),
                    })
                } else {
                    None
                }
...
pub struct ExactKeywordPrecedeParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 16],
    ...
    terminal_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 8],
...
impl<'i, 'arena> ExactKeywordPrecedeParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExactKeywordPrecedeScanner::new(input, vec_arena),
            ...
            intermediate_nodes_index: [const { InlineMap::Empty }; 16],
    ...
    fn parse_s_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        ...
        match matched {
            TerminalId(1) => {
                ...
                if !(j == 0 || self.scanner.match_token(TerminalId(5), j - 1).is_none()) {
                    return None;
                }
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(3), start, SlotId(3), None)?;
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(2), start, SlotId(4), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
                ...
                let right_child = {
                    let start = j;
                    let (end, node) = self.match_terminal(TerminalId(4), start, SlotId(7), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
            ...
            TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let (end, node) =
                        self.match_terminal(TerminalId(2), start, SlotId(10), None)?;
                    j = end;
                    node
                };
                current = self.create_intermediate_node_ll1(
