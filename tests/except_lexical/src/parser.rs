...
impl<'i> Parser<'i> for ExceptLexicalParser<'i> {
    ...
    fn execute(
        ...
        match slot_id {
            ...
            //StartS : Layout . start:S Layout
            SlotId(3) => {
                self.create_s(result, gss_node_id, SlotId(4));
            }
            //StartS : Layout start:S . Layout
            SlotId(4) => {
                let i = input_index;
