// To regenerate parser:  cargo run -p iguana -- generate --grammar tests/except_lexical/except_lexical.iggy --output tests/except_lexical
// To update golden files: REGENERATE=1 cargo test -p iguana-tests --test grammar_tests except_lexical::

use except_lexical::{parse_s, parse_tree::to_sexpr};
use iguana_runtime::input::Input;
use iguana_runtime::parse_tree::ParseContext;
use iguana_runtime::testing::{check_golden_file, golden_path};

const GRAMMAR_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/except_lexical");
