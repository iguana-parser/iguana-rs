...
pub fn parse_e<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = IndirectPrefixParser::new(input, grammar_data::E, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_lambda<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a Lambda<'a>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = IndirectPrefixParser::new(input, grammar_data::LAMBDA, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_body<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = IndirectPrefixParser::new(input, grammar_data::BODY, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
