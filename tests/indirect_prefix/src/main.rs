...
use indirect_prefix::{IndirectPrefixGrammar, IndirectPrefixParser};
#[cfg(feature = "profile")]
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in IndirectPrefixGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: IndirectPrefixGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: IndirectPrefixGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: IndirectPrefixGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = IndirectPrefixParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(tree, IndirectPrefixGrammar::LAYOUT_NAME, display_options)
                    }
            ...
            match parser.run(start_nonterminal_id) {
                GLLResult::Success(success) => {
                    ...
                    cli::ReplOutcome::Parsed {
                        tree: to_sexpr_with(
                            tree,
                            IndirectPrefixGrammar::LAYOUT_NAME,
                            display_options,
                        ),
                        ambiguous,
                    }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, IndirectPrefixGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    IndirectPrefixGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
