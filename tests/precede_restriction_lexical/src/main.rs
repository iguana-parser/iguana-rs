...
use precede_restriction_lexical::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{
        PrecedeRestrictionLexicalParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
#[derive(ClapParser)]
#[command(about = "Parser for the PrecedeRestrictionLexical grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the PrecedeRestrictionLexical parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder =
                    PrecedeRestrictionLexicalParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser =
                    PrecedeRestrictionLexicalParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser = PrecedeRestrictionLexicalParser::new(
                        &input,
                        start_nonterminal_id,
                        &vec_arena,
                    );
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let tree_arena = Bump::new();
            let parse_tree_builder = PrecedeRestrictionLexicalParseTreeBuilder::new(&tree_arena);
            let vec_arena = Bump::new();
            let mut parser =
                PrecedeRestrictionLexicalParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
    ...
    let tree_arena = Bump::new();
    let vec_arena = Bump::new();
    let mut parser = PrecedeRestrictionLexicalParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
