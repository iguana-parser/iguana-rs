...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, TokenOnlyUnsafeParseTreeBuilder, create_parse_tree},
    scanner::TokenOnlyUnsafeScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for TokenOnlyUnsafeParser<'i, 'arena> {
    type Grammar = TokenOnlyUnsafeGrammar;
    type ConcreteParser<'input, 'parser_arena> = TokenOnlyUnsafeParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        TokenOnlyUnsafeParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = TokenOnlyUnsafeParseTreeBuilder::new(tree_arena);
...
impl<'i, 'arena> TokenOnlyUnsafeParser<'i, 'arena> {
    ...
    pub fn parse_mod(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartMod<'_>>, ParseError> {
        Ok(self
            .parse(START_MOD, tree_arena)?
            .map(ParseTree::unwrap_start_mod))
    }
    pub fn parse_empty(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartEmpty<'_>>, ParseError> {
        Ok(self
            .parse(START_EMPTY, tree_arena)?
            .map(ParseTree::unwrap_start_empty))
    }
    pub fn parse_tag(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartTag<'_>>, ParseError> {
        Ok(self
            .parse(START_TAG, tree_arena)?
            .map(ParseTree::unwrap_start_tag))
    }
