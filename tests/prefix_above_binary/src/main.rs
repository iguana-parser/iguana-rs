...
use prefix_above_binary::{
    parse_tree::{PrefixAboveBinaryParseTreeBuilder, create_parse_tree, to_json, to_sexpr},
    parser::{NONTERMINALS, PrefixAboveBinaryParser, SLOTS, TERMINALS},
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = PrefixAboveBinaryParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = PrefixAboveBinaryParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = PrefixAboveBinaryParseTreeBuilder;
