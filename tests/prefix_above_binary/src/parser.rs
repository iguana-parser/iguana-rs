...
use crate::{
    grammar::*,
    parse_tree::{self, ParseTree, PrefixAboveBinaryParseTreeBuilder, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for PrefixAboveBinaryParser<'i, 'arena> {
    type Grammar = PrefixAboveBinaryGrammar;
    type ConcreteParser<'input, 'parser_arena> = PrefixAboveBinaryParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        PrefixAboveBinaryParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = PrefixAboveBinaryParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // E(p: i32) : "-" WS . E(2) return 2
            SlotId(15) => {
                self.create_e(result, gss_node_id, SlotId(16), env, 2);
            }
            // E(p: i32) : "-" WS E(2) . return 2
            SlotId(16) => {
            ...
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] WS "+" WS . E(1) return 1
            SlotId(24) => {
                self.create_e(result, gss_node_id, SlotId(25), env, 1);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] WS "+" WS E(1) . return 1
            SlotId(25) => {
...
impl<'i, 'arena> PrefixAboveBinaryParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: PrefixAboveBinaryScanner::new(input, vec_arena),
