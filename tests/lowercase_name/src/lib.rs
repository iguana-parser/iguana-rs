...
use parse_tree::*;
use parser::LowercaseNameParser;
...
pub fn parse_s<'a>(
    ...
    let mut parser = LowercaseNameParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_b<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = LowercaseNameParser::new(input, grammar_data::START_B, &vec_arena);
    match parser.run() {
        GLLResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
