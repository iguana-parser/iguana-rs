...
use crate::{
    grammar::*,
    parse_tree::{self, LowercaseNameParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::LowercaseNameScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LowercaseNameParser<'i, 'arena> {
    type Grammar = LowercaseNameGrammar;
    type ConcreteParser<'input, 'parser_arena> = LowercaseNameParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LowercaseNameParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LowercaseNameParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(1) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(1), Some(gss_node_id))))
                        {
                    ...
                    SlotId(7) => {
                        if let Some(right_child) =
                            self.parse_b_ll1(input_index, Some((SlotId(7), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartB : start:B.
                            input_index = j;
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(3) => self.scanner.match_any(&FOLLOW_SET_START_B, input_index),
            _ => true,
        }
    }
    fn follow_set(&self, nonterminal_id: NonterminalId) -> &'static TerminalSet {
        match nonterminal_id {
            ...
            NonterminalId(3) => &FOLLOW_SET_START_B,
...
pub struct LowercaseNameParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 9],
...
impl<'i, 'arena> LowercaseNameParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LowercaseNameScanner::new(input, vec_arena),
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let node = self.parse_b_ll1(start, Some((SlotId(2), None)))?;
