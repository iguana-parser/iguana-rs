...
use alt_seq::{AltSeqGrammar, AltSeqParser};
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in AltSeqGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: AltSeqGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: AltSeqGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: AltSeqGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = AltSeqParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(tree, AltSeqGrammar::LAYOUT_NAME, display_options)
                    }
            ...
            match parser.run(start_nonterminal_id) {
                GLLResult::Success(success) => {
                    ...
                    cli::ReplOutcome::Parsed {
                        tree: to_sexpr_with(tree, AltSeqGrammar::LAYOUT_NAME, display_options),
                        ambiguous,
                    }
                }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, AltSeqGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
                ...
                && args.trace.is_none()
            {
                if let Some(ref parse_tree) = parse_tree_opt {
                    ...
                    println!(
                        "{}",
                        to_sexpr_with(*parse_tree, AltSeqGrammar::LAYOUT_NAME, display_options)
                    );
                }
            }
        }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    AltSeqGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
