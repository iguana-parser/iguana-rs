...
use crate::{
    grammar::*,
    parse_tree::{self, ExactKeywordMultipleIdsParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExactKeywordMultipleIdsScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExactKeywordMultipleIdsParser<'i, 'arena> {
    type Grammar = ExactKeywordMultipleIdsGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        ExactKeywordMultipleIdsParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExactKeywordMultipleIdsParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExactKeywordMultipleIdsParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(12) => {
                if let Some((_, right_child)) =
                    self.match_terminal(TerminalId(4), input_index, SlotId(12), Some(gss_node_id))
                {
                    if let Some((j, new_node)) =
...
impl<'i, 'arena> ExactKeywordMultipleIdsParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExactKeywordMultipleIdsScanner::new(input, vec_arena),
