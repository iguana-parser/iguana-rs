...
use exact_keyword_multiple_ids::{ExactKeywordMultipleIdsGrammar, ExactKeywordMultipleIdsParser};
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in ExactKeywordMultipleIdsGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: ExactKeywordMultipleIdsGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: ExactKeywordMultipleIdsGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: ExactKeywordMultipleIdsGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = ExactKeywordMultipleIdsParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            ExactKeywordMultipleIdsGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, ExactKeywordMultipleIdsGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    ExactKeywordMultipleIdsGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
