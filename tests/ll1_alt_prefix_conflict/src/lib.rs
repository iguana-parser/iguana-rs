...
use parse_tree::*;
use parser::Ll1AltPrefixConflictParser;
use std::error::Error;
...
pub fn parse_s<'a>(
    ...
    let mut parser = Ll1AltPrefixConflictParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_x<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = Ll1AltPrefixConflictParser::new(input, grammar_data::START_X, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
