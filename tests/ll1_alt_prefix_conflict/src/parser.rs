...
use crate::{
    grammar::*,
    parse_tree::{self, Ll1AltPrefixConflictParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::Ll1AltPrefixConflictScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for Ll1AltPrefixConflictParser<'i, 'arena> {
    type Grammar = Ll1AltPrefixConflictGrammar;
    type ConcreteParser<'input, 'parser_arena> = Ll1AltPrefixConflictParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        Ll1AltPrefixConflictParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = Ll1AltPrefixConflictParseTreeBuilder::new(tree_arena);
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(3) => self.scanner.match_any(&FOLLOW_SET_START_X, input_index),
            _ => true,
        }
    }
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(1) => FOLLOW_SET_X.terminals.to_vec(),
            NonterminalId(2) => FOLLOW_SET_START_S.terminals.to_vec(),
            NonterminalId(3) => FOLLOW_SET_START_X.terminals.to_vec(),
            _ => vec![],
        }
    }
...
impl<'i, 'arena> Ll1AltPrefixConflictParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: Ll1AltPrefixConflictScanner::new(input, vec_arena),
