...
impl<'i> Parser<'i> for PlusGroupParser<'i> {
    ...
    fn execute(
        ...
        match slot_id {
            ...
            //S_Plus_0 : S_Plus_0 . S_Group_0
            SlotId(13) => {
                if let Some(right_child) = self.parse_s_group_0_ll1(input_index) {
                    if let Some((j, new_node)) = self
            ...
            SlotId(14) => {
                if let Some(nonterminal_node_id) = self
                    .create_nonterminal_node(result, NonterminalId(5), SlotId(14))
                {
            ...
            SlotId(16) => {
                if let Some(nonterminal_node_id) = self
                    .create_nonterminal_node(result, NonterminalId(5), SlotId(16))
                {
    ...
    fn parse_a_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        if self.scanner.match_any(PREDICTION_SET_A_ALT0, i) {
            ...
            let right_child = {
                let start = j;
                let (end, node) = self
                    .match_terminal(TerminalId(0), start, SlotId(3), None, "\"a\"")?;
                j = end;
                node
            };
            let left_extent = self.sppf_node(right_child).left_extent();
    ...
    fn parse_b_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        if self.scanner.match_any(PREDICTION_SET_B_ALT0, i) {
            ...
            let right_child = {
                let start = j;
                let (end, node) = self
                    .match_terminal(TerminalId(1), start, SlotId(5), None, "\"b\"")?;
                j = end;
                node
            };
            let left_extent = self.sppf_node(right_child).left_extent();
    ...
    fn parse_c_ll1(&mut self, i: u32) -> Option<SPPFNodeId> {
        if self.scanner.match_any(PREDICTION_SET_C_ALT0, i) {
            ...
            let right_child = {
                let start = j;
                let (end, node) = self
                    .match_terminal(TerminalId(2), start, SlotId(7), None, "\"c\"")?;
                j = end;
                node
            };
            let left_extent = self.sppf_node(right_child).left_extent();
