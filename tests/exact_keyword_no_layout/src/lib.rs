...
use parse_tree::*;
use parser::ExactKeywordNoLayoutParser;
use std::error::Error;
...
pub fn parse_s<'a>(
    ...
    let mut parser = ExactKeywordNoLayoutParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_word<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordNoLayoutParser::new(input, grammar_data::START_WORD, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
