...
use indirect_postfix::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{IndirectPostfixParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the IndirectPostfix grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the IndirectPostfix parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser =
                    IndirectPostfixParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let parse_tree_builder = IndirectPostfixParseTreeBuilder::new(&tree_arena);
            let vec_arena = Bump::new();
            let mut parser = IndirectPostfixParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
                ParseResult::Success(success) => {
