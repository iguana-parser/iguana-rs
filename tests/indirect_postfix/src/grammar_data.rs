// @generated by iguana. Do not edit manually.

use crate::types::{Nonterminal, Slot, Terminal};
use iguana_runtime::ids::{NonterminalId, SlotId, TerminalId};
pub const NONTERMINALS: [Nonterminal; 5] = [
    Nonterminal {
        name: "S",
        display: "S",
        derived: false,
    },
    Nonterminal {
        name: "StartS",
        display: "S",
        derived: true,
    },
    Nonterminal {
        name: "E",
        display: "E",
        derived: false,
    },
    Nonterminal {
        name: "Postfix",
        display: "Postfix",
        derived: false,
    },
    Nonterminal {
        name: "Body",
        display: "Body",
        derived: false,
    },
];
// User-declared nonterminals in `.iggy` source order. Used by `--list-nonterminals`.
pub const NONTERMINAL_DISPLAY_ORDER: [&str; 4] = ["S", "E", "Postfix", "Body"];
pub const S: NonterminalId = NonterminalId(0);
pub const START_S: NonterminalId = NonterminalId(1);
pub const E: NonterminalId = NonterminalId(2);
pub const POSTFIX: NonterminalId = NonterminalId(3);
pub const BODY: NonterminalId = NonterminalId(4);
pub fn nonterminal_id(name: &str) -> Option<NonterminalId> {
    match name {
        "S" => Some(S),
        "StartS" => Some(START_S),
        "E" => Some(E),
        "Postfix" => Some(POSTFIX),
        "Body" => Some(BODY),
        _ => None,
    }
}
pub const TERMINALS: [Terminal; 6] = [
    Terminal { name: "WS" },
    Terminal { name: "\"a\"" },
    Terminal { name: "\"+\"" },
    Terminal { name: "\"!\"" },
    Terminal { name: "Epsilon" },
    Terminal { name: "EOF" },
];
pub const SLOTS: [Slot; 31] = [
    Slot {
        display_name: "S : . E(0)",
    },
    Slot {
        display_name: "S : E(0).",
    },
    Slot {
        display_name: "S : . WS start:S WS",
    },
    Slot {
        display_name: "S : WS . start:S WS",
    },
    Slot {
        display_name: "S : WS start:S . WS",
    },
    Slot {
        display_name: "S : WS start:S WS.",
    },
    Slot {
        display_name: "E : . \"a\" return 0",
    },
    Slot {
        display_name: "E : \"a\" . return 0",
    },
    Slot {
        display_name: "E : \"a\" return 0.",
    },
    Slot {
        display_name: "E : . [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS \"+\" WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] . l=E(p) [(l == 0) || (l >= 2)] WS \"+\" WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) . [(l == 0) || (l >= 2)] WS \"+\" WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] . WS \"+\" WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS . \"+\" WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS \"+\" . WS E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS \"+\" WS . E(2) return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS \"+\" WS E(2) . return 2",
    },
    Slot {
        display_name: "E : [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS \"+\" WS E(2) return 2.",
    },
    Slot {
        display_name: "E : . [1 >= p] l=Postfix(p) [(l == 0) || (l >= 1)] return 0",
    },
    Slot {
        display_name: "E : [1 >= p] . l=Postfix(p) [(l == 0) || (l >= 1)] return 0",
    },
    Slot {
        display_name: "E : [1 >= p] l=Postfix(p) . [(l == 0) || (l >= 1)] return 0",
    },
    Slot {
        display_name: "E : [1 >= p] l=Postfix(p) [(l == 0) || (l >= 1)] . return 0",
    },
    Slot {
        display_name: "E : [1 >= p] l=Postfix(p) [(l == 0) || (l >= 1)] return 0.",
    },
    Slot {
        display_name: "Postfix : . r=Body(p) WS \"!\" return r",
    },
    Slot {
        display_name: "Postfix : r=Body(p) . WS \"!\" return r",
    },
    Slot {
        display_name: "Postfix : r=Body(p) WS . \"!\" return r",
    },
    Slot {
        display_name: "Postfix : r=Body(p) WS \"!\" . return r",
    },
    Slot {
        display_name: "Postfix : r=Body(p) WS \"!\" return r.",
    },
    Slot {
        display_name: "Body : . r=E(p) return r",
    },
    Slot {
        display_name: "Body : r=E(p) . return r",
    },
    Slot {
        display_name: "Body : r=E(p) return r.",
    },
];
// S { WS, EOF }
pub static FOLLOW_SET_S: &[TerminalId] = &[TerminalId(0), TerminalId(5)];
// S { "a" }
pub static FIRST_SET_S: &[TerminalId] = &[TerminalId(1)];
// S : . E(0) { "a" }
pub static FIRST_SET_S_ALT0: &[TerminalId] = &[TerminalId(1)];
// E { "+", EOF, "!", WS }
pub static FOLLOW_SET_E: &[TerminalId] =
    &[TerminalId(2), TerminalId(5), TerminalId(3), TerminalId(0)];
// E { "a" }
pub static FIRST_SET_E: &[TerminalId] = &[TerminalId(1)];
// E(p: i32) : . "a" return 0 { "a" }
pub static FIRST_SET_E_ALT0: &[TerminalId] = &[TerminalId(1)];
// E(p: i32) : . [2 >= p] l=E(p) [(l == 0) || (l >= 2)] WS "+" WS E(2) return 2 { "a" }
pub static FIRST_SET_E_ALT1: &[TerminalId] = &[TerminalId(1)];
// E(p: i32) : . [1 >= p] l=Postfix(p) [(l == 0) || (l >= 1)] return 0 { "a" }
pub static FIRST_SET_E_ALT2: &[TerminalId] = &[TerminalId(1)];
// Postfix { "+", EOF, "!", WS }
pub static FOLLOW_SET_POSTFIX: &[TerminalId] =
    &[TerminalId(2), TerminalId(5), TerminalId(3), TerminalId(0)];
// Postfix { "a" }
pub static FIRST_SET_POSTFIX: &[TerminalId] = &[TerminalId(1)];
// Postfix(p: i32) : . r=Body(p) WS "!" return r { "a" }
pub static FIRST_SET_POSTFIX_ALT0: &[TerminalId] = &[TerminalId(1)];
// Body { EOF, "!", WS }
pub static FOLLOW_SET_BODY: &[TerminalId] = &[TerminalId(5), TerminalId(3), TerminalId(0)];
// Body { "a" }
pub static FIRST_SET_BODY: &[TerminalId] = &[TerminalId(1)];
// Body(p: i32) : . r=E(p) return r { "a" }
pub static FIRST_SET_BODY_ALT0: &[TerminalId] = &[TerminalId(1)];
// StartS { EOF }
pub static FOLLOW_SET_START_S: &[TerminalId] = &[TerminalId(5)];
// StartS { "a", WS }
pub static FIRST_SET_START_S: &[TerminalId] = &[TerminalId(1), TerminalId(0)];
// StartS : . WS start:S WS { "a", WS }
pub static FIRST_SET_START_S_ALT0: &[TerminalId] = &[TerminalId(1), TerminalId(0)];
