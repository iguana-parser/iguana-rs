...
use crate::{
    grammar::*,
    parse_tree::{self, IndirectPostfixParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::IndirectPostfixScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for IndirectPostfixParser<'i, 'arena> {
    type Grammar = IndirectPostfixGrammar;
    type ConcreteParser<'input, 'parser_arena> = IndirectPostfixParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        IndirectPostfixParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = IndirectPostfixParseTreeBuilder::new(tree_arena);
    ...
    fn add_start_gss_node(
        ...
        match nonterminal_id {
            ...
            NonterminalId(6) => self.add_gss_node_postfix(input_index, 0, gss_node_id),
    ...
    fn record_stats(&self) -> iguana_runtime::instrument::Stats {
        ...
        stats.record(
            "Parser::gss_nodes_index_postfix",
            self.gss_nodes_index_postfix.len(),
        );
        stats.record(
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(6) => self.scanner.match_any(&FOLLOW_SET_POSTFIX, input_index),
            ...
            NonterminalId(3) => self
                .scanner
                .match_any(&FOLLOW_SET_START_POSTFIX, input_index),
    ...
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(6) => FOLLOW_SET_POSTFIX.terminals.to_vec(),
            ...
            NonterminalId(3) => FOLLOW_SET_START_POSTFIX.terminals.to_vec(),
...
pub struct IndirectPostfixParser<'i, 'arena> {
    ...
    // GSS index for nonterminal Postfix
    gss_nodes_index_postfix: InlineMap<'arena, (u32, i32), GssNodeId>,
    // GSS index for nonterminal Body
    gss_nodes_index_body: InlineMap<'arena, (u32, i32), GssNodeId>,
    ...
    dd_intermediate_nodes_index: [InlineMap<'arena, (Span, Option<EnvId>), SPPFNodeId>; 25],
...
impl<'i, 'arena> IndirectPostfixParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: IndirectPostfixScanner::new(input, vec_arena),
            gss_nodes_index: [const { InlineMap::Empty }; 8],
            gss_nodes_index_e: InlineMap::Empty,
            gss_nodes_index_postfix: InlineMap::Empty,
            ...
            dd_intermediate_nodes_index: [const { InlineMap::Empty }; 25],
    ...
    pub fn parse_postfix(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartPostfix<'_>>, ParseError> {
        Ok(self
            .parse(START_POSTFIX, tree_arena)?
            .map(ParseTree::unwrap_start_postfix))
    }
    pub fn parse_body(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartBody<'_>>, ParseError> {
        Ok(self
    ...
    fn create_postfix(
        ...
        // If there is already a GSS node for this call, add an edge.
        if let Some(existing_gss_node_id) = self.get_gss_node_postfix(i, p) {
    ...
    fn get_gss_node_postfix(&self, input_index: u32, p: i32) -> Option<GssNodeId> {
        self.gss_nodes_index_postfix.get(&(input_index, p)).copied()
    }
    ...
    fn add_gss_node_postfix(&mut self, input_index: u32, p: i32, gss_node_id: GssNodeId) {
        let arena = self.vec_arena;
        self.gss_nodes_index_postfix
            .insert((input_index, p), gss_node_id, arena);
    }
