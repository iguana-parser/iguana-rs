...
/// Parses `input` starting from `Program`.
///
...
pub fn parse_program<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Program<'a>, Token>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordParser::new(input, grammar_data::START_PROGRAM, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = ExactKeywordParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_program(
...
/// Parses `input` starting from `Stmt`.
///
/// `tree_arena` holds the constructed parse tree: the returned tree borrows it
/// and lives until the arena is reset or dropped. Once the tree goes out of
/// scope, the arena can be reset with `tree_arena.reset()` and reused for the
...
pub fn parse_stmt<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Stmt<'a>, Token>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordParser::new(input, grammar_data::START_STMT, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = ExactKeywordParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_stmt(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
...
pub fn parse_expr<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = ExactKeywordParser::new(input, grammar_data::START_EXPR, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
