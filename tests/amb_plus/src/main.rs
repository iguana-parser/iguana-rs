...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the AmbPlus parser in REPL mode"]
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbPlusParseTreeBuilder::new(&ctx);
                let mut parser = AmbPlusParser::new(&input, start_nonterminal_id);
