...
use crate::{grammar, parser::AmbPlusParser};
use iguana_runtime::{
    arena::Arena,
...
pub type StartX<'a> = Start<&'a X<'a>, ()>;
pub type StartY<'a> = Start<&'a Y<'a>, ()>;
#[derive(Debug, Clone, Copy)]
...
impl<'a> ParseTree<'a> {
    pub fn children(&self) -> Vec<ParseTree<'a>> {
        match self {
            ...
            ParseTree::StartX(start_x) => (0..start_x.child_count())
                ...
                .collect(),
            ParseTree::StartY(start_y) => (0..start_y.child_count())
                .filter_map(|i| start_y.child(i))
                .collect(),
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ...
            ParseTree::StartY(start_y) => start_y.display_name(),
    ...
    pub fn child_count(&self) -> usize {
        match self {
            ...
            ParseTree::StartY(start_y) => start_y.child_count(),
            ParseTree::Token(_) => 0,
        }
    }
    pub fn span(&self) -> Span {
        match self {
            ...
            ParseTree::StartY(start_y) => start_y.span(),
    ...
    pub fn is_amb(&self) -> bool {
        match self {
            ...
            ParseTree::Y(y) => matches!(y, Y::Amb(_)),
    ...
    pub fn node_id(&self) -> Option<usize> {
        match self {
            ...
            ParseTree::Y(y) => Some(*y as *const _ as usize),
            ...
            ParseTree::StartY(start_y) => Some(*start_y as *const _ as usize),
    ...
    pub fn origin(&self) -> Option<Origin> {
        match self {
            ...
            ParseTree::StartY(start_y) => start_y.origin(),
            ParseTree::Token(_) => None,
        }
    }
    ...
    pub(crate) fn unwrap_x(self) -> &'a X<'a> {
        match self {
            ParseTree::X(x) => x,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_y(self) -> &'a Y<'a> {
        match self {
            ParseTree::Y(y) => y,
            _ => panic!(),
        }
    ...
    pub(crate) fn unwrap_start_x(self) -> &'a Start<&'a X<'a>, ()> {
        match self {
            ParseTree::StartX(start_x) => start_x,
            _ => panic!(),
        }
    }
    pub(crate) fn unwrap_start_y(self) -> &'a Start<&'a Y<'a>, ()> {
        match self {
            ParseTree::StartY(start_y) => start_y,
            _ => panic!(),
        }
    }
...
impl<'a> Start<&'a Y<'a>, ()> {
    pub fn as_parse_tree(&'a self) -> ParseTree<'a> {
        ParseTree::StartY(self)
    }
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match index {
            0 => Some(ParseTree::Y(self.node)),
            _ => None,
        }
    }
    pub fn child_count(&self) -> usize {
        ...
        match nonterminal_node.nonterminal_id {
            ...
            NonterminalId(3) => match nonterminal_node.return_slot {
                ...
                SlotId(10) => {
                    ...
                    ParseTree::Y(self.arena.alloc(Y::Alt0 {
                        lit_0: lit_0.unwrap_token(),
            ...
            NonterminalId(10) => match nonterminal_node.return_slot {
                ...
                SlotId(28) => {
                    let [start] = children.into_array::<1usize>();
                    ParseTree::StartY(self.arena.alloc(Start {
                        before: (),
                        node: start.unwrap_y(),
                        after: (),
                        span: nonterminal_node.span,
    ...
    fn new_ambiguity_node(
        ...
        match parent {
            ...
            grammar::Y => {
                let slice = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_y()));
                ParseTree::Y(self.arena.alloc(Y::Amb(slice)))
            }
            grammar::PLUS_0 => {
                let slice = self
                    .arena
            ...
            grammar::START_Y => {
                let first = alternatives[0].unwrap_start_y();
                let inner = self
                    .arena
                    .alloc_slice(alternatives.into_iter().map(|a| a.unwrap_start_y().node));
                let node = &*self.arena.alloc(Y::Amb(inner));
                ParseTree::StartY(self.arena.alloc(Start {
                    before: first.before,
                    node,
                    after: first.after,
...
pub fn create_parse_tree<'a>(
    ...
    match root.nonterminal_id {
        ...
        grammar::X => ParseTree::X(visit_sppf(root_id, parser, builder).unwrap_one().unwrap_x()),
        grammar::Y => ParseTree::Y(visit_sppf(root_id, parser, builder).unwrap_one().unwrap_y()),
        ...
        grammar::START_X => ParseTree::StartX(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_x(),
        ),
        grammar::START_Y => ParseTree::StartY(
            visit_sppf(root_id, parser, builder)
                .unwrap_one()
                .unwrap_start_y(),
        ),
