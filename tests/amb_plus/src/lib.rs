...
pub fn parse_s<'a>(
    ...
    let mut parser = AmbPlusParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_a<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::START_A, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_x<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a X<'a>, ()>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::START_X, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = AmbPlusParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_x(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
...
pub fn parse_y<'a>(
    input: &Input,
    tree_arena: &'a Arena,
) -> std::result::Result<ParseSuccess<&'a Start<&'a Y<'a>, ()>>, ParseError> {
    let vec_arena = Arena::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::START_Y, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = AmbPlusParseTreeBuilder::new(tree_arena);
            let tree = parse_tree::create_parse_tree_start_y(
                success.sppf_node_id,
                &parser,
                &parse_tree_builder,
            );
