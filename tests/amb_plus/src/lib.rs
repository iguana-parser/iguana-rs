...
pub fn parse_s<'a>(
    ...
    let mut parser = AmbPlusParser::new(input, grammar_data::S, &vec_arena);
    match parser.run() {
...
pub fn parse_a<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::A, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_x<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a X<'a>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::X, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = AmbPlusParseTreeBuilder::new(tree_arena);
            let tree =
                parse_tree::create_parse_tree_x(success.sppf_node_id, &parser, &parse_tree_builder);
            let tree_construction_duration = tree_start.elapsed();
            let ambiguity_node_added = parser.ambiguity_node_added();
            Ok(ParseSuccess {
                tree,
...
pub fn parse_y<'a>(
    input: &Input,
    tree_arena: &'a Bump,
) -> std::result::Result<ParseSuccess<&'a Y<'a>>, ParseError> {
    let vec_arena = Bump::new();
    let mut parser = AmbPlusParser::new(input, grammar_data::Y, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = AmbPlusParseTreeBuilder::new(tree_arena);
            let tree =
                parse_tree::create_parse_tree_y(success.sppf_node_id, &parser, &parse_tree_builder);
            let tree_construction_duration = tree_start.elapsed();
            let ambiguity_node_added = parser.ambiguity_node_added();
            Ok(ParseSuccess {
                tree,
