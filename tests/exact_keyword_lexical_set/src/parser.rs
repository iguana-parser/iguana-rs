...
use crate::{
    grammar::*,
    parse_tree::{self, ExactKeywordLexicalSetParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::ExactKeywordLexicalSetScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExactKeywordLexicalSetParser<'i, 'arena> {
    type Grammar = ExactKeywordLexicalSetGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        ExactKeywordLexicalSetParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ExactKeywordLexicalSetParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExactKeywordLexicalSetParseTreeBuilder::new(tree_arena);
        ...
        match slot_id {
            ...
            SlotId(0) => {
                if !(input_index == 0
                    || self
                        .scanner
                        .match_token(TerminalId(4), input_index - 1)
                        .is_none())
                {
                    return;
                }
...
impl<'i, 'arena> ExactKeywordLexicalSetParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExactKeywordLexicalSetScanner::new(input, vec_arena),
