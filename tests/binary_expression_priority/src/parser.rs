...
use crate::{
    grammar::*,
    parse_tree::{self, BinaryExpressionPriorityParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for BinaryExpressionPriorityParser<'i, 'arena> {
    type Grammar = BinaryExpressionPriorityGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        BinaryExpressionPriorityParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        BinaryExpressionPriorityParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = BinaryExpressionPriorityParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            // E(p: i32) : [2 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] "*" . E(2) return 2
            SlotId(13) => {
                self.create_e(result, gss_node_id, SlotId(14), env, 2);
            }
            // E(p: i32) : [2 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 2)] "*" E(2) . return 2
            SlotId(14) => {
            ...
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] "+" . E(1) return 1
            SlotId(20) => {
                self.create_e(result, gss_node_id, SlotId(21), env, 1);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] "+" E(1) . return 1
            SlotId(21) => {
            ...
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] "-" . E(1) return 1
            SlotId(27) => {
                self.create_e(result, gss_node_id, SlotId(28), env, 1);
            }
            // E(p: i32) : [1 >= p] l_pr=E(p) [(l_pr == 0) || (l_pr >= 1)] "-" E(1) . return 1
            SlotId(28) => {
