...
impl<'i, 'arena> Parser<'i, 'arena> for MultipleExceptParser<'i, 'arena> {
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = MultipleExceptParseTreeBuilder::new(tree_arena);
        ...
        while let Some(descriptor) = self.next_descriptor() {
            ...
            while let Some(slot_id) = next {
                ...
                match slot_id {
                    ...
                    SlotId(4) => {
                        if let Some(right_child) = self.parse_syntax_identifier_ll1(
                            input_index,
                    ...
                    SlotId(6) => {
                        if let Some(right_child) = self.parse_lexical_identifier_ll1(
                            input_index,
                            Some((SlotId(6), Some(gss_node_id))),
                        ) {
                            let j = self.sppf_node(right_child).right_extent();
                            // StartLexicalIdentifier : start:LexicalIdentifier.
                            input_index = j;
        ...
        match slot {
            SlotId(1) => {
                if self
                    ...
                    || self
                        .scanner
                        .match_exact(TerminalId(4), left_extent, right_extent)
                {
                    Some(GLLFailureKind::ExcludedMatch(
                        &EXCEPT_SYNTAX_IDENTIFIER_ALT0_POS0,
                    ))
                } else {
                    None
                }
...
impl<'i, 'arena> MultipleExceptParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: MultipleExceptScanner::new(input, vec_arena),
    ...
    pub fn parse_syntax_identifier(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartSyntaxIdentifier<'_>>, ParseError> {
        Ok(self
            .parse(START_SYNTAX_IDENTIFIER, tree_arena)?
            .map(ParseTree::unwrap_start_syntax_identifier))
    }
    pub fn parse_lexical_identifier(
        self,
        tree_arena: &Arena,
    ) -> Result<ParseSuccess<&parse_tree::StartLexicalIdentifier<'_>>, ParseError> {
        Ok(self
            .parse(START_LEXICAL_IDENTIFIER, tree_arena)?
            .map(ParseTree::unwrap_start_lexical_identifier))
    }
    fn parse_syntax_identifier_ll1(
        &mut self,
        i: u32,
        ...
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    ...
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_SYNTAX_IDENTIFIER),
                );
            }
        }
        result
    }
    fn parse_lexical_identifier_ll1(
        &mut self,
        i: u32,
        failure_context: Option<(SlotId, Option<GssNodeId>)>,
    ) -> Option<SPPFNodeId> {
        ...
        if result.is_none() {
            if let Some((slot_id, gss_node_id)) = failure_context {
                self.add_failure(
                    ...
                    slot_id,
                    gss_node_id,
                    GLLFailureKind::UnexpectedToken(&FIRST_SET_LEXICAL_IDENTIFIER),
                );
            }
        }
        result
