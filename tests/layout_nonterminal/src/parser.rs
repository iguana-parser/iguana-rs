...
use crate::{
    grammar::*,
    parse_tree::{self, LayoutNonterminalParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::LayoutNonterminalScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutNonterminalParser<'i, 'arena> {
    type Grammar = LayoutNonterminalGrammar;
    type ConcreteParser<'input, 'parser_arena> = LayoutNonterminalParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LayoutNonterminalParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LayoutNonterminalParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(2) => {
                        if let Some(right_child) =
                            self.parse_star_0_ll1(input_index, Some((SlotId(2), Some(gss_node_id))))
                        {
                    ...
                    SlotId(8) => {
                        if let Some(right_child) =
                            self.parse_plus_0_ll1(input_index, Some((SlotId(8), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Plus_0 : Plus_0 . Alt_0
                            input_index = j;
                    ...
                    SlotId(9) => {
                        if let Some(right_child) =
                            self.parse_alt_0_ll1(input_index, Some((SlotId(9), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                    ...
                    SlotId(13) => {
                        if let Some(right_child) = self
                            .parse_plus_0_ll1(input_index, Some((SlotId(13), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Opt_0 : Plus_0.
                            input_index = j;
                    ...
                    SlotId(16) => {
                        if let Some(right_child) =
                            self.parse_opt_0_ll1(input_index, Some((SlotId(16), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            // Star_0 : Opt_0.
                            input_index = j;
                    ...
                    SlotId(19) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(19), Some(gss_node_id))))
                        {
                            let j = self.sppf_node(right_child).right_extent();
                            if let Some(new_node) =
...
impl<'i, 'arena> LayoutNonterminalParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LayoutNonterminalScanner::new(input, vec_arena),
    ...
    fn parse_layout_ll1(
        ...
        let result = (|| -> Option<SPPFNodeId> {
            ...
            let right_child = {
                let start = j;
                let node = self.parse_star_0_ll1(start, Some((SlotId(3), None)))?;
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        let start = j;
                        let (end, node) =
                            self.match_terminal(TerminalId(0), start, SlotId(5), None)?;
                        j = end;
                        node
                    };
                    let left_extent = self.sppf_node(right_child).left_extent();
    ...
    fn parse_opt_0_ll1(
        ...
        match matched {
            TerminalId(1) | TerminalId(0) => {
                ...
                let right_child = {
                    let start = j;
                    let node = self.parse_plus_0_ll1(start, Some((SlotId(14), None)))?;
                    let end = self.sppf_node(node).right_extent();
                    j = end;
                    node
                };
    ...
    fn parse_star_0_ll1(
        ...
        let right_child = {
            let start = j;
            let node = self.parse_opt_0_ll1(start, Some((SlotId(17), None)))?;
            let end = self.sppf_node(node).right_extent();
            j = end;
            node
        };
