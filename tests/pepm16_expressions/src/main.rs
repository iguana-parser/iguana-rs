...
use pepm16_expressions::{
    parse_tree::{Pepm16ExpressionsParseTreeBuilder, create_parse_tree, to_json, to_sexpr},
    parser::{NONTERMINALS, Pepm16ExpressionsParser, SLOTS, TERMINALS},
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id = Pepm16ExpressionsParser::nonterminal_id(&start_nonterminal_name)
        .ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let mut parser = Pepm16ExpressionsParser::new(&input, start_nonterminal_id);
    #[cfg(feature = "debug-trace")]
    if cli.trace.is_some() {
        parser.trace_events = Some(vec![]);
    }
    let parse_tree_builder = Pepm16ExpressionsParseTreeBuilder;
