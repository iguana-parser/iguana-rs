...
pub fn parse_s<'a>(
    ...
    let mut parser = Pepm16ExpressionsParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
...
pub fn parse_e<'a>(
    ...
    let vec_arena = Bump::new();
    let mut parser = Pepm16ExpressionsParser::new(input, grammar_data::START_E, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
