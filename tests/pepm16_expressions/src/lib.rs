...
use parse_tree::Pepm16ExpressionsParseTreeBuilder;
...
pub fn parse_s(input: &Input) -> Result<ParseSuccess<parse_tree::S>, ParseError> {
    let mut parser = Pepm16ExpressionsParser::new(input, grammar_data::S);
    match parser.run() {
        ...
        ParseResult::Failure(error) => {
            Err(to_parse_error::<Pepm16ExpressionsParser>(input, &error))
        }
    }
}
