...
#[derive(ClapParser)]
#[command(about = "Parser for the ExceptNonterminalLongestMatch grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the ExceptNonterminalLongestMatch parser in REPL mode"]
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser =
                    ExceptNonterminalLongestMatchParser::new(&input, start_nonterminal_id);
    ...
    if args.repl {
        ...
        cli::run_repl(sexpr_options, |text, sexpr_options| {
            ...
            let ctx = ParseContext::new();
            let parse_tree_builder = ExceptNonterminalLongestMatchParseTreeBuilder::new(&ctx);
            let mut parser = ExceptNonterminalLongestMatchParser::new(&input, start_nonterminal_id);
            match parser.run() {
                ParseResult::Success(success) => {
