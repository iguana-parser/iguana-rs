...
use crate::{
    grammar::*,
    parse_tree::{
        self, ExceptNonterminalLongestMatchParseTreeBuilder, ParseTree, create_parse_tree,
    },
    scanner::ExceptNonterminalLongestMatchScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for ExceptNonterminalLongestMatchParser<'i, 'arena> {
    type Grammar = ExceptNonterminalLongestMatchGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        ExceptNonterminalLongestMatchParser<'input, 'parser_arena>;
    ...
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = ExceptNonterminalLongestMatchParseTreeBuilder::new(tree_arena);
    ...
    fn execute(
        ...
        match slot_id {
            ...
            SlotId(15) => {
                let nonterminal_node_id =
                    self.create_nonterminal_node(result, NonterminalId(5), SlotId(15), gss_node_id);
...
pub struct ExceptNonterminalLongestMatchParser<'i, 'arena> {
    // Arena backing the parser's internal collections; reset in bulk after the parse.
    vec_arena: &'arena Arena,
    scanner: ExceptNonterminalLongestMatchScanner<'i, 'arena>,
...
impl<'i, 'arena> ExceptNonterminalLongestMatchParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: ExceptNonterminalLongestMatchScanner::new(input, vec_arena),
