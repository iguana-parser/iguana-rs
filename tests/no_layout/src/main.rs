...
use no_layout::{
    ...
    parse_tree::{NoLayoutParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
#[derive(ClapParser)]
#[command(about = "Parser for the NoLayout grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the NoLayout parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = NoLayoutParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = NoLayoutParser::new(&input, start_nonterminal_id, &vec_arena);
