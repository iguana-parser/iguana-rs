...
use no_layout::{
    ...
    parser::{NONTERMINALS, NoLayoutParser, SLOTS, TERMINALS},
};
use std::{
    fs::File,
...
fn main() -> Result<(), io::Error> {
    ...
    let start_nonterminal_id =
        NoLayoutParser::nonterminal_id(&start_nonterminal_name).ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
    ...
    let parse_tree_builder = NoLayoutParseTreeBuilder;
