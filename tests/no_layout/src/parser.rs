...
use crate::{
    grammar::*,
    parse_tree::{self, NoLayoutParseTreeBuilder, ParseTree, create_parse_tree},
...
impl<'i, 'arena> Parser<'i, 'arena> for NoLayoutParser<'i, 'arena> {
    type Grammar = NoLayoutGrammar;
    type ConcreteParser<'input, 'parser_arena> = NoLayoutParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        NoLayoutParser::new(input, parser_arena)
    }
...
impl<'i, 'arena> NoLayoutParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: NoLayoutScanner::new(input, vec_arena),
