...
use parse_tree::*;
use parser::DanglingElseUnsafeParser;
use std::error::Error;
...
pub fn parse_s<'a>(
    ...
    let mut parser = DanglingElseUnsafeParser::new(input, grammar_data::START_S, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
            let parse_tree_builder = DanglingElseUnsafeParseTreeBuilder::new(tree_arena);
...
pub fn parse_statement<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser =
        DanglingElseUnsafeParser::new(input, grammar_data::START_STATEMENT, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
...
pub fn parse_layout<'a>(
    ...
    let vec_arena = Arena::new();
    let mut parser = DanglingElseUnsafeParser::new(input, grammar_data::LAYOUT, &vec_arena);
    match parser.run() {
        ParseResult::Success(success) => {
            let parse_duration = success.duration;
            let tree_start = iguana_runtime::Instant::now();
