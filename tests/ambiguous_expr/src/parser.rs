...
use crate::{
    grammar::*,
    parse_tree::{self, AmbiguousExprParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::AmbiguousExprScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for AmbiguousExprParser<'i, 'arena> {
    type Grammar = AmbiguousExprGrammar;
    type ConcreteParser<'input, 'parser_arena> = AmbiguousExprParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        AmbiguousExprParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = AmbiguousExprParseTreeBuilder::new(tree_arena);
...
pub struct AmbiguousExprParser<'i, 'arena> {
    ...
    intermediate_nodes_index: [InlineMap<'arena, Span, SPPFNodeId>; 68],
...
impl<'i, 'arena> AmbiguousExprParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: AmbiguousExprScanner::new(input, vec_arena),
