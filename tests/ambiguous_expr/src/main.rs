use ambiguous_expr::{
    ...
    parse_tree::{AmbiguousExprParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::AmbiguousExprParser,
};
...
#[derive(ClapParser)]
#[command(about = "Parser for the AmbiguousExpr grammar")]
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbiguousExprParseTreeBuilder::new(&tree_arena);
                let vec_arena = Bump::new();
                let mut parser = AmbiguousExprParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser =
                        AmbiguousExprParser::new(&input, start_nonterminal_id, &vec_arena);
