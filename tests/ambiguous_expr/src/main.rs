...
struct Cli {
    ...
    ext: Option<String>,
    #[doc = "Run the AmbiguousExpr parser in REPL mode"]
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = AmbiguousExprParseTreeBuilder::new(&ctx);
                let mut parser = AmbiguousExprParser::new(&input, start_nonterminal_id);
