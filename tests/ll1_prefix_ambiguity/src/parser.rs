...
use crate::{
    grammar::*,
    parse_tree::{self, Ll1PrefixAmbiguityParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::Ll1PrefixAmbiguityScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for Ll1PrefixAmbiguityParser<'i, 'arena> {
    type Grammar = Ll1PrefixAmbiguityGrammar;
    type ConcreteParser<'input, 'parser_arena> = Ll1PrefixAmbiguityParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        Ll1PrefixAmbiguityParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = Ll1PrefixAmbiguityParseTreeBuilder::new(tree_arena);
    ...
    fn follow_set_check(&mut self, nonterminal_id: NonterminalId, input_index: u32) -> bool {
        match nonterminal_id {
            ...
            NonterminalId(1) => self.scanner.match_any(&FOLLOW_SET_EXPR, input_index),
    ...
    fn follow_set_terminals(&self, nonterminal_id: NonterminalId) -> Vec<TerminalId> {
        match nonterminal_id {
            ...
            NonterminalId(1) => FOLLOW_SET_EXPR.terminals.to_vec(),
...
impl<'i, 'arena> Ll1PrefixAmbiguityParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: Ll1PrefixAmbiguityScanner::new(input, vec_arena),
