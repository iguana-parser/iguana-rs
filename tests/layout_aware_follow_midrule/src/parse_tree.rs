...
use crate::{grammar, parser::LayoutAwareFollowMidruleParser};
use iguana_runtime::{
    arena::Arena,
...
impl<'a> S<'a> {
    ...
    pub fn child(&self, index: usize) -> Option<ParseTree<'a>> {
        match self {
            S::Alt0 { id_0, ws, id_2, .. } => match index {
...
pub struct LayoutAwareFollowMidruleParseTreeBuilder<'a> {
    pub arena: &'a Arena,
}
impl<'a> LayoutAwareFollowMidruleParseTreeBuilder<'a> {
    pub fn new(tree_arena: &'a Arena) -> Self {
        Self { arena: tree_arena }
    }
}
impl<'a> ParseTreeBuilder<ParseTree<'a>> for LayoutAwareFollowMidruleParseTreeBuilder<'a> {
...
pub fn create_parse_tree<'a>(
    root_id: SPPFNodeId,
    parser: &LayoutAwareFollowMidruleParser,
    builder: &LayoutAwareFollowMidruleParseTreeBuilder<'a>,
) -> ParseTree<'a> {
