...
use crate::{
    grammar::*,
    parse_tree::{self, LayoutAwareFollowMidruleParseTreeBuilder, ParseTree, create_parse_tree},
    scanner::LayoutAwareFollowMidruleScanner,
};
...
impl<'i, 'arena> Parser<'i, 'arena> for LayoutAwareFollowMidruleParser<'i, 'arena> {
    type Grammar = LayoutAwareFollowMidruleGrammar;
    type ConcreteParser<'input, 'parser_arena> =
        LayoutAwareFollowMidruleParser<'input, 'parser_arena>;
    fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        LayoutAwareFollowMidruleParser::new(input, parser_arena)
    }
    type Tree<'a> = ParseTree<'a>;
    fn build_tree<'a>(&self, root: SPPFNodeId, tree_arena: &'a Arena) -> ParseTree<'a> {
        let builder = LayoutAwareFollowMidruleParseTreeBuilder::new(tree_arena);
                ...
                match slot_id {
                    ...
                    SlotId(1) => {
                        if let Some((j, right_child)) = self.match_terminal(
                            ...
                            if let Some(failure) = self.post_conditions(SlotId(2), input_index, j) {
                                self.add_failure(j, SlotId(2), Some(gss_node_id), failure);
                            } else {
                    ...
                    SlotId(5) => {
                        if let Some(right_child) =
                            self.parse_s_ll1(input_index, Some((SlotId(5), Some(gss_node_id))))
                        {
            ...
            SlotId(2) => {
                    ...
                    .match_any(&FOLLOW_RESTRICTION_S_ALT0_POS1, right_extent)
                {
                    Some(GLLFailureKind::ForbiddenFollow(
                        &FOLLOW_RESTRICTION_S_ALT0_POS1,
                    ))
                } else {
                    None
                }
...
impl<'i, 'arena> LayoutAwareFollowMidruleParser<'i, 'arena> {
    ...
    pub fn new(input: &'i Input, parser_arena: &'arena Arena) -> Self {
        ...
        Self {
            vec_arena,
            scanner: LayoutAwareFollowMidruleScanner::new(input, vec_arena),
            ...
            match matched {
                TerminalId(0) => {
                    ...
                    let right_child = {
                        ...
                        if let Some(failure) = self.post_conditions(SlotId(2), start, end) {
                            self.add_failure(end, SlotId(2), None, failure);
                            return None;
                        }
                        j = end;
                        node
