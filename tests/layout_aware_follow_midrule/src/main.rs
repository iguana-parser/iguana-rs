...
use layout_aware_follow_midrule::{
    grammar_data::{NONTERMINAL_DISPLAY_ORDER, NONTERMINALS, SLOTS, TERMINALS, nonterminal_id},
    parse_tree::{
        LayoutAwareFollowMidruleParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with,
    },
...
#[derive(ClapParser)]
#[command(about = "Parser for the LayoutAwareFollowMidrule grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the LayoutAwareFollowMidrule parser in REPL mode
    ///
...
fn main() -> Result<(), io::Error> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let parse_tree_builder = LayoutAwareFollowMidruleParseTreeBuilder::new(&tree_arena);
                let vec_arena = Arena::new();
                let mut parser =
                    LayoutAwareFollowMidruleParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        for entry in &entries {
            ...
            let report = cli::run_corpus(
                ...
                |path| {
                    ...
                    let mut parser = LayoutAwareFollowMidruleParser::new(
                        &input,
                        start_nonterminal_id,
                        &vec_arena,
                    );
    ...
    let tree_arena = Arena::new();
    let vec_arena = Arena::new();
    let mut parser = LayoutAwareFollowMidruleParser::new(&input, start_nonterminal_id, &vec_arena);
    #[cfg(feature = "debug-trace")]
    if args.trace.is_some() {
