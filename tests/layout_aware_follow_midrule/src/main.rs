...
use layout_aware_follow_midrule::{
    LayoutAwareFollowMidruleGrammar, LayoutAwareFollowMidruleParser,
};
#[cfg(feature = "profile")]
...
fn run() -> io::Result<()> {
    ...
    if args.list_nonterminals {
        for name in LayoutAwareFollowMidruleGrammar::DISPLAY_ORDER.iter() {
            println!("{}", name);
        }
        return Ok(());
    }
    if let Some(ref path) = args.write_symbols {
        let symbols = cli::Symbols {
            nonterminals: LayoutAwareFollowMidruleGrammar::NONTERMINALS
                .iter()
                .map(|nt| nt.display_name.to_string())
                .collect(),
            terminals: LayoutAwareFollowMidruleGrammar::TERMINALS
                .iter()
                .map(|t| t.name.to_string())
                .collect(),
            slots: LayoutAwareFollowMidruleGrammar::SLOTS
                .iter()
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = LayoutAwareFollowMidruleParser::new(&input, &parser_arena);
                let content = match parser.run(start_nonterminal_id) {
                    GLLResult::Success(success) => {
                        ...
                        to_sexpr_with(
                            tree,
                            LayoutAwareFollowMidruleGrammar::LAYOUT_NAME,
                            display_options,
                        )
                    }
    ...
    match result {
        GLLResult::Success(parse_success) => {
            ...
            {
                let json = to_json(*parse_tree, LayoutAwareFollowMidruleGrammar::LAYOUT_NAME);
                let file = File::create(path)?;
                let mut writer = BufWriter::new(file);
                writeln!(writer, "{}", json)?;
            }
...
fn resolve_start_nonterminal(name: &str) -> io::Result<NonterminalId> {
    LayoutAwareFollowMidruleGrammar::nonterminal_id(&format!("Start{}", name)).ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!(
