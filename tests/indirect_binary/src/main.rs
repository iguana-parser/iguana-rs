...
use indirect_binary::{
    ...
    parse_tree::{IndirectBinaryParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
...
fn run() -> io::Result<()> {
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser =
                    IndirectBinaryParser::new(&input, start_nonterminal_id, &vec_arena);
    ...
    if args.repl {
        ...
        cli::run_repl(display_options, |text, display_options| {
            ...
            let parse_tree_builder = IndirectBinaryParseTreeBuilder::new(&tree_arena);
            let vec_arena = Arena::new();
            let mut parser = IndirectBinaryParser::new(&input, start_nonterminal_id, &vec_arena);
            match parser.run() {
                GLLResult::Success(success) => {
